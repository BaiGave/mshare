    ...
    static int compress64k(byte[] src, int srcOff, int srcLen, byte[] dest, int destOff, int destEnd) {
        ...
        if (srcLen >= 13) {
            ...
            block0: while (true) {
                ...
                int tokenOff = dOff++;
                if (LZ4Utils.notEnoughSpace(destEnd - dOff, LZ4Utils.sequenceLength(runLen, 0) + 1 + 5)) {
                ...
                while (true) {
                    ...
                    if (LZ4Utils.notEnoughSpace(destEnd - (dOff += 2), 6 + LZ4Utils.lengthOfEncodedInteger(matchLen))) {
                        throw new LZ4Exception("maxDestLen is too small");
                    }
                    sOff += matchLen;
                    if (matchLen >= 15) {
