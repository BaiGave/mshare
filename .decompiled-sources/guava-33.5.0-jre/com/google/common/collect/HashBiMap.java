...
Serializable {
    ...
    private @Nullable V put(@ParametricNullness K key, @ParametricNullness V value, boolean force) {
        ...
        if (oldEntryForKey != null && valueHash == oldEntryForKey.valueHash && Objects.equals(value, oldEntryForKey.getValue())) {
            return value;
        }
    ...
    private @Nullable K putInverse(@ParametricNullness V value, @ParametricNullness K key, boolean force) {
        ...
        BiEntry<K, V> oldEntryForValue = this.seekByValue(value, valueHash);
        BiEntry<K, V> oldEntryForKey = this.seekByKey(key, keyHash);
        if (oldEntryForValue != null && keyHash == oldEntryForValue.keyHash && Objects.equals(key, oldEntryForValue.getKey())) {
            return key;
        }
        ...
        return new Itr<Map.Entry<K, V>>(){
            ...
            extends AbstractMapEntry<K, V> {
                ...
                public V setValue(@ParametricNullness V value) {
                    Object oldValue = this.delegate.getValue();
                    int valueHash = Hashing.smearedHash(value);
                    if (valueHash == this.delegate.valueHash && Objects.equals(value, oldValue)) {
                        return value;
                    }
                    ...
                    BiEntry newEntry = new BiEntry(this.delegate.getKey(), this.delegate.keyHash, value, valueHash);
    ...
    public void replaceAll(BiFunction<? super K, ? super V, ? extends V> function) {
        ...
        while (entry != null) {
            this.put(entry.getKey(), function.apply(entry.getKey(), entry.getValue()));
            entry = entry.nextInKeyInsertionOrder;
        }
    }
    ...
    Serializable {
            ...
            return new Itr<Map.Entry<V, K>>(){
                ...
                extends AbstractMapEntry<V, K> {
                    ...
                    public K setValue(@ParametricNullness K key) {
                        Object oldKey = this.delegate.getKey();
                        int keyHash = Hashing.smearedHash(key);
                        if (keyHash == this.delegate.keyHash && Objects.equals(key, oldKey)) {
                            return key;
                        }
                        Preconditions.checkArgument(HashBiMap.this.seekByKey(key, keyHash) == null, "value already present: %s", key);
                        HashBiMap.this.delete(this.delegate);
                        BiEntry newEntry = new BiEntry(key, keyHash, this.delegate.getValue(), this.delegate.valueHash);
                        this.delegate = newEntry;
        ...
        public void replaceAll(BiFunction<? super V, ? super K, ? extends K> function) {
            ...
            while (entry != null) {
                this.put((V)entry.getValue(), (K)function.apply(entry.getValue(), entry.getKey()));
                entry = entry.nextInKeyInsertionOrder;
            }
        }
    ...
    implements Iterator<T> {
        @Nullable BiEntry<K, V> next;
        @Nullable BiEntry<K, V> toRemove;
