...
extends ResourceLeakDetector<T> {
    ...
    extends Throwable {
        ...
        public synchronized String getMessage() {
            if (this.message == null) {
                this.message = LeakPresenceDetector.inStaticInitializerSlow(this.getStackTrace()) ? "Resource created in static initializer. Please wrap the static initializer in LeakPresenceDetector.staticInitializer so that this resource is excluded." : "Resource created outside static initializer on thread '" + this.thread.getName() + "', likely leak.";
            }
            return this.message;
        }
    }
    ...
    implements Closeable {
        ...
        void checkOpen() {
            if (this.closed) {
                throw new IllegalStateException("Resource scope '" + this.name + "' already closed");
            }
        }
