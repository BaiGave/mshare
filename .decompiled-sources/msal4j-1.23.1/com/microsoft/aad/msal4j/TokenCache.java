...
implements ITokenCache {
        ...
        try {
            jsonReader.readObject(reader -> {
                block14: while (reader.nextToken() != JsonToken.END_OBJECT) {
                    ...
                    switch (fieldName) {
                        case "AccessToken": {
                            this.deserializeCollection((JsonReader)reader, (Map)this.accessTokens, AccessTokenCacheEntity::fromJson);
                            continue block14;
                        }
                        case "RefreshToken": {
                            this.deserializeCollection((JsonReader)reader, (Map)this.refreshTokens, RefreshTokenCacheEntity::fromJson);
                            continue block14;
                        }
                        case "IdToken": {
                            this.deserializeCollection((JsonReader)reader, (Map)this.idTokens, IdTokenCacheEntity::fromJson);
                            continue block14;
                        }
                        case "Account": {
                            this.deserializeCollection((JsonReader)reader, (Map)this.accounts, AccountCacheEntity::fromJson);
                            continue block14;
                        }
                        case "AppMetadata": {
                            this.deserializeCollection((JsonReader)reader, (Map)this.appMetadata, AppMetadataCacheEntity::fromJson);
                            continue block14;
                        }
                    }
                    reader.skipChildren();
    ...
    private <T> void deserializeCollection(JsonReader reader, Map<String, T> targetCollection, ReadValueCallback<JsonReader, T> deserializer) throws IOException {
        reader.readObject(entityReader -> {
            while (entityReader.nextToken() != JsonToken.END_OBJECT) {
                ...
                Object entity = deserializer.read((JsonReader)entityReader);
    ...
    public String serialize() {
        ...
        try {
            String updatedCache;
            if (!StringHelper.isBlank(this.serializedCachedSnapshot) && (updatedCache = this.mergeWithExistingCache()) != null) {
                String string = updatedCache;
                return string;
            }
            String string = this.serializeToJson();
            return string;
        }
        finally {
    ...
    private <T> void writeCollection(JsonWriter jsonWriter, String collectionName, Map<String, T> collection) throws IOException {
        ...
        for (Map.Entry<String, T> entry : collection.entrySet()) {
            ...
            if (!(entry.getValue() instanceof JsonSerializable)) continue;
    ...
    private static AccessTokenCacheEntity createAccessTokenCacheEntity(TokenRequestExecutor tokenRequestExecutor, AuthenticationResult authenticationResult, String environmentAlias) {
        ...
        String scopes = !StringHelper.isBlank(authenticationResult.scopes()) ? authenticationResult.scopes() : String.join((CharSequence)" ", tokenRequestExecutor.getMsalRequest().msalAuthorizationGrant().getScopes());
        at.target(scopes);
        if (tokenRequestExecutor.getMsalRequest() instanceof OnBehalfOfRequest) {
            OnBehalfOfRequest onBehalfOfRequest = (OnBehalfOfRequest)tokenRequestExecutor.getMsalRequest();
    ...
    Set<IAccount> getAccounts(String clientId) {
        ...
        try (CacheAspect cacheAspect = new CacheAspect(TokenCacheAccessContext.builder().clientId(clientId).tokenCache(this).build());){
            try {
                this.lock.readLock().lock();
                HashMap<String, IAccount> rootAccounts = new HashMap<String, IAccount>();
                for (AccountCacheEntity accCached : this.accounts.values()) {
                    IdTokenCacheEntity idToken = this.idTokens.get(this.getIdTokenKey(accCached.homeAccountId(), accCached.environment(), clientId, accCached.realm()));
                    TenantProfile profile = null;
                    if (idToken != null) {
                        Map<String, Object> idTokenClaims = JsonHelper.parseJsonToMap(JsonHelper.getTokenPayloadClaims(idToken.secret));
                    ...
                    if (rootAccounts.get(accCached.homeAccountId()) == null) {
                        ...
                        ((Account)acc).tenantProfiles = new HashMap<String, ITenantProfile>();
                        rootAccounts.put(accCached.homeAccountId(), acc);
                    }
                    if (profile != null) {
                        ((Account)rootAccounts.get((Object)accCached.homeAccountId())).tenantProfiles.put(accCached.realm(), profile);
                    }
                    if (accCached.localAccountId() == null || !accCached.homeAccountId().contains(accCached.localAccountId())) continue;
                    ((Account)rootAccounts.get(accCached.homeAccountId())).username(accCached.username());
                }
                HashSet hashSet = new HashSet(rootAccounts.values());
                this.lock.readLock().unlock();
                return hashSet;
            }
    ...
    private Optional<AccessTokenCacheEntity> getAccessTokenCacheEntity(IAccount account, Authority authority, Set<String> scopes, String clientId, Set<String> environmentAliases) {
        return this.accessTokens.values().stream().filter(accessToken -> accessToken.homeAccountId != null && accessToken.homeAccountId.equals(account.homeAccountId()) && environmentAliases.contains(accessToken.environment) && accessToken.realm.equals(authority.tenant()) && accessToken.clientId.equals(clientId) && this.isMatchingScopes((AccessTokenCacheEntity)accessToken, scopes)).findAny();
    }
    ...
    private Optional<RefreshTokenCacheEntity> getRefreshTokenCacheEntity(IAccount account, String clientId, Set<String> environmentAliases) {
        return this.refreshTokens.values().stream().filter(refreshToken -> refreshToken.homeAccountId != null && refreshToken.homeAccountId.equals(account.homeAccountId()) && environmentAliases.contains(refreshToken.environment) && refreshToken.clientId.equals(clientId)).findAny();
    }
