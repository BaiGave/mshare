...
implements LoggerContextAware {
    ...
    protected WeakReference<LoggerContext> loggerContext = new WeakReference<Object>(null);
    ...
    public Interpolator(StrLookup defaultLookup, List<String> pluginPackages) {
        ...
        for (Map.Entry<String, PluginType<?>> entry : plugins.entrySet()) {
            try {
                ...
                this.strLookupMap.put(Strings.toRootLowerCase(entry.getKey()), ReflectionUtil.instantiate(clazz));
            }
            catch (Throwable t) {
    ...
    public void setConfiguration(Configuration configuration) {
        ...
        for (StrLookup lookup : this.strLookupMap.values()) {
            if (!(lookup instanceof ConfigurationAware)) continue;
            ((ConfigurationAware)((Object)lookup)).setConfiguration(configuration);
        }
    }
    ...
    public void setLoggerContext(LoggerContext loggerContext) {
        ...
        for (StrLookup lookup : this.strLookupMap.values()) {
            if (!(lookup instanceof LoggerContextAware)) continue;
            ((LoggerContextAware)((Object)lookup)).setLoggerContext(loggerContext);
        }
    }
