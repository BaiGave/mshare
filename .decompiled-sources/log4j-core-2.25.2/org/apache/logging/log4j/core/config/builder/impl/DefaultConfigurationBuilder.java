...
implements ConfigurationBuilder<T> {
    ...
    public ConfigurationBuilder<T> addComponent(ComponentBuilder<?> builder) {
        return this.add(this.root, builder);
    }

    private Optional<Component> getTopLevelComponent(String pluginType) {
        return this.root.getComponents().stream().filter(component -> component.getPluginType().equals(pluginType)).findAny();
    }
    ...
    private void writeXmlConfiguration(XMLStreamWriter xmlWriter) throws XMLStreamException {
        ...
        Optional<Component> monitorResourcesComponent = this.getTopLevelComponent("MonitorResources");
        if (monitorResourcesComponent.isPresent() && !monitorResourcesComponent.get().getComponents().isEmpty()) {
            this.writeXmlComponent(xmlWriter, monitorResourcesComponent.get());
        }
        if ((asyncWaitStrategyFactoryComponent = this.getTopLevelComponent("AsyncWaitStrategyFactory")).isPresent()) {
            this.writeXmlComponent(xmlWriter, asyncWaitStrategyFactoryComponent.get());
        }
