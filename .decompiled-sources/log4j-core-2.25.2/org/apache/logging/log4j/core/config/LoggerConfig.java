...
implements LocationAware {
    ...
    protected void processLogEvent(LogEvent event, LoggerConfigPredicate predicate) {
    ...
    static {
        try {
            LOG_EVENT_FACTORY = LoaderUtil.newCheckedInstanceOfProperty("Log4jLogEventFactory", LogEventFactory.class, LoggerConfig::createDefaultLogEventFactory);
        }
        catch (Exception ex) {
            LOGGER.error("Unable to create LogEventFactory: {}", (Object)ex.getMessage(), (Object)ex);
            LOG_EVENT_FACTORY = LoggerConfig.createDefaultLogEventFactory();
        }
    }
