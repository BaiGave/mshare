...
implements Builder<PropertiesConfiguration> {
    ...
    public PropertiesConfiguration build() {
        ...
        this.builder.setStatusLevel(Level.toLevel(this.rootProperties.getProperty(STATUS_KEY), Level.ERROR)).setShutdownHook(this.rootProperties.getProperty(SHUTDOWN_HOOK)).setShutdownTimeout(Long.parseLong(this.rootProperties.getProperty(SHUTDOWN_TIMEOUT, "0")), TimeUnit.MILLISECONDS).setDestination(this.rootProperties.getProperty(DEST)).setPackages(this.rootProperties.getProperty(PACKAGES)).setConfigurationName(this.rootProperties.getProperty(CONFIG_NAME)).setMonitorInterval(this.rootProperties.getProperty(MONITOR_INTERVAL, "0")).setAdvertiser(this.rootProperties.getProperty(ADVERTISER_KEY));
        ...
        this.processRemainingProperties((ComponentBuilder)((Object)this.builder), this.rootProperties);
        this.builder.setLoggerContext(this.loggerContext);
        return this.builder.build(false);
    }

    private void processRemainingProperties(ConfigurationBuilder<PropertiesConfiguration> builder, Properties properties) {
        while (properties.size() > 0) {
            ...
            if (index > 0) {
                ...
                ComponentBuilder componentBuilder = PropertiesConfigurationBuilder.createComponent(builder, prefix, componentProperties);
    ...
    private static <B extends ComponentBuilder<B>> ComponentBuilder<B> createComponent(ComponentBuilder<?> parent, String key, Properties properties) {
        return PropertiesConfigurationBuilder.createComponent(parent.getBuilder(), key, properties);
    }

    private static <B extends ComponentBuilder<B>> ComponentBuilder<B> createComponent(ConfigurationBuilder<?> parentBuilder, String key, Properties properties) {
        String name = (String)properties.remove(CONFIG_NAME);
        String type = (String)properties.remove(CONFIG_TYPE);
        if (Strings.isEmpty(type)) {
            throw new ConfigurationException("No type attribute provided for component " + key);
        }
        ComponentBuilder componentBuilder = parentBuilder.newComponent(name, type);
