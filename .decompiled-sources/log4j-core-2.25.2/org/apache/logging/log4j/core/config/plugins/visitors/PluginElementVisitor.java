...
extends AbstractPluginVisitor<PluginElement> {
    ...
    private Node findNamedNode(String name, Iterable<Node> children) {
        for (Node child : children) {
            boolean isAssignableByPluginClass;
            PluginType<?> childType = child.getType();
            boolean elementNameMatch = childType != null && name.equalsIgnoreCase(childType.getElementName());
            boolean bl = isAssignableByPluginClass = childType != null && this.conversionType.isAssignableFrom(childType.getPluginClass());
            if (!elementNameMatch && !isAssignableByPluginClass) continue;
            return child;
        }
        return null;
    }
