...
@ServiceConsumer(value=WatchEventService.class, resolution="optional", cardinality="multiple")
...
extends AbstractLifeCycle {
    ...
    public WatchManager(ConfigurationScheduler scheduler) {
        ...
        this.eventServiceList = ServiceLoaderUtil.safeStream(WatchEventService.class, ServiceLoader.load(WatchEventService.class), logger).collect(Collectors.toList());
    }
    ...
    implements Runnable {
        ...
        public void run() {
            ...
            for (Map.Entry entry : WatchManager.this.watchers.entrySet()) {
                ...
                logger.info("Configuration source at `{}` was modified on `{}`, previous modification was on `{}`", () -> source, () -> Instant.ofEpochMilli(lastModified), () -> Instant.ofEpochMilli(monitor.lastModifiedMillis));
