...
extends FileManager {
    ...
    protected void createParentDir(File file) {
        File parent;
        if (this.directWrite && (parent = file.getParentFile()) != null) {
            parent.mkdirs();
        }
    }
    ...
    static long initialFileTime(File file) {
        ...
        if (Files.exists(path, new LinkOption[0])) {
            try {
                ...
                if (fileTime.compareTo(EPOCH) > 0) {
                    ...
                    return RollingFileManager.alignMillisToSecond(fileTime.toMillis());
                }
                LOGGER.info("Unable to obtain file creation time for {}", (Object)file.getAbsolutePath());
            }
            catch (Exception ex) {
                LOGGER.info("Unable to calculate file creation time for {}: {}", (Object)file.getAbsolutePath(), (Object)ex.getMessage());
            }
        }
        return RollingFileManager.alignMillisToSecond(file.lastModified());
    }

    static long alignMillisToSecond(long millis) {
        return Math.round((double)millis / 1000.0) * 1000L;
    }
    ...
    implements ManagerFactory<RollingFileManager, FactoryData> {
        ...
        @Override
        @SuppressFBWarnings(value={"PATH_TRAVERSAL_IN", "PATH_TRAVERSAL_OUT"}, justification="The destination file should be specified in the configuration file.")
