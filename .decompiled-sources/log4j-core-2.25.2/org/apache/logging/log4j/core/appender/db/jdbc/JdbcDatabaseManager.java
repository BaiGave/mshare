...
extends AbstractDatabaseManager {
    ...
    @SuppressFBWarnings(value={"SQL_INJECTION_JDBC"}, justification="The SQL statement is generated based on the configuration file.")
    ...
    protected void writeInternal(LogEvent event, Serializable serializable) {
        try {
            ...
            for (ColumnConfig column : this.columnConfigs) {
                ...
                if (column.isClob()) {
                    StringReader reader = new StringReader(column.getLayout().toSerializable(event));
                    if (column.isUnicode()) {
