...
public class UrlConnectionFactory {
    ...
    private static final String DEFAULT_ALLOWED_PROTOCOLS = SystemUtils.isGraalVm() ? "file, https, jar, resource" : "file, https, jar";
    ...
    @SuppressFBWarnings(value={"URLCONNECTION_SSRF_FD"}, justification="The URL parameter originates only from secure sources.")
    public static <T extends URLConnection> T createConnection(URL url, long lastModifiedMillis, SslConfiguration sslConfiguration, AuthorizationProvider authorizationProvider) throws IOException {
        ...
        List<String> allowed = Arrays.asList(Strings.splitList(Strings.toRootLowerCase(props.getStringProperty(ALLOWED_PROTOCOLS, DEFAULT_ALLOWED_PROTOCOLS))));
        ...
        if (url.getProtocol().equals(HTTP) || url.getProtocol().equals(HTTPS)) {
            ...
            if (url.getProtocol().equals(HTTPS) && sslConfiguration != null) {
                ((HttpsURLConnection)httpURLConnection).setSSLSocketFactory(sslConfiguration.getSslContext().getSocketFactory());
    ...
    @SuppressFBWarnings(value={"URLCONNECTION_SSRF_FD"}, justification="This method sanitizes the usage of the provided URL.")
