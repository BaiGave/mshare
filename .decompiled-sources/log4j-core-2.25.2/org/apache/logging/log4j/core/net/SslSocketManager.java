...
extends TcpSocketManager {
    ...
    private static String createSslConfigurationId(SslConfiguration sslConfig) {
        return String.valueOf(Stream.of(sslConfig.getKeyStoreConfig(), sslConfig.getTrustStoreConfig()).flatMap(keyStoreConfig -> {
    ...
    private static SSLSocketFactory createSslSocketFactory(SslConfiguration sslConf) {
        SSLSocketFactory socketFactory = sslConf != null ? sslConf.getSslContext().getSocketFactory() : (SSLSocketFactory)SSLSocketFactory.getDefault();
