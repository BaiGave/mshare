...
implements FileWatcher {
    ...
    public ScriptManager(Configuration configuration, WatchManager watchManager, String scriptLanguages) {
        ...
        this.allowedLanguages = Arrays.stream(Strings.splitList(scriptLanguages)).map(Strings::toRootLowerCase).collect(Collectors.toSet());
        if (logger.isDebugEnabled()) {
            ...
            for (ScriptEngineFactory factory : factories) {
                ...
                for (String name : languageNames) {
                    if (!this.allowedLanguages.contains(Strings.toRootLowerCase(name))) continue;
                    if (names.length() > 0) {
                        names.append(", ");
                    }
                    names.append(name);
