...
implements EventTranslator<RingBufferLogEvent> {
    ...
    public void translateTo(RingBufferLogEvent event, long sequence) {
        try {
            ...
            event.setValues(this.asyncLogger, this.loggerName, this.marker, this.fqcn, this.level, this.message, this.thrown, INJECTOR.injectContextData(null, contextData instanceof StringMap ? (StringMap)contextData : null), this.contextStack, this.threadId, this.threadName, this.threadPriority, this.location, this.clock, this.nanoClock);
        }
        finally {
            this.clear();
        }
