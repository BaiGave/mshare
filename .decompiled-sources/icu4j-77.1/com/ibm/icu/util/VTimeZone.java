    ...
    private boolean parse() {
        ...
        if (finalRuleCount == 1) {
            if (rules.size() == 1) {
                ...
                TimeZoneRule newRule = start == finalStart ? new TimeArrayTimeZoneRule(finalRule.getName(), finalRule.getRawOffset(), finalRule.getDSTSavings(), new long[]{finalStart.getTime()}, 2) : new AnnualTimeZoneRule(finalRule.getName(), finalRule.getRawOffset(), finalRule.getDSTSavings(), finalRule.getRule(), finalRule.getStartYear(), Grego.timeToYear(start.getTime()));
