...
extends Calendar {
    ...
    protected void validateField(int field) {
        if (!(field != 2 && field != 23 || HebrewCalendar.isLeapYear(this.handleGetExtendedYear()) || this.internalGetMonth() != 5)) {
    ...
    protected void handleComputeFields(int julianDay) {
        ...
        boolean isLeap = HebrewCalendar.isLeapYear(year);
    ...
    public void setTemporalMonthCode(String temporalMonth) {
        if (temporalMonth.length() == 3 || temporalMonth.length() == 4) {
            for (int m = 0; m < gTemporalMonthCodesForHebrew.length; ++m) {
                if (!temporalMonth.equals(gTemporalMonthCodesForHebrew[m])) continue;
                this.set(2, m);
                return;
            }
        }
    ...
    protected int internalGetMonth() {
        if (this.resolveFields(MONTH_PRECEDENCE) == 23) {
            ...
            return ordinalMonth + (!HebrewCalendar.isLeapYear(year) && ordinalMonth > 5 ? 1 : 0);
        }
