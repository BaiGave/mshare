...
extends NumberFormat {
    ...
    private void init(String description, String[][] localizations) {
        ...
        this.lenientParseRules = RuleBasedNumberFormat.extractSpecial(descBuf, "%%lenient-parse:");
        this.postProcessRules = RuleBasedNumberFormat.extractSpecial(descBuf, "%%post-process:");
        int numRuleSets = 1;
        int p = 0;
    ...
    private StringBuilder stripWhitespace(String description) {
        ...
        while (start < descriptionLength) {
            char ch;
            while (start < descriptionLength && (PatternProps.isWhiteSpace(ch = description.charAt(start)) || ch == ';')) {
                ++start;
            }
            ...
            if (p >= descriptionLength) continue;
            int end = p + 1;
