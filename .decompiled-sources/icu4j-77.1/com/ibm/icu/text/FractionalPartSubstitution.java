...
extends NFSubstitution {
    ...
    public Number doParse(String text, ParsePosition parsePosition, double baseValue, double upperBound, boolean lenientParse, int nonNumericalExecutedRuleMask, int recursionCount) {
        if (!this.byDigits) {
            return super.doParse(text, parsePosition, baseValue, 0.0, lenientParse, nonNumericalExecutedRuleMask, recursionCount);
        }
        ...
        while (workText.length() > 0 && workPos.getIndex() != 0) {
            Number n;
            workPos.setIndex(0);
            int digit = this.ruleSet.parse(workText, workPos, 10.0, nonNumericalExecutedRuleMask, recursionCount).intValue();
