...
public final class UScript {
    ...
    public static final int getScript(int codepoint) {
        if (codepoint >= 0 & codepoint <= 0x10FFFF) {
            int scriptX = UCharacterProperty.INSTANCE.getAdditional(codepoint, 0) & 0xFFF;
            int codeOrIndex = scriptX & 0x3FF;
            if (scriptX < 1024) {
                return codeOrIndex;
            }
            if (scriptX < 2048) {
    ...
    public static final boolean hasScript(int c, int sc) {
        int scriptX = UCharacterProperty.INSTANCE.getAdditional(c, 0) & 0xFFF;
        int codeOrIndex = scriptX & 0x3FF;
        if (scriptX < 1024) {
