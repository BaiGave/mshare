...
implements DataProvider {
    ...
    public CompletableFuture<?> run(CachedOutput cache) {
        ...
        return DataProvider.saveStable(cache, Report.CODEC.encodeStart(JsonOps.INSTANCE, report).getOrThrow(), path);
    }
    ...
    private record Report(Map<ResourceKey<? extends Registry<?>>, Entry> registries, Map<String, CustomPackEntry> others) {
        public static final Codec<Report> CODEC = RecordCodecBuilder.create(i -> i.group(((MapCodec)Codec.unboundedMap(REGISTRY_KEY_CODEC, Entry.CODEC).fieldOf("registries")).forGetter(Report::registries), ((MapCodec)Codec.unboundedMap(Codec.STRING, CustomPackEntry.CODEC).fieldOf("others")).forGetter(Report::others)).apply((Applicative<Report, ?>)i, Report::new));
    }

    private record Entry(boolean elements, boolean tags, boolean stable) {
        public static final MapCodec<Entry> MAP_CODEC = RecordCodecBuilder.mapCodec(i -> i.group(((MapCodec)Codec.BOOL.fieldOf("elements")).forGetter(Entry::elements), ((MapCodec)Codec.BOOL.fieldOf("tags")).forGetter(Entry::tags), ((MapCodec)Codec.BOOL.fieldOf("stable")).forGetter(Entry::stable)).apply((Applicative<Entry, ?>)i, Entry::new));
        public static final Codec<Entry> CODEC = MAP_CODEC.codec();
    }

    private record CustomPackEntry(Format format, Entry entry) {
        public static final Codec<CustomPackEntry> CODEC = RecordCodecBuilder.create(i -> i.group(((MapCodec)Format.CODEC.fieldOf("format")).forGetter(CustomPackEntry::format), Entry.MAP_CODEC.forGetter(CustomPackEntry::entry)).apply((Applicative<CustomPackEntry, ?>)i, CustomPackEntry::new));
    }
