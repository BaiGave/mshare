...
implements DataProvider {
    ...
    private static <E> CompletableFuture<?> dumpValue(Path path, CachedOutput cache, DynamicOps<JsonElement> ops, Encoder<E> codec, E value) {
        Optional<JsonElement> result = codec.encodeStart(ops, value).resultOrPartial(e -> LOGGER.error("Couldn't serialize element {}: {}", (Object)path, e));
        if (result.isPresent()) {
            return DataProvider.saveStable(cache, result.get(), path);
        }
