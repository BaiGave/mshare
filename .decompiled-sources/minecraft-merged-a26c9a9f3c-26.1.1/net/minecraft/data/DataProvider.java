...
public interface DataProvider {
    public static final ToIntFunction<String> FIXED_ORDER_FIELDS = Util.make(new Object2IntOpenHashMap(), m -> {
        m.put("type", 0);
        m.put("parent", 1);
    ...
    public static <T, E> CompletableFuture<?> saveAll(CachedOutput cache, Codec<E> codec, Function<T, Path> pathGetter, Map<T, E> contents) {
        return DataProvider.saveAll(cache, (E e) -> codec.encodeStart(JsonOps.INSTANCE, e).getOrThrow(), pathGetter, contents);
    }
    ...
    public static <T> CompletableFuture<?> saveStable(CachedOutput cache, Codec<T> codec, T value, Path path) {
        return DataProvider.saveStable(cache, JsonOps.INSTANCE, codec, value, path);
    }
    ...
    private static <T> CompletableFuture<?> saveStable(CachedOutput cache, DynamicOps<JsonElement> ops, Codec<T> codec, T value, Path path) {
        JsonElement json = codec.encodeStart(ops, (JsonElement)value).getOrThrow();
        return DataProvider.saveStable(cache, json, path);
    }
    ...
    public static CompletableFuture<?> saveStable(CachedOutput cache, JsonElement root, Path path) {
        return CompletableFuture.runAsync(() -> {
            try {
                ...
                try (JsonWriter jsonWriter = new JsonWriter(new OutputStreamWriter((OutputStream)hashedBytes, StandardCharsets.UTF_8));){
