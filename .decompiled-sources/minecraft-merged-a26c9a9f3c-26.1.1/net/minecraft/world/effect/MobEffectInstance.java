...
implements Comparable<MobEffectInstance> {
    ...
    public static final Codec<MobEffectInstance> CODEC = RecordCodecBuilder.create(i -> i.group(((MapCodec)MobEffect.CODEC.fieldOf("id")).forGetter(MobEffectInstance::getEffect), Details.MAP_CODEC.forGetter(MobEffectInstance::asDetails)).apply((Applicative<MobEffectInstance, ?>)i, MobEffectInstance::new));
    ...
    public int compareTo(MobEffectInstance o) {
        ...
        if (this.getDuration() > 32147 && o.getDuration() > 32147 || this.isAmbient() && o.isAmbient()) {
            return ComparisonChain.start().compare(this.isAmbient(), o.isAmbient()).compare(this.getEffect().value().getColor(), o.getEffect().value().getColor()).result();
        }
