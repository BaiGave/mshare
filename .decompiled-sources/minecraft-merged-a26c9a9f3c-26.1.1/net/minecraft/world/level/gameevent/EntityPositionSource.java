...
implements PositionSource {
    public static final MapCodec<EntityPositionSource> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(((MapCodec)UUIDUtil.CODEC.fieldOf("source_entity")).forGetter(EntityPositionSource::getUuid), ((MapCodec)Codec.FLOAT.fieldOf("y_offset")).orElse(Float.valueOf(0.0f)).forGetter(o -> Float.valueOf(o.yOffset))).apply((Applicative<EntityPositionSource, ?>)i, (uuid, offset) -> new EntityPositionSource(Either.right(Either.left(uuid)), offset.floatValue())));
    public static final StreamCodec<ByteBuf, EntityPositionSource> STREAM_CODEC = StreamCodec.composite(ByteBufCodecs.VAR_INT, EntityPositionSource::getId, ByteBufCodecs.FLOAT, o -> Float.valueOf(o.yOffset), (id, offset) -> new EntityPositionSource(Either.right(Either.right(id)), offset.floatValue()));
    ...
    private void resolveEntity(Level level) {
        this.entityOrUuidOrId.map(Optional::of, uuidOrId -> Optional.ofNullable(uuidOrId.map(uuid -> {
            Entity entity;
