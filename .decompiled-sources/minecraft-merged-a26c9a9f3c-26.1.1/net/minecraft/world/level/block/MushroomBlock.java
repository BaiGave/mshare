...
implements BonemealableBlock {
    public static final MapCodec<MushroomBlock> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(((MapCodec)ResourceKey.codec(Registries.CONFIGURED_FEATURE).fieldOf("feature")).forGetter(b -> b.feature), MushroomBlock.propertiesCodec()).apply((Applicative<MushroomBlock, ?>)i, MushroomBlock::new));
    ...
    public boolean growMushroom(ServerLevel level, BlockPos pos, BlockState state, RandomSource random) {
        Optional<Holder.Reference<ConfiguredFeature<?, ?>>> feature = level.registryAccess().lookupOrThrow(Registries.CONFIGURED_FEATURE).get(this.feature);
        if (feature.isEmpty()) {
            return false;
        }
    ...
    public boolean isValidBonemealTarget(LevelReader level, BlockPos pos, BlockState state) {
        if (level instanceof ServerLevel) {
            ...
            Optional<Holder.Reference<ConfiguredFeature<?, ?>>> featureHolder = serverLevel.registryAccess().lookupOrThrow(Registries.CONFIGURED_FEATURE).get(this.feature);
