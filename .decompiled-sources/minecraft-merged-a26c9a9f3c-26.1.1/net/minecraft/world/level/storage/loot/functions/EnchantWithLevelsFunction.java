...
extends LootItemConditionalFunction {
    public static final MapCodec<EnchantWithLevelsFunction> MAP_CODEC = RecordCodecBuilder.mapCodec(i -> EnchantWithLevelsFunction.commonFields(i).and(i.group(((MapCodec)NumberProviders.CODEC.fieldOf("levels")).forGetter(f -> f.levels), RegistryCodecs.homogeneousList(Registries.ENCHANTMENT).optionalFieldOf("options").forGetter(f -> f.options), Codec.BOOL.optionalFieldOf("include_additional_cost_component", false).forGetter(f -> f.includeAdditionalCostComponent))).apply((Applicative<EnchantWithLevelsFunction, ?>)i, EnchantWithLevelsFunction::new));
