...
extends LootItemConditionalFunction {
    ...
    public static final MapCodec<SetNameFunction> MAP_CODEC = RecordCodecBuilder.mapCodec(i -> SetNameFunction.commonFields(i).and(i.group(ComponentSerialization.CODEC.optionalFieldOf("name").forGetter(f -> f.name), LootContext.EntityTarget.CODEC.optionalFieldOf("entity").forGetter(f -> f.resolutionContext), Target.CODEC.optionalFieldOf("target", Target.CUSTOM_NAME).forGetter(f -> f.target))).apply((Applicative<SetNameFunction, ?>)i, SetNameFunction::new));
