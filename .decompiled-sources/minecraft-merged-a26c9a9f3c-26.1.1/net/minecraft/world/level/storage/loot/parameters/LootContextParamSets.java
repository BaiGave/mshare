...
public class LootContextParamSets {
    ...
    public static final Codec<ContextKeySet> CODEC = Identifier.CODEC.comapFlatMap(location -> Optional.ofNullable((ContextKeySet)REGISTRY.get(location)).map(DataResult::success).orElseGet(() -> DataResult.error(() -> "No parameter set exists with id: '" + String.valueOf(location) + "'")), REGISTRY.inverse()::get);
