...
extends LootItemConditionalFunction {
    private static final Codec<List<EffectEntry>> EFFECTS_LIST = EffectEntry.CODEC.listOf().validate((A entries) -> {
    ...
    public static final MapCodec<SetStewEffectFunction> MAP_CODEC = RecordCodecBuilder.mapCodec(i -> SetStewEffectFunction.commonFields(i).and(EFFECTS_LIST.optionalFieldOf("effects", List.of()).forGetter(f -> f.effects)).apply((Applicative<SetStewEffectFunction, ?>)i, SetStewEffectFunction::new));
    ...
    {
        public static final Codec<EffectEntry> CODEC = RecordCodecBuilder.create(i -> i.group(((MapCodec)MobEffect.CODEC.fieldOf("type")).forGetter(EffectEntry::effect), ((MapCodec)NumberProviders.CODEC.fieldOf("duration")).forGetter(EffectEntry::duration)).apply((Applicative<EffectEntry, ?>)i, EffectEntry::new));
