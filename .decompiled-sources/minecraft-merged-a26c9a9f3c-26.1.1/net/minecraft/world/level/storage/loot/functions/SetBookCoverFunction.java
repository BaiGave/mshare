...
extends LootItemConditionalFunction {
    public static final MapCodec<SetBookCoverFunction> MAP_CODEC = RecordCodecBuilder.mapCodec(i -> SetBookCoverFunction.commonFields(i).and(i.group(Filterable.codec(Codec.string(0, 32)).optionalFieldOf("title").forGetter(f -> f.title), Codec.STRING.optionalFieldOf("author").forGetter(f -> f.author), ExtraCodecs.intRange(0, 3).optionalFieldOf("generation").forGetter(f -> f.generation))).apply((Applicative<SetBookCoverFunction, ?>)i, SetBookCoverFunction::new));
