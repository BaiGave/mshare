...
implements Validatable {
    ...
    public static final Codec<LootTable> DIRECT_CODEC = Codec.lazyInitialized(() -> RecordCodecBuilder.create(i -> i.group(LootContextParamSets.CODEC.lenientOptionalFieldOf("type", DEFAULT_PARAM_SET).forGetter(t -> t.paramSet), Identifier.CODEC.optionalFieldOf("random_sequence").forGetter(t -> t.randomSequence), LootPool.CODEC.listOf().optionalFieldOf("pools", List.of()).forGetter(t -> t.pools), LootItemFunctions.ROOT_CODEC.listOf().optionalFieldOf("functions", List.of()).forGetter(t -> t.functions)).apply((Applicative<LootTable, ?>)i, LootTable::new)));
    ...
    private void shuffleAndSplitItems(ObjectArrayList<ItemStack> result, int availableSlots, RandomSource random) {
        ArrayList<ItemStack> splittableItems = Lists.newArrayList();
        ObjectIterator iterator = result.iterator();
        ...
        result.addAll((Collection<ItemStack>)splittableItems);
        Util.shuffle(result, random);
    }

    private List<Integer> getAvailableSlots(Container container, RandomSource random) {
        ObjectArrayList<Integer> slots = new ObjectArrayList<Integer>();
