...
extends LootItemConditionalFunction {
    ...
    public static final MapCodec<ExplorationMapFunction> MAP_CODEC = RecordCodecBuilder.mapCodec(i -> ExplorationMapFunction.commonFields(i).and(i.group(TagKey.codec(Registries.STRUCTURE).optionalFieldOf("destination", DEFAULT_DESTINATION).forGetter(f -> f.destination), MapDecorationType.CODEC.optionalFieldOf("decoration", DEFAULT_DECORATION).forGetter(f -> f.mapDecoration), Codec.BYTE.optionalFieldOf("zoom", (byte)2).forGetter(f -> f.zoom), Codec.INT.optionalFieldOf("search_radius", 50).forGetter(f -> f.searchRadius), Codec.BOOL.optionalFieldOf("skip_existing_chunks", true).forGetter(f -> f.skipKnownStructures))).apply((Applicative<ExplorationMapFunction, ?>)i, ExplorationMapFunction::new));
