...
implements LootContextUser {
    private static final Codec<IntRange> RECORD_CODEC = RecordCodecBuilder.create(i -> i.group(NumberProviders.CODEC.optionalFieldOf("min").forGetter(r -> Optional.ofNullable(r.min)), NumberProviders.CODEC.optionalFieldOf("max").forGetter(r -> Optional.ofNullable(r.max))).apply((Applicative<IntRange, ?>)i, IntRange::new));
    public static final Codec<IntRange> CODEC = Codec.either(Codec.INT, RECORD_CODEC).xmap(e -> e.map(IntRange::exact, Function.identity()), range -> {
        ...
        if (exact.isPresent()) {
            return Either.left(exact.getAsInt());
        }
