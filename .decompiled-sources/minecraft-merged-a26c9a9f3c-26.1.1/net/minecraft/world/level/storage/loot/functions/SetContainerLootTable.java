...
extends LootItemConditionalFunction {
    public static final MapCodec<SetContainerLootTable> MAP_CODEC = RecordCodecBuilder.mapCodec(i -> SetContainerLootTable.commonFields(i).and(i.group(((MapCodec)LootTable.KEY_CODEC.fieldOf("name")).forGetter(f -> f.name), Codec.LONG.optionalFieldOf("seed", 0L).forGetter(f -> f.seed), ((MapCodec)BuiltInRegistries.BLOCK_ENTITY_TYPE.holderByNameCodec().fieldOf("type")).forGetter(f -> f.type))).apply((Applicative<SetContainerLootTable, ?>)i, SetContainerLootTable::new));
