...
public interface ListOperation {
    ...
    public record ReplaceSection(int offset, Optional<Integer> size) implements ListOperation
    {
        ...
        public static final MapCodec<ReplaceSection> MAP_CODEC = RecordCodecBuilder.mapCodec(i -> i.group(ExtraCodecs.NON_NEGATIVE_INT.optionalFieldOf("offset", 0).forGetter(ReplaceSection::offset), ExtraCodecs.NON_NEGATIVE_INT.optionalFieldOf("size").forGetter(ReplaceSection::size)).apply((Applicative<ReplaceSection, ?>)i, ReplaceSection::new));
    ...
    public record StandAlone<T>(List<T> value, ListOperation operation) {
        public static <T> Codec<StandAlone<T>> codec(Codec<T> valueCodec, int maxSize) {
            return RecordCodecBuilder.create(i -> i.group(((MapCodec)valueCodec.sizeLimitedListOf(maxSize).fieldOf("values")).forGetter(f -> f.value), ListOperation.codec(maxSize).forGetter(f -> f.operation)).apply((Applicative<StandAlone, ?>)i, StandAlone::new));
        }
    ...
    {
        private static final Logger LOGGER = LogUtils.getLogger();
        public static final MapCodec<Insert> MAP_CODEC = RecordCodecBuilder.mapCodec(i -> i.group(ExtraCodecs.NON_NEGATIVE_INT.optionalFieldOf("offset", 0).forGetter(Insert::offset)).apply((Applicative<Insert, ?>)i, Insert::new));
