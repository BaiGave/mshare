...
implements AutoCloseable {
    ...
    private final Long2ObjectMap<CompletableFuture<Optional<PackedChunk<P>>>> pendingLoads = new Long2ObjectOpenHashMap<CompletableFuture<Optional<PackedChunk<P>>>>();
    ...
    private void unpackPendingLoads() {
        ...
        synchronized (object) {
            ObjectIterator<Long2ObjectMap.Entry<CompletableFuture<Optional<PackedChunk<P>>>>> iterator = Long2ObjectMaps.fastIterator(this.pendingLoads);
    ...
    public CompletableFuture<?> prefetch(ChunkPos chunkPos) {
        ...
        synchronized (object) {
            ...
            return this.pendingLoads.computeIfAbsent(chunkKey, k -> this.tryRead(chunkPos));
        }
    }
    ...
    private void unpackChunk(ChunkPos pos, @Nullable PackedChunk<P> packedChunk) {
        if (packedChunk == null) {
            for (int sectionY = this.levelHeightAccessor.getMinSectionY(); sectionY <= this.levelHeightAccessor.getMaxSectionY(); ++sectionY) {
                this.storage.put(SectionStorage.getKey(pos, sectionY), (Optional<R>)Optional.empty());
            }
        } else {
            ...
            for (int sectionY = this.levelHeightAccessor.getMinSectionY(); sectionY <= this.levelHeightAccessor.getMaxSectionY(); ++sectionY) {
                ...
                this.storage.put(key, (Optional<R>)section);
                section.ifPresent(s -> {
    ...
    private <T> Dynamic<T> writeChunk(ChunkPos chunkPos, DynamicOps<T> ops) {
        ...
        return new Dynamic<T>(ops, ops.createMap(ImmutableMap.of(ops.createString(SECTIONS_TAG), ops.createMap(sections), ops.createString("DataVersion"), ops.createInt(SharedConstants.getCurrentVersion().dataVersion().version()))));
    }
    ...
    private record PackedChunk<T>(Int2ObjectMap<T> sectionsByY, boolean versionChanged) {
        public static <T> PackedChunk<T> parse(Codec<T> codec, DynamicOps<Tag> ops, Tag tag, SimpleRegionStorage simpleRegionStorage, LevelHeightAccessor levelHeightAccessor) {
            Dynamic<Tag> originalTag = new Dynamic<Tag>(ops, tag);
            Dynamic<Tag> fixedTag = simpleRegionStorage.upgradeChunkTag(originalTag, 1945);
            boolean versionChanged = originalTag != fixedTag;
            OptionalDynamic<Tag> sections = fixedTag.get(SectionStorage.SECTIONS_TAG);
            ...
            for (int sectionY = levelHeightAccessor.getMinSectionY(); sectionY <= levelHeightAccessor.getMaxSectionY(); ++sectionY) {
                Optional section = sections.get(Integer.toString(sectionY)).result().flatMap(sectionData -> codec.parse(sectionData).resultOrPartial(LOGGER::error));
