...
extends SavedData {
    ...
    public static final Codec<TicketStorage> CODEC = RecordCodecBuilder.create(i -> i.group(TICKET_ENTRY.listOf().optionalFieldOf("tickets", List.of()).forGetter(TicketStorage::packTickets)).apply((Applicative<TicketStorage, ?>)i, TicketStorage::fromPacked));
    ...
    public TicketStorage() {
        this(new Long2ObjectOpenHashMap<List<Ticket>>(4), new Long2ObjectOpenHashMap<List<Ticket>>());
    }

    private static TicketStorage fromPacked(List<Pair<ChunkPos, Ticket>> tickets) {
        Long2ObjectOpenHashMap<List<Ticket>> ticketsToLoad = new Long2ObjectOpenHashMap<List<Ticket>>();
        for (Pair<ChunkPos, Ticket> ticket : tickets) {
            ChunkPos pos = ticket.getFirst();
            List ticketsInChunk = ticketsToLoad.computeIfAbsent(pos.pack(), k -> new ObjectArrayList(4));
            ticketsInChunk.add(ticket.getSecond());
        }
        return new TicketStorage(new Long2ObjectOpenHashMap<List<Ticket>>(4), ticketsToLoad);
    }
    ...
    private List<Pair<ChunkPos, Ticket>> packTickets() {
        ...
        this.forEachTicket((pos, ticket) -> {
            if (ticket.getType().persist()) {
                tickets.add(new Pair<ChunkPos, Ticket>((ChunkPos)pos, (Ticket)ticket));
            }
        });
        return tickets;
    }
        ...
        while (ticketsPerChunkIterator.hasNext()) {
            ...
            while (chunkTicketsIterator.hasNext()) {
                ...
                if (removedTickets != null) {
                    List tickets = removedTickets.computeIfAbsent(chunkPos, k -> new ObjectArrayList(((List)entry.getValue()).size()));
                    tickets.add(ticket);
                }
    ...
    public void replaceTicketLevelOfType(int newLevel, TicketType ticketType) {
        ArrayList<Pair<Ticket, Long>> affectedTickets = new ArrayList<Pair<Ticket, Long>>();
