...
public class Climate {
    ...
    public record ParameterPoint(Parameter temperature, Parameter humidity, Parameter continentalness, Parameter erosion, Parameter depth, Parameter weirdness, long offset) {
        public static final Codec<ParameterPoint> CODEC = RecordCodecBuilder.create(i -> i.group(((MapCodec)Parameter.CODEC.fieldOf("temperature")).forGetter(p -> p.temperature), ((MapCodec)Parameter.CODEC.fieldOf("humidity")).forGetter(p -> p.humidity), ((MapCodec)Parameter.CODEC.fieldOf("continentalness")).forGetter(p -> p.continentalness), ((MapCodec)Parameter.CODEC.fieldOf("erosion")).forGetter(p -> p.erosion), ((MapCodec)Parameter.CODEC.fieldOf("depth")).forGetter(p -> p.depth), ((MapCodec)Parameter.CODEC.fieldOf("weirdness")).forGetter(p -> p.weirdness), ((MapCodec)Codec.floatRange(0.0f, 1.0f).fieldOf("offset")).xmap(Climate::quantizeCoord, Climate::unquantizeCoord).forGetter(p -> p.offset)).apply((Applicative<ParameterPoint, ?>)i, ParameterPoint::new));
    ...
    public static class ParameterList<T> {
        ...
        public static <T> Codec<ParameterList<T>> codec(MapCodec<T> valueCodec) {
            return ExtraCodecs.nonEmptyList(RecordCodecBuilder.create(i -> i.group(((MapCodec)ParameterPoint.CODEC.fieldOf("parameters")).forGetter(Pair::getFirst), valueCodec.forGetter(Pair::getSecond)).apply((Applicative<Pair, ?>)i, Pair::of)).listOf()).xmap(ParameterList::new, ParameterList::values);
        }
    ...
    protected static final class RTree<T> {
        ...
        public static <T> RTree<T> create(List<Pair<ParameterPoint, T>> values) {
            ...
            List leaves = values.stream().map(p -> new Leaf((ParameterPoint)p.getFirst(), p.getSecond())).collect(Collectors.toCollection(ArrayList::new));
        ...
        private static <T> List<SubTree<T>> bucketize(List<? extends Node<T>> nodes) {
            ArrayList<SubTree<T>> buckets = Lists.newArrayList();
            ArrayList<Node<T>> children = Lists.newArrayList();
        ...
        private static <T> List<Parameter> buildParameterSpace(List<? extends Node<T>> children) {
            ...
            int dimensions = 7;
            ArrayList<Parameter> bounds = Lists.newArrayList();
            for (int d = 0; d < 7; ++d) {
                bounds.add(null);
            }
