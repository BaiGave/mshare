...
extends SavedData {
    ...
    public int remove(String id) {
        Collection<Event<Event>> eventsToRemove = this.events.row(id).values();
    ...
    public record Packed<T>(List<Event.Packed<T>> events) {
        public static <T> Codec<Packed<T>> codec(Codec<TimerCallback<T>> callbackCodec) {
            return RecordCodecBuilder.create(i -> i.group(((MapCodec)Event.Packed.codec(callbackCodec).listOf().fieldOf("events")).forGetter(Packed::events)).apply((Applicative<Packed, ?>)i, Packed::new));
        }
    }
    ...
    public record Event<T>(long triggerTime, UnsignedLong sequentialId, String id, TimerCallback<T> callback) {
        ...
        public record Packed<T>(long triggerTime, String id, TimerCallback<T> callback) {
            public static <T> Codec<Packed<T>> codec(Codec<TimerCallback<T>> callbackCodec) {
                return RecordCodecBuilder.create(i -> i.group(((MapCodec)Codec.LONG.fieldOf("trigger_time")).forGetter(Packed::triggerTime), ((MapCodec)Codec.STRING.fieldOf("id")).forGetter(Packed::id), ((MapCodec)callbackCodec.fieldOf("callback")).forGetter(Packed::callback)).apply((Applicative<Packed, ?>)i, Packed::new));
            }
        }
    }
}
