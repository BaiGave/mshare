...
extends BlockStateProvider {
    public static final MapCodec<RuleBasedStateProvider> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(BlockStateProvider.CODEC.optionalFieldOf("fallback").forGetter(provider -> Optional.ofNullable(provider.fallback)), ((MapCodec)Rule.CODEC.listOf().fieldOf("rules")).forGetter(p -> p.rules)).apply((Applicative<RuleBasedStateProvider, ?>)i, RuleBasedStateProvider::new));
    ...
    public record Rule(BlockPredicate ifTrue, BlockStateProvider then) {
        public static final Codec<Rule> CODEC = RecordCodecBuilder.create(i -> i.group(((MapCodec)BlockPredicate.CODEC.fieldOf("if_true")).forGetter(Rule::ifTrue), ((MapCodec)BlockStateProvider.CODEC.fieldOf("then")).forGetter(Rule::then)).apply((Applicative<Rule, ?>)i, Rule::new));
    }
