...
extends NoiseProvider {
    public static final MapCodec<DualNoiseProvider> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(((MapCodec)InclusiveRange.codec(Codec.INT, 1, 64).fieldOf("variety")).forGetter(p -> p.variety), ((MapCodec)NormalNoise.NoiseParameters.DIRECT_CODEC.fieldOf("slow_noise")).forGetter(p -> p.slowNoiseParameters), ((MapCodec)ExtraCodecs.POSITIVE_FLOAT.fieldOf("slow_scale")).forGetter(p -> Float.valueOf(p.slowScale))).and(DualNoiseProvider.noiseProviderCodec(i)).apply(i, DualNoiseProvider::new));
    ...
    public BlockState getState(WorldGenLevel level, RandomSource random, BlockPos pos) {
        ...
        ArrayList<BlockState> possibleStates = Lists.newArrayListWithCapacity(localVariety);
