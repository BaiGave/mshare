...
extends TreeDecorator {
    public static final MapCodec<PlaceOnGroundDecorator> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(((MapCodec)ExtraCodecs.POSITIVE_INT.fieldOf("tries")).orElse(128).forGetter(p -> p.tries), ((MapCodec)ExtraCodecs.NON_NEGATIVE_INT.fieldOf("radius")).orElse(2).forGetter(p -> p.radius), ((MapCodec)ExtraCodecs.NON_NEGATIVE_INT.fieldOf("height")).orElse(1).forGetter(p -> p.height), ((MapCodec)BlockStateProvider.CODEC.fieldOf("block_state_provider")).forGetter(p -> p.blockStateProvider)).apply((Applicative<PlaceOnGroundDecorator, ?>)i, PlaceOnGroundDecorator::new));
