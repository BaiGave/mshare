...
extends TreeDecorator {
    public static final MapCodec<AttachedToLogsDecorator> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(((MapCodec)Codec.floatRange(0.0f, 1.0f).fieldOf("probability")).forGetter(p -> Float.valueOf(p.probability)), ((MapCodec)BlockStateProvider.CODEC.fieldOf("block_provider")).forGetter(p -> p.blockProvider), ((MapCodec)ExtraCodecs.nonEmptyList(Direction.CODEC.listOf()).fieldOf("directions")).forGetter(p -> p.directions)).apply((Applicative<AttachedToLogsDecorator, ?>)i, AttachedToLogsDecorator::new));
