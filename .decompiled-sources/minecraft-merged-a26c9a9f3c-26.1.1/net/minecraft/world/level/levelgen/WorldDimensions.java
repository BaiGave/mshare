...
public record WorldDimensions(Map<ResourceKey<LevelStem>, LevelStem> dimensions) {
    public static final MapCodec<WorldDimensions> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(((MapCodec)Codec.unboundedMap(ResourceKey.codec(Registries.LEVEL_STEM), LevelStem.CODEC).fieldOf("dimensions")).forGetter(WorldDimensions::dimensions)).apply((Applicative<WorldDimensions, ?>)i, i.stable(WorldDimensions::new)));
    ...
    public Complete bake(Registry<LevelStem> baseDimensions) {
        ...
        Sets.SetView<ResourceKey<LevelStem>> knownDimensions = Sets.union(baseDimensions.registryKeySet(), this.dimensions.keySet());
        ArrayList results = new ArrayList();
        WorldDimensions.keysInOrder(knownDimensions).forEach(key -> baseDimensions.getOptional((ResourceKey<LevelStem>)key).or(() -> Optional.ofNullable(this.dimensions.get(key))).ifPresent(levelStem -> results.add(new Entry((ResourceKey<LevelStem>)key, (LevelStem)levelStem))));
