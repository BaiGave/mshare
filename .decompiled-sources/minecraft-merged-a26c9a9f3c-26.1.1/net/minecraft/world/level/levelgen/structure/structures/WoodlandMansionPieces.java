...
public class WoodlandMansionPieces {
    ...
    private static class MansionGrid {
        ...
        private void setupThirdFloor() {
            ArrayList<Tuple<Integer, Integer>> potentialRooms = Lists.newArrayList();
            ...
            ArrayList<Direction> potentialCorridors = Lists.newArrayList();
            for (Direction direction : Direction.Plane.HORIZONTAL) {
        ...
        private void identifyRooms(SimpleGrid fromGrid, SimpleGrid roomGrid) {
            ObjectArrayList<Tuple<Integer, Integer>> roomPos = new ObjectArrayList<Tuple<Integer, Integer>>();
            ...
            int roomId = 10;
            for (Tuple tuple : roomPos) {
                int y;
                int x = (Integer)tuple.getA();
                if (roomGrid.get(x, y = ((Integer)tuple.getB()).intValue()) != 0) continue;
                int x0 = x;
                int x1 = x;
                int y0 = y;
                int y1 = y;
    ...
    private static class MansionPiecePlacer {
        ...
        public void createMansion(BlockPos origin, Rotation rotation, List<WoodlandMansionPiece> pieces, MansionGrid mansion) {
            ...
            for (int floorNum = 0; floorNum < 3; ++floorNum) {
                ...
                ArrayList<Direction> doorDirs = Lists.newArrayList();
                for (int y = 0; y < grid.height; ++y) {
                    for (int x = 0; x < grid.width; ++x) {
                        Direction roomDir;
                        BlockPos pos;
