...
public class StructureTemplatePool {
    ...
    public static final Codec<StructureTemplatePool> DIRECT_CODEC = RecordCodecBuilder.create(i -> i.group(((MapCodec)Codec.lazyInitialized(CODEC_REFERENCE).fieldOf("fallback")).forGetter(StructureTemplatePool::getFallback), ((MapCodec)Codec.mapPair(StructurePoolElement.CODEC.fieldOf("element"), Codec.intRange(1, 150).fieldOf("weight")).codec().listOf().fieldOf("elements")).forGetter(p -> p.rawTemplates)).apply((Applicative<StructureTemplatePool, ?>)i, StructureTemplatePool::new));
