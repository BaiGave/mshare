...
public class StructureCheck {
    ...
    private @Nullable Object2IntMap<Structure> loadStructures(CompoundTag chunkTag) {
        ...
        Object2IntOpenHashMap<Structure> knownStarts = new Object2IntOpenHashMap<Structure>();
        HolderLookup.RegistryLookup structuresRegistry = this.registryAccess.lookupOrThrow(Registries.STRUCTURE);
        startsTag.forEach((arg_0, arg_1) -> StructureCheck.lambda$loadStructures$1((Registry)structuresRegistry, knownStarts, arg_0, arg_1));
        return knownStarts;
    }
    ...
    public void onStructureLoad(ChunkPos pos, Map<Structure, StructureStart> starts) {
        long posKey = pos.pack();
        Object2IntOpenHashMap<Structure> startsToReferences = new Object2IntOpenHashMap<Structure>();
        starts.forEach((structure, structureStart) -> {
            if (structureStart.isValid()) {
                startsToReferences.put((Structure)structure, structureStart.getReferences());
            }
        });
    ...
    public void incrementReference(ChunkPos chunkPos, Structure structure) {
        this.loadedChunks.compute(chunkPos.pack(), (key, counts) -> {
            if (counts == null || counts.isEmpty()) {
                counts = new Object2IntOpenHashMap<Structure>();
            }
