...
extends StructureProcessor {
    public static final MapCodec<BlockIgnoreProcessor> CODEC = ((MapCodec)BlockState.CODEC.xmap(BlockBehaviour.BlockStateBase::getBlock, Block::defaultBlockState).listOf().fieldOf("blocks")).xmap(BlockIgnoreProcessor::new, p -> p.toIgnore);
    ...
    public @Nullable StructureTemplate.StructureBlockInfo processBlock(LevelReader level, BlockPos targetPosition, BlockPos referencePos, StructureTemplate.StructureBlockInfo originalBlockInfo, StructureTemplate.StructureBlockInfo processedBlockInfo, StructurePlaceSettings settings) {
        if (this.toIgnore.contains(processedBlockInfo.state().getBlock())) {
            return null;
        }
