...
extends Structure {
    ...
    public static final MapCodec<JigsawStructure> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(JigsawStructure.settingsCodec(i), ((MapCodec)StructureTemplatePool.CODEC.fieldOf("start_pool")).forGetter(j -> j.startPool), Identifier.CODEC.optionalFieldOf("start_jigsaw_name").forGetter(j -> j.startJigsawName), ((MapCodec)Codec.intRange(0, 20).fieldOf("size")).forGetter(j -> j.maxDepth), ((MapCodec)HeightProvider.CODEC.fieldOf("start_height")).forGetter(j -> j.startHeight), ((MapCodec)Codec.BOOL.fieldOf("use_expansion_hack")).forGetter(j -> j.useExpansionHack), Heightmap.Types.CODEC.optionalFieldOf("project_start_to_heightmap").forGetter(j -> j.projectStartToHeightmap), ((MapCodec)MaxDistance.CODEC.fieldOf("max_distance_from_center")).forGetter(j -> j.maxDistanceFromCenter), Codec.list(PoolAliasBinding.CODEC).optionalFieldOf("pool_aliases", List.of()).forGetter(j -> j.poolAliases), DimensionPadding.CODEC.optionalFieldOf("dimension_padding", DEFAULT_DIMENSION_PADDING).forGetter(j -> j.dimensionPadding), LiquidSettings.CODEC.optionalFieldOf("liquid_settings", DEFAULT_LIQUID_SETTINGS).forGetter(j -> j.liquidSettings)).apply((Applicative<JigsawStructure, ?>)i, JigsawStructure::new)).validate(JigsawStructure::verifyRange);
    ...
    public record MaxDistance(int horizontal, int vertical) {
        ...
        private static final Codec<MaxDistance> FULL_CODEC = RecordCodecBuilder.create(i -> i.group(((MapCodec)HORIZONTAL_VALUE_CODEC.fieldOf("horizontal")).forGetter(MaxDistance::horizontal), ExtraCodecs.intRange(1, DimensionType.Y_SIZE).optionalFieldOf("vertical", DimensionType.Y_SIZE).forGetter(MaxDistance::vertical)).apply((Applicative<MaxDistance, ?>)i, MaxDistance::new));
        public static final Codec<MaxDistance> CODEC = Codec.either(FULL_CODEC, HORIZONTAL_VALUE_CODEC).xmap(either -> either.map(Function.identity(), MaxDistance::new), distance -> distance.horizontal == distance.vertical ? Either.right(distance.horizontal) : Either.left(distance));
