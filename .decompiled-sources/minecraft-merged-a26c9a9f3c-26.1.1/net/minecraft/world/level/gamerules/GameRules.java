...
public class GameRules {
    ...
    public <T> void set(GameRule<T> gameRule, T value, @Nullable MinecraftServer server) {
        if (!this.rules.has(gameRule)) {
            LOGGER.warn("Tried to set invalid game rule '{}' to value '{}'", (Object)gameRule.getIdentifierWithFallback(), (Object)value);
            return;
        }
    ...
    private static GameRule<Integer> registerInteger(String id, GameRuleCategory category, int defaultValue, int min, int max, FeatureFlagSet requiredFeatures) {
        return GameRules.register(id, category, GameRuleType.INT, IntegerArgumentType.integer(min, max), Codec.intRange(min, max), defaultValue, requiredFeatures, GameRuleTypeVisitor::visitInteger, i -> i);
    }
