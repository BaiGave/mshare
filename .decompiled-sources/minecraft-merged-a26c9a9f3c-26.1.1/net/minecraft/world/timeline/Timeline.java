...
public class Timeline {
    ...
    public static final Codec<Timeline> DIRECT_CODEC = RecordCodecBuilder.create(i -> i.group(((MapCodec)WorldClock.CODEC.fieldOf("clock")).forGetter(t -> t.clock), ExtraCodecs.POSITIVE_INT.optionalFieldOf("period_ticks").forGetter(t -> t.periodTicks), TRACKS_CODEC.optionalFieldOf("tracks", Map.of()).forGetter(t -> t.tracks), Codec.unboundedMap(ClockTimeMarker.KEY_CODEC, TimeMarkerInfo.CODEC).optionalFieldOf("time_markers", Map.of()).forGetter(t -> t.timeMarkers)).apply((Applicative<Timeline, ?>)i, Timeline::new)).validate(Timeline::validateInternal);
    ...
    private record TimeMarkerInfo(int ticks, boolean showInCommands) {
        private static final Codec<TimeMarkerInfo> FULL_CODEC = RecordCodecBuilder.create(i -> i.group(((MapCodec)ExtraCodecs.NON_NEGATIVE_INT.fieldOf("ticks")).forGetter(TimeMarkerInfo::ticks), Codec.BOOL.optionalFieldOf("show_in_commands", false).forGetter(TimeMarkerInfo::showInCommands)).apply((Applicative<TimeMarkerInfo, ?>)i, TimeMarkerInfo::new));
        public static final Codec<TimeMarkerInfo> CODEC = Codec.either(ExtraCodecs.NON_NEGATIVE_INT, FULL_CODEC).xmap(either -> either.map(t -> new TimeMarkerInfo((int)t, false), t -> t), timeMarker -> timeMarker.showInCommands ? Either.right(timeMarker) : Either.left(timeMarker.ticks));
    }
