...
public final class EnvironmentAttributeMap {
    ...
    public record Entry<Value, Argument>(Argument argument, AttributeModifier<Value, Argument> modifier) {
        private static <Value> Codec<Entry<Value, ?>> createCodec(EnvironmentAttribute<Value> attribute) {
            Codec<Entry> fullCodec = attribute.type().modifierCodec().dispatch("modifier", Entry::modifier, Util.memoize(modifier -> Entry.createFullCodec(attribute, modifier)));
            return Codec.either(attribute.valueCodec(), fullCodec).xmap(either -> either.map(value -> new Entry(value, AttributeModifier.override()), e -> e), entry -> {
        ...
        private static <Value, Argument> MapCodec<Entry<Value, Argument>> createFullCodec(EnvironmentAttribute<Value> attribute, AttributeModifier<Value, Argument> modifier) {
            return RecordCodecBuilder.mapCodec(i -> i.group(((MapCodec)modifier.argumentCodec(attribute).fieldOf("argument")).forGetter(Entry::argument)).apply((Applicative<Entry, ?>)i, value -> new Entry(value, modifier)));
        }
