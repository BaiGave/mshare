...
public record ModelAndTexture<T>(T model, ClientAsset.ResourceTexture asset) {
    ...
    public static <T> MapCodec<ModelAndTexture<T>> codec(Codec<T> modelCodec, T defaultModel) {
        return RecordCodecBuilder.mapCodec(i -> i.group(modelCodec.optionalFieldOf("model", defaultModel).forGetter(ModelAndTexture::model), ClientAsset.ResourceTexture.DEFAULT_FIELD_CODEC.forGetter(ModelAndTexture::asset)).apply((Applicative<ModelAndTexture, ?>)i, ModelAndTexture::new));
    }
