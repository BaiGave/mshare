...
public class CreakingAi {
    ...
    static ActivityData<Creaking> initIdleActivity() {
        ...
        }, (level, mob) -> mob.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_ATTACKABLE_PLAYER)), SetEntityLookTargetSometimes.create(8.0f, UniformInt.of(30, 60)), new RunOne(ImmutableList.of(Pair.of(RandomStroll.stroll(0.3f), 2), Pair.of(SetWalkTargetFromLookTarget.create(0.3f, 3), 2), Pair.of(new DoNothing(30, 60), 1)))));
    }

    static ActivityData<Creaking> initFightActivity(Creaking body) {
        return ActivityData.create(Activity.FIGHT, 10, ImmutableList.of(SetWalkTargetFromAttackTargetIfTargetOutOfReach.create(1.0f), MeleeAttack.create(Creaking::canMove, 40), StopAttackingIfTargetInvalid.create((level, target) -> !CreakingAi.isAttackTargetStillReachable(body, target))), ImmutableSet.of(Pair.of(MemoryModuleType.ATTACK_TARGET, MemoryStatus.VALUE_PRESENT)));
    }
