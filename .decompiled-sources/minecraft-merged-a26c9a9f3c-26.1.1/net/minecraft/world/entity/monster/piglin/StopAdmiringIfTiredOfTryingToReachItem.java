...
public class StopAdmiringIfTiredOfTryingToReachItem {
    public static BehaviorControl<LivingEntity> create(int maxTimeToReachItem, int disableTime) {
        return BehaviorBuilder.create(i -> i.group(i.present(MemoryModuleType.ADMIRING_ITEM), i.present(MemoryModuleType.NEAREST_VISIBLE_WANTED_ITEM), i.registered(MemoryModuleType.TIME_TRYING_TO_REACH_ADMIRE_ITEM), i.registered(MemoryModuleType.DISABLE_WALK_TO_ADMIRE_ITEM)).apply(i, (admiring, nearestVisible, time, disableWalk) -> (level, body, timestamp) -> {
