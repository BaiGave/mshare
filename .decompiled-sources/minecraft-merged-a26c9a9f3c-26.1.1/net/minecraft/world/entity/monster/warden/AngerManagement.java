...
public class AngerManagement {
    ...
    private static final Codec<Pair<UUID, Integer>> SUSPECT_ANGER_PAIR = RecordCodecBuilder.create(i -> i.group(((MapCodec)UUIDUtil.CODEC.fieldOf("uuid")).forGetter(Pair::getFirst), ((MapCodec)ExtraCodecs.NON_NEGATIVE_INT.fieldOf("anger")).forGetter(Pair::getSecond)).apply((Applicative<Pair, ?>)i, Pair::of));
    ...
    public static Codec<AngerManagement> codec(Predicate<Entity> filter) {
        return RecordCodecBuilder.create(i -> i.group(((MapCodec)SUSPECT_ANGER_PAIR.listOf().fieldOf("suspects")).orElse(Collections.emptyList()).forGetter(AngerManagement::createUuidAngerPairs)).apply((Applicative<AngerManagement, ?>)i, list -> new AngerManagement(filter, (List<Pair<UUID, Integer>>)list)));
    }
    ...
    public AngerManagement(Predicate<Entity> filter, List<Pair<UUID, Integer>> angerByUuid) {
        ...
        this.angerByUuid = new Object2IntOpenHashMap<UUID>(angerByUuid.size());
    ...
    private List<Pair<UUID, Integer>> createUuidAngerPairs() {
        return Streams.concat(this.suspects.stream().map(e -> Pair.of(e.getUUID(), this.angerBySuspect.getInt(e))), this.angerByUuid.object2IntEntrySet().stream().map(e -> Pair.of((UUID)e.getKey(), e.getIntValue()))).collect(Collectors.toList());
    }
    ...
    public void tick(ServerLevel level, Predicate<Entity> validEntity) {
        ...
        Iterator serializedIterator = this.angerByUuid.object2IntEntrySet().iterator();
        ...
        Iterator iterator = this.angerBySuspect.object2IntEntrySet().iterator();
    ...
    private void convertFromUuids(ServerLevel level) {
        Iterator iterator = this.angerByUuid.object2IntEntrySet().iterator();
        while (iterator.hasNext()) {
            Object2IntMap.Entry entry = (Object2IntMap.Entry)iterator.next();
            int anger = entry.getIntValue();
    ...
    public int increaseAnger(Entity entity, int increment) {
        ...
        if (newSuspect) {
            ...
            this.angerBySuspect.put(entity, currentAnger += serializedAnger);
            this.suspects.add(entity);
        }
        this.sortAndUpdateHighestAnger();
        return currentAnger;
