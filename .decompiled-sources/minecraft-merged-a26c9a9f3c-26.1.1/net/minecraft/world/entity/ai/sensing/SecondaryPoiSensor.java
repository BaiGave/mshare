...
extends Sensor<Villager> {
    ...
    protected void doTick(ServerLevel level, Villager body) {
        ...
        ArrayList<GlobalPos> jobSites = Lists.newArrayList();
        int horizontalSearch = 4;
        for (int x = -4; x <= 4; ++x) {
            for (int y = -2; y <= 2; ++y) {
                for (int z = -4; z <= 4; ++z) {
                    BlockPos testPos = center.offset(x, y, z);
                    if (!body.getVillagerData().profession().value().secondaryPoi().contains(level.getBlockState(testPos).getBlock())) continue;
