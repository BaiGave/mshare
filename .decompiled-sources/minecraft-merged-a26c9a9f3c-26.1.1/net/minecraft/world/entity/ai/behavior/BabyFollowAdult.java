...
public class BabyFollowAdult {
    ...
    public static OneShot<LivingEntity> create(UniformInt followRange, Function<LivingEntity, Float> speedModifier, MemoryModuleType<? extends LivingEntity> nearestVisibleType, boolean targetEye) {
        return BehaviorBuilder.create(i -> i.group(i.present(nearestVisibleType), i.registered(MemoryModuleType.LOOK_TARGET), i.absent(MemoryModuleType.WALK_TARGET)).apply(i, (nearestAdult, lookTarget, walkTarget) -> (level, body, timestamp) -> {
            if (!body.isBaby()) {
                return false;
            }
