...
public class AcquirePoi {
    ...
    public static BehaviorControl<PathfinderMob> create(Predicate<Holder<PoiType>> poiType, MemoryModuleType<GlobalPos> memoryToValidate, MemoryModuleType<GlobalPos> memoryToAcquire, boolean onlyIfAdult, Optional<Byte> onPoiAcquisitionEvent, BiPredicate<ServerLevel, BlockPos> validPoi) {
        ...
        OneShot<PathfinderMob> acquirePoi = BehaviorBuilder.create(i -> i.group(i.absent(memoryToAcquire)).apply(i, toAcquire -> (level, body, timestamp) -> {
            ...
            if (path != null && path.canReach()) {
                ...
                for (Pair<Holder<PoiType>, BlockPos> p2 : poiPositions) {
                    batchCache.computeIfAbsent(p2.getSecond().asLong(), key -> new JitteredLinearRetry(random, timestamp));
                }
            }
            return true;
        }));
        ...
        return BehaviorBuilder.create(i -> i.group(i.absent(memoryToValidate)).apply(i, toValidate -> acquirePoi));
    }
