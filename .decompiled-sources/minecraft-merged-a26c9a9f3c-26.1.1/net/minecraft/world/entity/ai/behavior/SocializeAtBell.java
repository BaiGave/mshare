...
public class SocializeAtBell {
    ...
    public static OneShot<LivingEntity> create() {
        return BehaviorBuilder.create(i -> i.group(i.registered(MemoryModuleType.WALK_TARGET), i.registered(MemoryModuleType.LOOK_TARGET), i.present(MemoryModuleType.MEETING_POINT), i.present(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES), i.absent(MemoryModuleType.INTERACTION_TARGET)).apply(i, (walkTarget, lookTarget, meetingPoint, nearestEntities, interactionTarget) -> (level, body, timestamp) -> {
