...
public class PlayTagWithOtherKids {
    ...
    public static BehaviorControl<PathfinderMob> create() {
        return BehaviorBuilder.create(i -> i.group(i.present(MemoryModuleType.VISIBLE_VILLAGER_BABIES), i.absent(MemoryModuleType.WALK_TARGET), i.registered(MemoryModuleType.LOOK_TARGET), i.registered(MemoryModuleType.INTERACTION_TARGET)).apply(i, (babies, walkTarget, lookTarget, interactionTarget) -> (level, me, timestamp) -> {
    ...
    private static Map<LivingEntity, Integer> checkHowManyChasersEachFriendHas(List<LivingEntity> friendsNearby) {
        HashMap<LivingEntity, Integer> chasedKids = Maps.newHashMap();
