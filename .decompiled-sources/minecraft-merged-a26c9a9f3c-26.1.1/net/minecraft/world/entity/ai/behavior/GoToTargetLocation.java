...
public class GoToTargetLocation {
    ...
    public static <E extends Mob> OneShot<E> create(MemoryModuleType<BlockPos> locationMemory, int closeEnoughDist, float speedModifier) {
        return BehaviorBuilder.create(i -> i.group(i.present(locationMemory), i.absent(MemoryModuleType.ATTACK_TARGET), i.absent(MemoryModuleType.WALK_TARGET), i.registered(MemoryModuleType.LOOK_TARGET)).apply(i, (location, attackTarget, walkTarget, lookTarget) -> (level, body, timestamp) -> {
