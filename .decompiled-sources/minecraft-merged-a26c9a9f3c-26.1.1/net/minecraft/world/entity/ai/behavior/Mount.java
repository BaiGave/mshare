...
public class Mount {
    ...
    public static BehaviorControl<LivingEntity> create(float speedModifier) {
        return BehaviorBuilder.create(i -> i.group(i.registered(MemoryModuleType.LOOK_TARGET), i.absent(MemoryModuleType.WALK_TARGET), i.present(MemoryModuleType.RIDE_TARGET)).apply(i, (lookTarget, walkTarget, rideTarget) -> (level, body, timestamp) -> {
            if (body.isPassenger()) {
                return false;
            }
