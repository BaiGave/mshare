...
public class PoiSection {
    ...
    public void refresh(Consumer<BiConsumer<BlockPos, Holder<PoiType>>> updater) {
        if (!this.isValid) {
            Short2ObjectOpenHashMap<PoiRecord> oldRecords = new Short2ObjectOpenHashMap<PoiRecord>(this.records);
            this.clear();
            updater.accept((blockPos, poiType) -> {
                short key = SectionPos.sectionRelativePos(blockPos);
                PoiRecord newRecord = oldRecords.computeIfAbsent(key, k -> new PoiRecord((BlockPos)blockPos, (Holder<PoiType>)poiType, this.setDirty));
                this.add(newRecord);
            });
            this.isValid = true;
            this.setDirty.run();
    ...
    public record Packed(boolean isValid, List<PoiRecord.Packed> records) {
        public static final Codec<Packed> CODEC = RecordCodecBuilder.create(i -> i.group(Codec.BOOL.lenientOptionalFieldOf("Valid", false).forGetter(Packed::isValid), ((MapCodec)PoiRecord.Packed.CODEC.listOf().fieldOf("Records")).forGetter(Packed::records)).apply((Applicative<Packed, ?>)i, Packed::new));
