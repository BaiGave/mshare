...
public record VillagerData(Holder<VillagerType> type, Holder<VillagerProfession> profession, int level) {
    ...
    public static final Codec<VillagerData> CODEC = RecordCodecBuilder.create(i -> i.group(((MapCodec)BuiltInRegistries.VILLAGER_TYPE.holderByNameCodec().fieldOf("type")).orElseGet(() -> BuiltInRegistries.VILLAGER_TYPE.getOrThrow(VillagerType.PLAINS)).forGetter(d -> d.type), ((MapCodec)BuiltInRegistries.VILLAGER_PROFESSION.holderByNameCodec().fieldOf("profession")).orElseGet(() -> BuiltInRegistries.VILLAGER_PROFESSION.getOrThrow(VillagerProfession.NONE)).forGetter(d -> d.profession), ((MapCodec)Codec.INT.fieldOf("level")).orElse(1).forGetter(d -> d.level)).apply((Applicative<VillagerData, ?>)i, VillagerData::new));
