...
public class ArmadilloAi {
    ...
    private static final OneShot<Armadillo> ARMADILLO_ROLLING_OUT = BehaviorBuilder.create(i -> i.group(i.absent(MemoryModuleType.DANGER_DETECTED_RECENTLY)).apply(i, location -> (level, body, timestamp) -> {
        if (body.isScared()) {
            body.rollOut();
            return true;
        }
    ...
    private static ActivityData<Armadillo> initIdleActivity() {
        return ActivityData.create(Activity.IDLE, ImmutableList.of(Pair.of(0, SetEntityLookTargetSometimes.create(EntityType.PLAYER, 6.0f, UniformInt.of(30, 60))), Pair.of(1, new AnimalMakeLove(EntityType.ARMADILLO, 1.0f, 1)), Pair.of(2, new RunOne(ImmutableList.of(Pair.of(new FollowTemptation(armadillo -> Float.valueOf(1.25f), armadillo -> armadillo.isBaby() ? 1.0 : 2.0), 1), Pair.of(BabyFollowAdult.create(ADULT_FOLLOW_RANGE, 1.25f), 1)))), Pair.of(3, new RandomLookAround(UniformInt.of(150, 250), 30.0f, 0.0f, 0.0f)), Pair.of(4, new RunOne(ImmutableMap.of(MemoryModuleType.WALK_TARGET, MemoryStatus.VALUE_ABSENT), ImmutableList.of(Pair.of(RandomStroll.stroll(1.0f), 1), Pair.of(SetWalkTargetFromLookTarget.create(1.0f, 3), 1), Pair.of(new DoNothing(30, 60), 1))))));
    }
