...
FabricRecipeManager {
    ...
    private static List<ServerDisplayInfo> unpackRecipeInfo(Iterable<RecipeHolder<?>> recipes, FeatureFlagSet enabledFeatures) {
        ...
        Object2IntOpenHashMap<String> recipeGroups = new Object2IntOpenHashMap<String>();
        for (RecipeHolder<?> recipeHolder : recipes) {
            Object recipe = recipeHolder.value();
            OptionalInt groupId = recipe.group().isEmpty() ? OptionalInt.empty() : OptionalInt.of(recipeGroups.computeIfAbsent(recipe.group(), id -> recipeGroups.size()));
            ...
            for (RecipeDisplay recipeDisplay : recipe.display()) {
                ...
                RecipeDisplayId id2 = new RecipeDisplayId(nextDisplayId);
                RecipeDisplayEntry entry = new RecipeDisplayEntry(id2, recipeDisplay, groupId, recipe.recipeBookCategory(), placementCheck);
