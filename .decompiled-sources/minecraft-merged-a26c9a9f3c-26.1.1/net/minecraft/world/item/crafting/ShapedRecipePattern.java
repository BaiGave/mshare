...
public final class ShapedRecipePattern {
    ...
    public record Data(Map<Character, Ingredient> key, List<String> pattern) {
        ...
        private static final Codec<Character> SYMBOL_CODEC = Codec.STRING.comapFlatMap(symbol -> {
            ...
            return DataResult.success(Character.valueOf(symbol.charAt(0)));
        }, String::valueOf);
        public static final MapCodec<Data> MAP_CODEC = RecordCodecBuilder.mapCodec(i -> i.group(((MapCodec)ExtraCodecs.strictUnboundedMap(SYMBOL_CODEC, Ingredient.CODEC).fieldOf("key")).forGetter(d -> d.key), ((MapCodec)PATTERN_CODEC.fieldOf("pattern")).forGetter(d -> d.pattern)).apply((Applicative<Data, ?>)i, Data::new));
    }
}
