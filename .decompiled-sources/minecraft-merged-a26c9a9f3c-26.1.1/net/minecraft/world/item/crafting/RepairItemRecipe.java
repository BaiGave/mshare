...
extends CustomRecipe {
    ...
    private static @Nullable Pair<ItemStack, ItemStack> getItemsToCombine(CraftingInput input) {
        ...
        for (int i = 0; i < input.size(); ++i) {
            ...
            return RepairItemRecipe.canCombine(first, itemStack) ? Pair.of(first, itemStack) : null;
        }
        return null;
    }
    ...
    public ItemStack assemble(CraftingInput input) {
        ...
        EnchantmentHelper.updateEnchantments(itemStack, newEnchantments -> {
            for (Holder holder : Sets.union(firstEnchants.keySet(), secondEnchants.keySet())) {
                if (!holder.is(EnchantmentTags.CURSE)) continue;
                int enchantLevel = Math.max(firstEnchants.getLevel(holder), secondEnchants.getLevel(holder));
                newEnchantments.set(holder, enchantLevel);
            }
        });
        return itemStack;
    }
