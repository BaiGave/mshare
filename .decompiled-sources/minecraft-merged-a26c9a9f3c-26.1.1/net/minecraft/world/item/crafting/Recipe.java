...
public interface Recipe<T extends RecipeInput> {
    ...
    public static interface BookInfo<CategoryType> {
        ...
        public static <CategoryType, SelfType extends BookInfo<CategoryType>> MapCodec<SelfType> mapCodec(Codec<CategoryType> categoryCodec, CategoryType defaultCategory, Constructor<CategoryType, SelfType> constructor) {
            return RecordCodecBuilder.mapCodec(i -> i.group(((MapCodec)categoryCodec.fieldOf("category")).orElse(defaultCategory).forGetter(BookInfo::category), Codec.STRING.optionalFieldOf("group", "").forGetter(BookInfo::group)).apply((Applicative<BookInfo, ?>)i, constructor));
        }
    ...
    public record CommonInfo(boolean showNotification) {
        public static final MapCodec<CommonInfo> MAP_CODEC = RecordCodecBuilder.mapCodec(i -> i.group(Codec.BOOL.optionalFieldOf("show_notification", true).forGetter(CommonInfo::showNotification)).apply((Applicative<CommonInfo, ?>)i, CommonInfo::new));
