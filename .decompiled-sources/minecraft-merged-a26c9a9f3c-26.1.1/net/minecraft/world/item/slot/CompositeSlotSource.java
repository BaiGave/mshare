...
implements SlotSource {
    ...
    protected static <T extends CompositeSlotSource> MapCodec<T> createCodec(Function<List<SlotSource>, T> factory) {
        return RecordCodecBuilder.mapCodec(i -> i.group(((MapCodec)SlotSources.CODEC.listOf().fieldOf("terms")).forGetter(t -> t.terms)).apply((Applicative<CompositeSlotSource, ?>)i, factory));
    }
