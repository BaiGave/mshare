...
public record Enchantment(Component description, EnchantmentDefinition definition, HolderSet<Enchantment> exclusiveSet, DataComponentMap effects) {
    ...
    public static final Codec<Enchantment> DIRECT_CODEC = RecordCodecBuilder.create(i -> i.group(((MapCodec)ComponentSerialization.CODEC.fieldOf("description")).forGetter(Enchantment::description), EnchantmentDefinition.CODEC.forGetter(Enchantment::definition), RegistryCodecs.homogeneousList(Registries.ENCHANTMENT).optionalFieldOf("exclusive_set", HolderSet.empty()).forGetter(Enchantment::exclusiveSet), EnchantmentEffectComponents.CODEC.optionalFieldOf("effects", DataComponentMap.EMPTY).forGetter(Enchantment::effects)).apply((Applicative<Enchantment, ?>)i, Enchantment::new));
    ...
    public Map<EquipmentSlot, ItemStack> getSlotItems(LivingEntity entity) {
        EnumMap<EquipmentSlot, ItemStack> itemStacks = Maps.newEnumMap(EquipmentSlot.class);
    ...
    public final void modifyItemFilteredCount(DataComponentType<List<ConditionalEffect<EnchantmentValueEffect>>> effectType, ServerLevel serverLevel, int enchantmentLevel, ItemInstance itemStack, MutableFloat value) {
    ...
    public final void modifyEntityFilteredValue(DataComponentType<List<ConditionalEffect<EnchantmentValueEffect>>> effectType, ServerLevel serverLevel, int enchantmentLevel, ItemStack itemStack, Entity entity, MutableFloat value) {
    ...
    public final void modifyDamageFilteredValue(DataComponentType<List<ConditionalEffect<EnchantmentValueEffect>>> effectType, ServerLevel serverLevel, int enchantmentLevel, ItemStack itemStack, Entity victim, DamageSource damageSource, MutableFloat value) {
    ...
    public static <T> void applyEffects(List<ConditionalEffect<T>> effects, LootContext filterData, GenericAction<T> action) {
    ...
    public static <T> void applyEffects(List<ConditionalEffect<T>> effects, LootContext filterData, MutableFloat value, FloatAction<T> action) {
    ...
    public void runLocationChangedEffects(ServerLevel serverLevel, int enchantmentLevel, EnchantedItemInUse item, LivingEntity entity) {
        ...
        for (ConditionalEffect filteredEffect : this.getEffects(EnchantmentEffectComponents.LOCATION_CHANGED)) {
            ...
            if (filteredEffect.matches(Enchantment.locationContext(serverLevel, enchantmentLevel, entity, wasActive))) {
                if (!wasActive) {
                    if (activeEffects == null) {
                        activeEffects = new ObjectArraySet<EnchantmentLocationBasedEffect>();
    ...
    public record EnchantmentDefinition(HolderSet<Item> supportedItems, Optional<HolderSet<Item>> primaryItems, int weight, int maxLevel, Cost minCost, Cost maxCost, int anvilCost, List<EquipmentSlotGroup> slots) {
        public static final MapCodec<EnchantmentDefinition> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(((MapCodec)RegistryCodecs.homogeneousList(Registries.ITEM).fieldOf("supported_items")).forGetter(EnchantmentDefinition::supportedItems), RegistryCodecs.homogeneousList(Registries.ITEM).optionalFieldOf("primary_items").forGetter(EnchantmentDefinition::primaryItems), ((MapCodec)ExtraCodecs.intRange(1, 1024).fieldOf("weight")).forGetter(EnchantmentDefinition::weight), ((MapCodec)ExtraCodecs.intRange(1, 255).fieldOf("max_level")).forGetter(EnchantmentDefinition::maxLevel), ((MapCodec)Cost.CODEC.fieldOf("min_cost")).forGetter(EnchantmentDefinition::minCost), ((MapCodec)Cost.CODEC.fieldOf("max_cost")).forGetter(EnchantmentDefinition::maxCost), ((MapCodec)ExtraCodecs.NON_NEGATIVE_INT.fieldOf("anvil_cost")).forGetter(EnchantmentDefinition::anvilCost), ((MapCodec)EquipmentSlotGroup.CODEC.listOf().fieldOf("slots")).forGetter(EnchantmentDefinition::slots)).apply((Applicative<EnchantmentDefinition, ?>)i, EnchantmentDefinition::new));
    }

    public record Cost(int base, int perLevelAboveFirst) {
        public static final Codec<Cost> CODEC = RecordCodecBuilder.create(i -> i.group(((MapCodec)Codec.INT.fieldOf("base")).forGetter(Cost::base), ((MapCodec)Codec.INT.fieldOf("per_level_above_first")).forGetter(Cost::perLevelAboveFirst)).apply((Applicative<Cost, ?>)i, Cost::new));
    ...
    @FunctionalInterface
    public static interface FloatAction<T> {
