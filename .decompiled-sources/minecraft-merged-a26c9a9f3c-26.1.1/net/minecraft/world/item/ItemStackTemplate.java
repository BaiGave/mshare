...
public record ItemStackTemplate(Holder<Item> item, int count, DataComponentPatch components) implements ItemInstance
{
    ...
    public static final MapCodec<ItemStackTemplate> MAP_CODEC = RecordCodecBuilder.mapCodec(i -> i.group(((MapCodec)Item.CODEC.fieldOf("id")).forGetter(ItemStackTemplate::item), ExtraCodecs.intRange(1, 99).optionalFieldOf("count", 1).forGetter(ItemStackTemplate::count), DataComponentPatch.CODEC.optionalFieldOf("components", DataComponentPatch.EMPTY).forGetter(ItemStackTemplate::components)).apply((Applicative<ItemStackTemplate, ?>)i, ItemStackTemplate::new));
    ...
    private ItemStack validate(ItemStack result) {
        ...
        if (error.isPresent()) {
            LOGGER.warn("Can't create item stack with properties {}, error: {}", (Object)this, (Object)error.get().message());
