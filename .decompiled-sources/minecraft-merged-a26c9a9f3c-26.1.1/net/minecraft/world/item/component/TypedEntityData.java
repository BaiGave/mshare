    ...
    public static <T> Codec<TypedEntityData<T>> codec(final Codec<T> typeCodec) {
        return new Codec<TypedEntityData<T>>(){
            ...
            public <V> DataResult<Pair<TypedEntityData<T>, V>> decode(DynamicOps<V> ops, V input) {
                return CustomData.COMPOUND_TAG_CODEC.decode(ops, input).flatMap((? super R pair) -> {
                    ...
                    return typeCodec.parse(1.asNbtOps(ops), typeTag).map((? super R type) -> Pair.of(new TypedEntityData<Object>(type, tagWithoutType), pair.getSecond()));
                });
            }

            @Override
            public <V> DataResult<V> encode(TypedEntityData<T> input, DynamicOps<V> ops, V prefix) {
                return typeCodec.encodeStart(1.asNbtOps(ops), input.type).flatMap((? super R typeTag) -> {
