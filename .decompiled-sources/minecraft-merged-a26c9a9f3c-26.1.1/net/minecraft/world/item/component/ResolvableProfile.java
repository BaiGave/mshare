...
implements TooltipProvider {
    private static final Codec<ResolvableProfile> FULL_CODEC = RecordCodecBuilder.create(i -> i.group(Codec.mapEither(ExtraCodecs.STORED_GAME_PROFILE, Partial.MAP_CODEC).forGetter(ResolvableProfile::unpack), PlayerSkin.Patch.MAP_CODEC.forGetter(ResolvableProfile::skinPatch)).apply((Applicative<ResolvableProfile, ?>)i, ResolvableProfile::create));
    ...
    private static ResolvableProfile create(Either<GameProfile, Partial> value, PlayerSkin.Patch patch) {
        return value.map(full -> new Static(Either.left(full), patch), partial -> {
            if (!partial.properties.isEmpty() || partial.id.isPresent() == partial.name.isPresent()) {
                return new Static(Either.right(partial), patch);
            }
            return partial.name.map(s -> new Dynamic(Either.left(s), patch)).orElseGet(() -> new Dynamic(Either.right(partial.id.get()), patch));
        });
    }
    ...
    protected record Partial(Optional<String> name, Optional<UUID> id, PropertyMap properties) {
        ...
        private static final MapCodec<Partial> MAP_CODEC = RecordCodecBuilder.mapCodec(i -> i.group(ExtraCodecs.PLAYER_NAME.optionalFieldOf("name").forGetter(Partial::name), UUIDUtil.CODEC.optionalFieldOf("id").forGetter(Partial::id), ExtraCodecs.PROPERTY_MAP.optionalFieldOf("properties", PropertyMap.EMPTY).forGetter(Partial::properties)).apply((Applicative<Partial, ?>)i, Partial::new));
