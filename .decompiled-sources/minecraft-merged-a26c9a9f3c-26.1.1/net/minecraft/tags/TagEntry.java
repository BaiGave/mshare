...
public class TagEntry {
    private static final Codec<TagEntry> FULL_CODEC = RecordCodecBuilder.create(i -> i.group(((MapCodec)ExtraCodecs.TAG_OR_ELEMENT_ID.fieldOf("id")).forGetter(TagEntry::elementOrTag), Codec.BOOL.optionalFieldOf("required", true).forGetter(e -> e.required)).apply((Applicative<TagEntry, ?>)i, TagEntry::new));
    public static final Codec<TagEntry> CODEC = Codec.either(ExtraCodecs.TAG_OR_ELEMENT_ID, FULL_CODEC).xmap(e -> e.map(l -> new TagEntry((ExtraCodecs.TagOrElementLocation)l, true), r -> r), entry -> entry.required ? Either.left(entry.elementOrTag()) : Either.right(entry));
