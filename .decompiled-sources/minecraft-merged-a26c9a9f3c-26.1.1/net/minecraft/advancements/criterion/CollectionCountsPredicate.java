...
extends Predicate<Iterable<? extends T>> {
    ...
    public record Entry<T, P extends Predicate<T>>(P test, MinMaxBounds.Ints count) {
        public static <T, P extends Predicate<T>> Codec<Entry<T, P>> codec(Codec<P> elementCodec) {
            return RecordCodecBuilder.create(i -> i.group(((MapCodec)elementCodec.fieldOf("test")).forGetter(Entry::test), ((MapCodec)MinMaxBounds.Ints.CODEC.fieldOf("count")).forGetter(Entry::count)).apply((Applicative<Entry, ?>)i, Entry::new));
        }
