...
implements Comparable<AdvancementProgress> {
    ...
    public static final Codec<AdvancementProgress> CODEC = RecordCodecBuilder.create(i -> i.group(CRITERIA_CODEC.optionalFieldOf("criteria", Map.of()).forGetter(a -> a.criteria), ((MapCodec)Codec.BOOL.fieldOf("done")).orElse(true).forGetter(AdvancementProgress::isDone)).apply((Applicative<AdvancementProgress, ?>)i, (criteria, done) -> new AdvancementProgress(new HashMap<String, CriterionProgress>((Map<String, CriterionProgress>)criteria))));
    ...
    public void serializeToNetwork(FriendlyByteBuf output) {
        output.writeMap(this.criteria, FriendlyByteBuf::writeUtf, (b, v) -> v.serializeToNetwork((FriendlyByteBuf)b));
    }
    ...
    public Iterable<String> getRemainingCriteria() {
        ArrayList<String> remaining = Lists.newArrayList();
