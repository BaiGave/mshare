...
extends RecipeBook {
    ...
    public void rebuildCollections() {
        ...
        recipeListsByCategory.forEach((category, categoryRecipes) -> byCategory.put((SearchRecipeBookCategory)category, categoryRecipes.stream().map(RecipeCollection::new).peek(all::add).collect(ImmutableList.toImmutableList())));
        for (SearchRecipeBookCategory searchCategory : SearchRecipeBookCategory.values()) {
            byCategory.put(searchCategory, searchCategory.includedCategories().stream().flatMap(subCategory -> byCategory.getOrDefault(subCategory, List.of()).stream()).collect(ImmutableList.toImmutableList()));
        }
    ...
    private static Map<RecipeBookCategory, List<List<RecipeDisplayEntry>>> categorizeAndGroupRecipes(Iterable<RecipeDisplayEntry> recipes) {
        ...
        for (RecipeDisplayEntry entry : recipes) {
            ...
            ArrayList<RecipeDisplayEntry> groupRecipes = (ArrayList<RecipeDisplayEntry>)multiItemGroups.get(category, groupId.getAsInt());
