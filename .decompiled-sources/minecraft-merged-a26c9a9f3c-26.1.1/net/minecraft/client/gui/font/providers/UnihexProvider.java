...
implements GlyphProvider {
    ...
    static void readFromStream(InputStream input, ReaderOutput output) throws IOException {
        ...
        while (true) {
            boolean foundColon = UnihexProvider.copyUntil(input, buffer, 58);
            ...
            buffer.clear();
            UnihexProvider.copyUntil(input, buffer, 10);
            int dataDigitCount = buffer.size();
            LineData contents = switch (dataDigitCount) {
                case 32 -> ByteContents.read(line, buffer);
                case 64 -> ShortContents.read(line, buffer);
                case 96 -> IntContents.read24(line, buffer);
                case 128 -> IntContents.read32(line, buffer);
    ...
    implements GlyphProviderDefinition {
        public static final MapCodec<Definition> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(((MapCodec)Identifier.CODEC.fieldOf("hex_file")).forGetter(o -> o.hexFile), OverrideRange.CODEC.listOf().optionalFieldOf("size_overrides", List.of()).forGetter(o -> o.sizeOverrides)).apply((Applicative<Definition, ?>)i, Definition::new));
    ...
    public record Dimensions(int left, int right) {
        public static final MapCodec<Dimensions> MAP_CODEC = RecordCodecBuilder.mapCodec(i -> i.group(((MapCodec)Codec.INT.fieldOf("left")).forGetter(Dimensions::left), ((MapCodec)Codec.INT.fieldOf("right")).forGetter(Dimensions::right)).apply((Applicative<Dimensions, ?>)i, Dimensions::new));
    ...
    private record OverrideRange(int from, int to, Dimensions dimensions) {
        private static final Codec<OverrideRange> RAW_CODEC = RecordCodecBuilder.create(i -> i.group(((MapCodec)ExtraCodecs.CODEPOINT.fieldOf("from")).forGetter(OverrideRange::from), ((MapCodec)ExtraCodecs.CODEPOINT.fieldOf("to")).forGetter(OverrideRange::to), Dimensions.MAP_CODEC.forGetter(OverrideRange::dimensions)).apply((Applicative<OverrideRange, ?>)i, OverrideRange::new));
