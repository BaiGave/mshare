...
extends Screen {
    ...
    private void openExperimentsScreen(WorldDataConfiguration dataConfiguration) {
        ...
        if (settings != null) {
            this.minecraft.setScreen(new ExperimentsScreen(this, settings.getSecond(), packRepository -> this.tryApplyNewDataPacks((PackRepository)packRepository, false, this::openExperimentsScreen)));
        }
    }
    ...
    private void openDataPackSelectionScreen(WorldDataConfiguration dataConfiguration) {
        Pair<Path, PackRepository> settings = this.getDataPackSelectionSettings(dataConfiguration);
        if (settings != null) {
            this.minecraft.setScreen(new PackSelectionScreen(settings.getSecond(), packRepository -> this.tryApplyNewDataPacks((PackRepository)packRepository, true, this::openDataPackSelectionScreen), settings.getFirst(), Component.translatable("dataPack.title")));
        }
    }
    ...
    private void tryApplyNewDataPacks(PackRepository packRepository, boolean isDataPackScreen, Consumer<WorldDataConfiguration> onAbort) {
        List newDisabled;
        ImmutableList<String> newEnabled = ImmutableList.copyOf(packRepository.getSelectedIds());
        WorldDataConfiguration newConfig = new WorldDataConfiguration(new DataPackConfig(newEnabled, newDisabled = (List)packRepository.getAvailableIds().stream().filter(id -> !newEnabled.contains(id)).collect(ImmutableList.toImmutableList())), this.uiState.getSettings().dataConfiguration().enabledFeatures());
    ...
    private void applyNewPackConfig(PackRepository packRepository, WorldDataConfiguration newConfig, Consumer<WorldDataConfiguration> onAbort) {
        ...
        ((CompletableFuture)((CompletableFuture)WorldLoader.load(config, context -> {
            ...
            RegistryOps<JsonElement> writeOps = existingContext.worldgenLoadContext().createSerializationContext(JsonOps.INSTANCE);
            ...
            RegistryOps<JsonElement> readOps = context.datapackWorldgen().createSerializationContext(JsonOps.INSTANCE);
    ...
    public static @Nullable Path createTempDataPackDirFromExistingWorld(Path sourcePackDir, Minecraft minecraft) {
        MutableObject<@Nullable T> tempDataPackDir = new MutableObject();
