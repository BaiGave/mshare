...
implements AutoCloseable {
    ...
    public static FaviconTexture forWorld(TextureManager textureManager, String levelId) {
        return new FaviconTexture(textureManager, Identifier.withDefaultNamespace("worlds/" + Util.sanitizeName(levelId, Identifier::validPathChar) + "/" + String.valueOf(Hashing.sha1().hashUnencodedChars(levelId)) + "/icon"));
    }

    public static FaviconTexture forServer(TextureManager textureManager, String address) {
        return new FaviconTexture(textureManager, Identifier.withDefaultNamespace("servers/" + String.valueOf(Hashing.sha1().hashUnencodedChars(address)) + "/icon"));
    }
