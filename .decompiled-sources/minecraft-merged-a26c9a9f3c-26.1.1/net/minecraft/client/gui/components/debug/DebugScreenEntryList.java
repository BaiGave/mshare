...
public class DebugScreenEntryList {
    ...
    public void load() {
        try {
            ...
            Dynamic<JsonElement> data = new Dynamic<JsonElement>(JsonOps.INSTANCE, StrictJsonParser.parse(FileUtils.readFileToString(this.debugProfileFile, StandardCharsets.UTF_8)));
    ...
    public void save() {
        ...
        try {
            FileUtils.writeStringToFile(this.debugProfileFile, this.codec.encodeStart(JsonOps.INSTANCE, serializedOptions).getOrThrow().toString(), StandardCharsets.UTF_8);
        }
        catch (IOException e) {
    ...
    record SerializedOptions(Optional<DebugScreenProfile> profile, Optional<Map<Identifier, DebugScreenEntryStatus>> custom) {
        ...
        public static final Codec<SerializedOptions> CODEC = RecordCodecBuilder.create(i -> i.group(DebugScreenProfile.CODEC.optionalFieldOf("profile").forGetter(SerializedOptions::profile), CUSTOM_ENTRIES_CODEC.optionalFieldOf("custom").forGetter(SerializedOptions::custom)).apply((Applicative<SerializedOptions, ?>)i, SerializedOptions::new));
    }
}
