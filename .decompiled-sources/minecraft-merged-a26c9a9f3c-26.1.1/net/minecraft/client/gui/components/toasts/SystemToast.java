...
implements Toast {
    ...
    public SystemToast(SystemToastId id, Component title, @Nullable Component message) {
        this(id, title, SystemToast.nullToEmpty(message), Math.max(160, 30 + Math.max(Minecraft.getInstance().font.width(title), message == null ? 0 : Minecraft.getInstance().font.width(message))));
    }
