...
implements AutoCloseable {
    ...
    protected void apply(Map<String, List<Notification>> preparations, ResourceManager manager, ProfilerFiller profiler) {
        List<Notification> notifications = preparations.entrySet().stream().filter(e -> (Boolean)this.selector.apply((String)e.getKey())).map(Map.Entry::getValue).flatMap(Collection::stream).collect(Collectors.toList());
