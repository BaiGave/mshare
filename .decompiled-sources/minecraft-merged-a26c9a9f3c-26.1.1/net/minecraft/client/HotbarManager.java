...
public class HotbarManager {
    ...
    private void load() {
        try {
            ...
            for (int i = 0; i < 9; ++i) {
                this.hotbars[i] = Hotbar.CODEC.parse(NbtOps.INSTANCE, tag.get(String.valueOf(i))).resultOrPartial(error -> LOGGER.warn("Failed to parse hotbar: {}", error)).orElseGet(Hotbar::new);
            }
        }
        catch (Exception e) {
