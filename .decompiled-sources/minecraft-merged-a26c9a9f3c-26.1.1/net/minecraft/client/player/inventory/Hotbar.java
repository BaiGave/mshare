...
public class Hotbar {
    ...
    private static final Dynamic<?> EMPTY_STACK = new Dynamic<Tag>(DEFAULT_OPS, ItemStack.OPTIONAL_CODEC.encodeStart(DEFAULT_OPS, ItemStack.EMPTY).getOrThrow());
    ...
    public void storeFrom(Inventory inventory, RegistryAccess lookupProvider) {
        ...
        ImmutableList.Builder newItems = ImmutableList.builderWithExpectedSize(SIZE);
        for (int i = 0; i < SIZE; ++i) {
            ItemStack item = inventory.getItem(i);
            Optional<Dynamic> result = ItemStack.OPTIONAL_CODEC.encodeStart(registryOps, item).resultOrPartial(error -> LOGGER.warn("Could not encode hotbar item: {}", error)).map(tag -> new Dynamic<Tag>(DEFAULT_OPS, (Tag)tag));
