...
public class BlockTintCache {
        ...
        try {
            ...
            for (int offsetX = -1; offsetX <= 1; ++offsetX) {
                for (int offsetZ = -1; offsetZ <= 1; ++offsetZ) {
                    ...
                    CacheData removed = this.cache.remove(key);
    ...
    private CacheData findOrCreateChunkCache(int x, int z) {
        ...
        try {
            CacheData existing = this.cache.get(key);
            if (existing != null) {
        ...
        try {
            CacheData cacheData;
            CacheData existingNow = this.cache.get(key);
            if (existingNow != null) {
                CacheData cacheData2 = existingNow;
                return cacheData2;
            }
            CacheData newCache = new CacheData();
            if (this.cache.size() >= 256 && (cacheData = this.cache.removeFirst()) != null) {
    ...
    private static class CacheData {
        ...
        public int[] getLayer(int y) {
            ...
            this.lock.writeLock().lock();
            try {
                int[] nArray = this.cache.computeIfAbsent(y, n -> this.allocateLayer());
                return nArray;
            }
            finally {
                this.lock.writeLock().unlock();
