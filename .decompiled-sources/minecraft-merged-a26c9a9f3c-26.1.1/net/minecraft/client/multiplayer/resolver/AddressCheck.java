...
public interface AddressCheck {
    ...
    public static AddressCheck createFromService() {
        final ImmutableList blockLists = Streams.stream(ServiceLoader.load(BlockListSupplier.class)).map(BlockListSupplier::createBlockList).filter(Objects::nonNull).collect(ImmutableList.toImmutableList());
