...
implements ItemModel {
    ...
    public record SwitchCase<T>(List<T> values, ItemModel.Unbaked model) {
        public static <T> Codec<SwitchCase<T>> codec(Codec<T> valueCodec) {
            return RecordCodecBuilder.create(i -> i.group(((MapCodec)ExtraCodecs.nonEmptyList(ExtraCodecs.compactListCodec(valueCodec)).fieldOf("when")).forGetter(SwitchCase::values), ((MapCodec)ItemModels.CODEC.fieldOf("model")).forGetter(SwitchCase::model)).apply((Applicative<SwitchCase, ?>)i, SwitchCase::new));
        }
    }
    ...
    public record UnbakedSwitch<P extends SelectItemModelProperty<T>, T>(P property, List<SwitchCase<T>> cases) {
        ...
        private ModelSelector<T> createModelGetter(Object2ObjectMap<T, ItemModel> originalModels, @Nullable RegistryContextSwapper registrySwapper) {
            ...
            CacheSlot<ClientLevel, Object2ObjectMap> remappedModelCache = new CacheSlot<ClientLevel, Object2ObjectMap>(clientLevel -> {
                ...
                originalModels.forEach((value, model) -> registrySwapper.swapTo(this.property.valueCodec(), value, clientLevel.registryAccess()).ifSuccess(remappedValue -> remappedModels.put(remappedValue, (ItemModel)model)));
    ...
    {
        public static final MapCodec<Unbaked> MAP_CODEC = RecordCodecBuilder.mapCodec(i -> i.group(Transformation.EXTENDED_CODEC.optionalFieldOf("transformation").forGetter(Unbaked::transformation), UnbakedSwitch.MAP_CODEC.forGetter(Unbaked::unbakedSwitch), ItemModels.CODEC.optionalFieldOf("fallback").forGetter(Unbaked::fallback)).apply((Applicative<Unbaked, ?>)i, Unbaked::new));
