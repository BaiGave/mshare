...
implements ItemModel {
    ...
    public record Entry(float threshold, ItemModel.Unbaked model) {
        public static final Codec<Entry> CODEC = RecordCodecBuilder.create(i -> i.group(((MapCodec)Codec.FLOAT.fieldOf("threshold")).forGetter(Entry::threshold), ((MapCodec)ItemModels.CODEC.fieldOf("model")).forGetter(Entry::model)).apply((Applicative<Entry, ?>)i, Entry::new));
    ...
    {
        public static final MapCodec<Unbaked> MAP_CODEC = RecordCodecBuilder.mapCodec(i -> i.group(Transformation.EXTENDED_CODEC.optionalFieldOf("transformation").forGetter(Unbaked::transformation), RangeSelectItemModelProperties.MAP_CODEC.forGetter(Unbaked::property), Codec.FLOAT.optionalFieldOf("scale", Float.valueOf(1.0f)).forGetter(Unbaked::scale), ((MapCodec)Entry.CODEC.listOf().fieldOf("entries")).forGetter(Unbaked::entries), ItemModels.CODEC.optionalFieldOf("fallback").forGetter(Unbaked::fallback)).apply((Applicative<Unbaked, ?>)i, Unbaked::new));
