...
implements AutoCloseable {
    ...
    public void uploadSpriteUbo(ByteBuffer uboBuffer, int startOffset, int maxMipLevel, int atlasWidth, int atlasHeight, int spriteUboSize) {
        for (int level = 0; level <= maxMipLevel; ++level) {
            Std140Builder.intoBuffer(MemoryUtil.memSlice(uboBuffer, startOffset + level * spriteUboSize, spriteUboSize)).putMat4f(new Matrix4f().ortho2D(0.0f, atlasWidth >> level, 0.0f, atlasHeight >> level)).putMat4f(new Matrix4f().translate(this.x >> level, this.y >> level, 0.0f).scale(this.contents.width() + this.padding * 2 >> level, this.contents.height() + this.padding * 2 >> level, 1.0f)).putFloat((float)this.padding / (float)this.contents.width()).putFloat((float)this.padding / (float)this.contents.height()).putInt(level);
        }
    }
