...
public class SpriteLoader {
    ...
    private Preparations stitch(List<SpriteContents> sprites, int maxMipmapLevels, Executor executor) {
        try (Zone ignored = Profiler.get().zone(() -> "stitch " + String.valueOf(this.location));){
            ...
            for (SpriteContents spriteInfo : sprites) {
                ...
                LOGGER.warn("Texture {} with size {}x{} limits mip level from {} to {}", spriteInfo.name(), spriteInfo.width(), spriteInfo.height(), Mth.log2(lowestOneBit), Mth.log2(lowestTextureBit));
    ...
    @Environment(value=EnvType.CLIENT)
    public record Preparations(int width, int height, int mipLevel, TextureAtlasSprite missing, Map<Identifier, TextureAtlasSprite> regions, CompletableFuture<Void> readyForUpload) implements FabricPreparations
    {
