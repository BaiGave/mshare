...
public record PalettedPermutations(List<Identifier> textures, Identifier paletteKey, Map<String, Identifier> permutations, String separator) implements SpriteSource
{
    ...
    public static final MapCodec<PalettedPermutations> MAP_CODEC = RecordCodecBuilder.mapCodec(i -> i.group(((MapCodec)Codec.list(Identifier.CODEC).fieldOf("textures")).forGetter(PalettedPermutations::textures), ((MapCodec)Identifier.CODEC.fieldOf("palette_key")).forGetter(PalettedPermutations::paletteKey), ((MapCodec)Codec.unboundedMap(Codec.STRING, Identifier.CODEC).fieldOf("permutations")).forGetter(PalettedPermutations::permutations), Codec.STRING.optionalFieldOf("separator", DEFAULT_SEPARATOR).forGetter(PalettedPermutations::separator)).apply((Applicative<PalettedPermutations, ?>)i, PalettedPermutations::new));
    ...
    public void run(ResourceManager resourceManager, SpriteSource.Output output) {
        ...
        this.permutations.forEach((suffix, palette) -> palettes.put(suffix, Suppliers.memoize(() -> PalettedPermutations.lambda$run$2((java.util.function.Supplier)paletteKeySupplier, resourceManager, palette))));
