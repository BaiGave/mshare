...
public record EquipmentClientInfo(Map<LayerType, List<Layer>> layers) {
    ...
    public static final Codec<EquipmentClientInfo> CODEC = RecordCodecBuilder.create(i -> i.group(((MapCodec)ExtraCodecs.nonEmptyMap(Codec.unboundedMap(LayerType.CODEC, LAYER_LIST_CODEC)).fieldOf("layers")).forGetter(EquipmentClientInfo::layers)).apply((Applicative<EquipmentClientInfo, ?>)i, EquipmentClientInfo::new));
    ...
    public static class Builder {
        ...
        public EquipmentClientInfo build() {
            return new EquipmentClientInfo((Map<LayerType, List<Layer>>)this.layersByType.entrySet().stream().collect(ImmutableMap.toImmutableMap(Map.Entry::getKey, entry -> List.copyOf((Collection)entry.getValue()))));
        }
    }
    ...
    public record Dyeable(Optional<Integer> colorWhenUndyed) {
        public static final Codec<Dyeable> CODEC = RecordCodecBuilder.create(i -> i.group(ExtraCodecs.RGB_COLOR_CODEC.optionalFieldOf("color_when_undyed").forGetter(Dyeable::colorWhenUndyed)).apply((Applicative<Dyeable, ?>)i, Dyeable::new));
    }
}
