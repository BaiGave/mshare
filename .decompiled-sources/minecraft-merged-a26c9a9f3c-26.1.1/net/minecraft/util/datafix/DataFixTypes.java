...
public enum DataFixTypes {
    ...
    public <A> Codec<A> wrapCodec(final Codec<A> codec, final DataFixer dataFixer, final int defaultVersion) {
        return new Codec<A>(this){
            ...
            public <T> DataResult<T> encode(A input, DynamicOps<T> ops, T prefix) {
                return codec.encode(input, ops, prefix).flatMap((? super R data) -> ops.mergeToMap(data, ops.createString("DataVersion"), ops.createInt(DataFixTypes.currentVersion())));
            }
            ...
            public <T> DataResult<Pair<A, T>> decode(DynamicOps<T> ops, T input) {
                ...
                Dynamic<T> dataWithoutVersion = new Dynamic<T>(ops, ops.remove(input, "DataVersion"));
                Dynamic<T> fixedData = this.this$0.updateToCurrentVersion(dataFixer, dataWithoutVersion, fromVersion);
    ...
    public CompoundTag update(DataFixer fixer, CompoundTag tag, int fromVersion, int toVersion) {
        return this.update(fixer, new Dynamic<CompoundTag>(NbtOps.INSTANCE, tag), fromVersion, toVersion).getValue();
    }
