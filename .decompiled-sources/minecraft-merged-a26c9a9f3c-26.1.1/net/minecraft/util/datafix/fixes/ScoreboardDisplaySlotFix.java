...
extends DataFix {
    ...
    protected TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped("Scoreboard DisplaySlot rename", scoreboardType, input -> input.updateTyped(rootTagFinder, scoreboardInfo -> scoreboardInfo.update(DSL.remainderFinder(), tag -> tag.update("DisplaySlots", slots -> slots.updateMapValues(pair -> pair.mapFirst(key -> DataFixUtils.orElse(key.asString().result().map(ScoreboardDisplaySlotFix::rename).map(key::createString), key)))))));
    }
}
