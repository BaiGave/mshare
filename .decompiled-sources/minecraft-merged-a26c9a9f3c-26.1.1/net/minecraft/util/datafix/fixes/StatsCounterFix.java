...
extends DataFix {
    ...
    private TypeRewriteRule makeStatFixer() {
        ...
        return this.fixTypeEverywhereTyped("StatsCounterFix", inputType, outputType, (Typed<?> input) -> {
            Dynamic<?> tag = input.get(DSL.remainderFinder());
            HashMap stats = Maps.newHashMap();
            Optional<Map<Dynamic<?>, Dynamic<?>>> map = tag.getMapValues().result();
            if (map.isPresent()) {
                for (Map.Entry<Dynamic<?>, Dynamic<?>> entry : map.get().entrySet()) {
                    String key;
                    StatType statType;
                    if (!entry.getValue().asNumber().result().isPresent() || (statType = StatsCounterFix.unpackLegacyKey(key = entry.getKey().asString(""))) == null) continue;
    ...
    private TypeRewriteRule makeObjectiveFixer() {
        ...
        return this.fixTypeEverywhereTyped("ObjectiveStatFix", inputType, outputType, (Typed<?> input) -> {
            Dynamic<?> tag = input.get(DSL.remainderFinder());
