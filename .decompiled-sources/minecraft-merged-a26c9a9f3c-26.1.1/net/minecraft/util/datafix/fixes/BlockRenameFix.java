...
extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        Type<Pair<String, String>> expectedType;
        Type<?> blockType = this.getInputSchema().getType(References.BLOCK_NAME);
        if (!Objects.equals(blockType, expectedType = DSL.named(References.BLOCK_NAME.typeName(), NamespacedSchema.namespacedString()))) {
        ...
        TypeRewriteRule flatBlockStateRule = this.fixTypeEverywhereTyped(this.name + " for flat_block_state", this.getInputSchema().getType(References.FLAT_BLOCK_STATE), input -> input.update(DSL.remainderFinder(), tag -> DataFixUtils.orElse(tag.asString().result().map(this::fixFlatBlockState).map(tag::createString), tag)));
    ...
    private Dynamic<?> fixBlockState(Dynamic<?> tag) {
        ...
        if (name.isPresent()) {
            return tag.set("Name", tag.createString(this.renameBlock(name.get())));
        }
        return tag;
    }
