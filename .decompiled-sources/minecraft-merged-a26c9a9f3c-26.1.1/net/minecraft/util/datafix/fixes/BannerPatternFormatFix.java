...
extends NamedEntityFix {
    ...
    private static Dynamic<?> fixLayer(Dynamic<?> dynamic) {
        dynamic = dynamic.renameAndFixField("Pattern", "pattern", pattern -> DataFixUtils.orElse(pattern.asString().map(id -> PATTERN_ID_MAP.getOrDefault(id, (String)id)).map(pattern::createString).result(), pattern));
