...
extends DataFix {
    ...
    private <T> Dynamic<T> fix(Dynamic<T> input) {
        ...
        Dynamic result = input.createMap(Map.of(input.createString("type"), input.createString(id)));
        return switch (id) {
            case "minecraft:item" -> {
                if (parts.length > 1) {
    ...
    private <T> Dynamic<T> updateItem(Dynamic<T> result, String contents) {
        ...
        if (tagPartStart == -1) {
            ...
            Dynamic itemTag = ParticleUnflatteningFix.parseTag(result.getOps(), contents.substring(tagPartStart));
            if (itemTag != null) {
    ...
    private static <T> @Nullable Dynamic<T> parseTag(DynamicOps<T> ops, String contents) {
        try {
            return new Dynamic<T>(ops, TagParser.create(ops).parseFully(contents));
        }
        catch (Exception e) {
    ...
    private static <T> Dynamic<T> readVector(Dynamic<T> result, StringReader reader) throws CommandSyntaxException {
        ...
        return result.createList(Stream.of(Float.valueOf(x), Float.valueOf(y), Float.valueOf(z)).map(result::createFloat));
    }
