...
extends DataFix {
    ...
    private static Pair<Dynamic<?>, Dynamic<?>> fixDimension(Pair<Dynamic<?>, Dynamic<?>> entry) {
        ...
        return Pair.of(entry.getFirst(), dimension.update("generator", g -> g.update("settings", s -> s.update("structures", StructureSettingsFlattenFix::fixStructures))));
    }
    ...
    private static Dynamic<?> fixStructures(Dynamic<?> input) {
        ...
        return DataFixUtils.orElse(input.get("stronghold").result().map(stronghold -> structures.set("minecraft:stronghold", stronghold.set("type", input.createString("minecraft:concentric_rings")))), structures);
    }
}
