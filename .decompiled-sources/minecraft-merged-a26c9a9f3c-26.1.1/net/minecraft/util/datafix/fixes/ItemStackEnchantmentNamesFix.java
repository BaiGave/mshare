...
extends DataFix {
    ...
    private Dynamic<?> fixTag(Dynamic<?> tag) {
        Optional<Dynamic> newEnch = tag.get("ench").asStreamOpt().map(s -> s.map(element -> element.set("id", element.createString(MAP.getOrDefault(element.get("id").asInt(0), "null"))))).map(tag::createList).result();
        if (newEnch.isPresent()) {
            tag = tag.remove("ench").set("Enchantments", newEnch.get());
        }
        return tag.update("StoredEnchantments", list -> DataFixUtils.orElse(list.asStreamOpt().map(l -> l.map(enchant -> enchant.set("id", enchant.createString(MAP.getOrDefault(enchant.get("id").asInt(0), "null"))))).map(list::createList).result(), list));
    }
}
