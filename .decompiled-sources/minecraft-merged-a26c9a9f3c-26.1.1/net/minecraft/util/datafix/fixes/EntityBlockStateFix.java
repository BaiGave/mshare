...
extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped("EntityBlockStateFix", inputSchema.getType(References.ENTITY), outputSchema.getType(References.ENTITY), (Typed<?> input) -> {
    ...
    private Typed<?> updateFallingBlock(Typed<?> input) {
        ...
        return input.update(oldType.finder(), newType, tile -> {
            int block = tile.map(l -> ((Either)l.getSecond()).map(l2 -> l2, EntityBlockStateFix::getBlockId), r -> {
    ...
    private Typed<?> updateBlockToBlockState(Typed<?> input, String oldFieldName, String dataName, String newFieldName) {
        Tag.TagType<Pair<String, Either<Integer, String>>> oldType = DSL.field(oldFieldName, DSL.named(References.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), NamespacedSchema.namespacedString())));
        Tag.TagType<Pair<String, Dynamic<?>>> newType = DSL.field(newFieldName, DSL.named(References.BLOCK_STATE.typeName(), DSL.remainderType()));
        Dynamic<?> tag = input.getOrCreate(DSL.remainderFinder());
        return input.update(oldType.finder(), newType, tile -> {
            int block = ((Either)tile.getSecond()).map(l -> l, EntityBlockStateFix::getBlockId);
