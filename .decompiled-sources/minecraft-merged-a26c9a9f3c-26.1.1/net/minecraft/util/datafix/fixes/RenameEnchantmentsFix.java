...
extends DataFix {
    ...
    private Dynamic<?> fixEnchantmentList(Dynamic<?> itemStack, String field) {
        return itemStack.update(field, tag -> tag.asStreamOpt().map(s -> s.map(element -> element.update("id", id -> id.asString().map(stringId -> element.createString(this.renames.getOrDefault(NamespacedSchema.ensureNamespaced(stringId), (String)stringId))).mapOrElse(Function.identity(), fail -> id)))).map(tag::createList).mapOrElse(Function.identity(), fail -> tag));
    }
}
