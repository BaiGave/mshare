...
extends DataFix {
    ...
    private Dynamic<?> fix(Dynamic<?> input) {
        Optional<Dynamic<?>> level = input.get("Level").result();
        if (level.isPresent() && level.get().get("Sections").asStreamOpt().result().isPresent()) {
            return input.set("Level", new UpgradeChunk(level.get()).write());
        }
        return input;
    }
    ...
    private static final class UpgradeChunk {
        ...
        public UpgradeChunk(Dynamic<?> level) {
            ...
            level.get("TileEntities").asStreamOpt().ifSuccess(s -> s.forEach(entity -> {
                ...
                if (this.blockEntities.put(key, (Dynamic<?>)entity) != null) {
                    LOGGER.warn("In chunk: {}x{} found a duplicate block entity at position: [{}, {}, {}]", this.x, this.z, x, y, z);
                }
            }));
        ...
        public Dynamic<?> write() {
            Dynamic<Object> level = this.level;
            ...
            ArrayList<Dynamic<?>> sections = Lists.newArrayList();
            for (Section section : this.sections) {
                if (section == null) continue;
    ...
    private static class Section {
        ...
        public int upgrade(int sides) {
            ...
            DataLayer data = this.section.get("Data").asByteBufferOpt().map(buffer -> new DataLayer(DataFixUtils.toArray(buffer))).result().orElseGet(DataLayer::new);
            DataLayer addBlocks = this.section.get("Add").asByteBufferOpt().map(buffer -> new DataLayer(DataFixUtils.toArray(buffer))).result().orElseGet(DataLayer::new);
        ...
        public Dynamic<?> write() {
            Dynamic<Object> section = this.section;
            if (!this.hasData) {
                return section;
            }
