...
extends DataFix {
    ...
    private static void fixItemStack(ItemStackData itemStack, Dynamic<?> dynamic) {
        ...
        itemStack.fixSubTag("BlockEntityTag", false, blockEntityTag -> {
            ...
            if (withoutId.equals(blockEntityTag.emptyMap())) {
                return withoutId;
            }
        ...
        Optional<Dynamic<?>> trim = itemStack.removeTag("Trim").result();
        if (trim.isPresent()) {
            Dynamic fixedTrim = trim.get();
            if ((hideFlags & 0x80) != 0) {
    ...
    private static Dynamic<?> fixBlockStateTag(Dynamic<?> blockStateTag) {
        return DataFixUtils.orElse(blockStateTag.asMapOpt().result().map(entries -> entries.collect(Collectors.toMap(Pair::getFirst, entry -> {
            ...
            if (number.isPresent()) {
                return value.createString(number.get().toString());
            }
            return value;
    ...
    private static <T> Dynamic<T> fixBlockEntityTag(ItemStackData itemStack, Dynamic<T> blockEntity, String id) {
        ...
        if (lootTable.isPresent()) {
            Dynamic containerLoot = blockEntity.emptyMap().set("loot_table", lootTable.get());
        ...
        return switch (id) {
            ...
            case "minecraft:decorated_pot" -> {
                ...
                if (item.isPresent()) {
                    itemStack.setComponent("minecraft:container", blockEntity.createList(Stream.of(blockEntity.emptyMap().set("slot", blockEntity.createInt(0)).set("item", item.get()))));
                }
            ...
            case "minecraft:shulker_box", "minecraft:chest", "minecraft:trapped_chest", "minecraft:furnace", "minecraft:ender_chest", "minecraft:dispenser", "minecraft:dropper", "minecraft:brewing_stand", "minecraft:hopper", "minecraft:barrel", "minecraft:smoker", "minecraft:blast_furnace", "minecraft:campfire", "minecraft:chiseled_bookshelf", "minecraft:crafter" -> {
                List<Dynamic> items = blockEntity.get("Items").asList(dynamic -> dynamic.emptyMap().set("slot", dynamic.createInt(dynamic.get("Slot").asByte((byte)0) & 0xFF)).set("item", dynamic.remove("Slot")));
                if (!items.isEmpty()) {
    ...
    private static void fixEnchantments(ItemStackData itemStack, Dynamic<?> dynamic, String key, String componentType, boolean hideInTooltip) {
        ...
        if (!enchantments.isEmpty() || hideInTooltip) {
            ...
            for (Pair enchantment2 : enchantments) {
                levels = levels.set((String)enchantment2.getFirst(), dynamic.createInt((Integer)enchantment2.getSecond()));
            }
    ...
    private static void fixBlockStatePredicates(ItemStackData itemStack, Dynamic<?> dynamic, String tag, String componentId, boolean hideInTooltip) {
        Optional<Dynamic<?>> oldPredicate = itemStack.removeTag(tag).result();
        if (oldPredicate.isEmpty()) {
            return;
        }
        Dynamic component = dynamic.emptyMap().set("predicates", dynamic.createList(oldPredicate.get().asStream().map(value -> DataFixUtils.orElse(value.asString().map(string -> ItemStackComponentizationFix.fixBlockStatePredicate(value, string)).result(), value))));
        if (hideInTooltip) {
            component = component.set("show_in_tooltip", dynamic.createBoolean(false));
        }
    ...
    private static Dynamic<?> fixBlockStatePredicate(Dynamic<?> dynamic, String string) {
        ...
        if (startProperties != -1 && endProperties != -1) {
            ...
            Iterable<String> flatProperties = PROPERTY_SPLITTER.split(string.substring(startProperties + 1, endProperties));
    ...
    private static void fixAttributeModifiers(ItemStackData itemStack, Dynamic<?> dynamic, int hideFlags) {
        ...
        List<Dynamic> attributeModifiers = attributeModifiersField.asList(ItemStackComponentizationFix::fixAttributeModifier);
    ...
    private static Dynamic<?> fixAttributeModifier(Dynamic<?> input) {
        Dynamic<Object> result = input.emptyMap().set("name", input.createString("")).set("amount", input.createDouble(0.0)).set("operation", input.createString("add_value"));
        ...
        result = Dynamic.copyAndFixField(input, "Operation", result, "operation", operation -> operation.createString(switch (operation.asInt(0)) {
            default -> "add_value";
    ...
    private static void fixPotionContents(ItemStackData itemStack, Dynamic<?> dynamic) {
        Dynamic<Object> component = dynamic.emptyMap();
        ...
        if (!(component = itemStack.moveTagInto("custom_potion_effects", component, "custom_effects")).equals(dynamic.emptyMap())) {
    ...
    private static @Nullable Dynamic<?> fixBookPages(ItemStackData itemStack, Dynamic<?> dynamic) {
        List<String> pages = itemStack.removeTag("pages").asList(page -> page.asString(""));
        Map<String, String> filteredPages = itemStack.removeTag("filtered_pages").asMap(key -> key.asString("0"), page -> page.asString(""));
        if (pages.isEmpty()) {
            return null;
        }
        ArrayList fixedPages = new ArrayList(pages.size());
        for (int i = 0; i < pages.size(); ++i) {
            String page2 = pages.get(i);
    ...
    private static void fixBucketedMobData(ItemStackData itemStack, Dynamic<?> dynamic) {
        Dynamic<Object> data = dynamic.emptyMap();
    ...
    private static void fixLodestoneTracker(ItemStackData itemStack, Dynamic<?> dynamic) {
        Optional<Dynamic<?>> lodestonePos = itemStack.removeTag("LodestonePos").result();
        Optional<Dynamic<?>> lodestoneDimension = itemStack.removeTag("LodestoneDimension").result();
    ...
    public static Dynamic<?> fixProfile(Dynamic<?> dynamic) {
        ...
        if (simpleName.isPresent()) {
            if (ItemStackComponentizationFix.isValidPlayerName(simpleName.get())) {
                return dynamic.emptyMap().set("name", dynamic.createString(simpleName.get()));
            }
            return dynamic.emptyMap();
        }
        String name = dynamic.get("Name").asString("");
        Optional<Dynamic<?>> id = dynamic.get("Id").result();
    ...
    private static @Nullable Dynamic<?> fixProfileProperties(OptionalDynamic<?> dynamic) {
        Map<String, List> properties = dynamic.asMap(key -> key.asString(""), list -> list.asList(property -> {
    ...
    private static class ItemStackData {
        ...
        public static Optional<ItemStackData> read(Dynamic<?> dynamic) {
            return dynamic.get("id").asString().apply2stable((item, count) -> new ItemStackData((String)item, count.intValue(), dynamic.remove("id").remove("Count")), dynamic.get("Count").asNumber()).result();
        }
        ...
        public void setComponent(String type, OptionalDynamic<?> optionalValue) {
            optionalValue.result().ifPresent(value -> {
                this.components = this.components.set(type, (Dynamic<?>)value);
            });
        }

        public Dynamic<?> moveTagInto(String fromKey, Dynamic<?> target, String toKey) {
            Optional<Dynamic<?>> value = this.removeTag(fromKey).result();
            if (value.isPresent()) {
                return target.set(toKey, value.get());
            }
            return target;
        }

        public void moveTagToComponent(String key, String type, Dynamic<?> defaultValue) {
            Optional<Dynamic<?>> value = this.removeTag(key).result();
            if (value.isPresent() && !value.get().equals(defaultValue)) {
                this.setComponent(type, value.get());
            }
        }
        ...
        public void fixSubTag(String key, boolean dontFixWhenFieldIsMissing, UnaryOperator<Dynamic<?>> function) {
            ...
            this.tag = (map = (Dynamic)function.apply(map)).equals(map.emptyMap()) ? this.tag.remove(key) : this.tag.set(key, map);
        }
        ...
        private static <T> Dynamic<T> mergeRemainder(Dynamic<T> itemStack, Dynamic<?> remainder) {
            ...
            return ops.getMap(itemStack.getValue()).flatMap(itemStackMap -> ops.mergeToMap((Object)remainder.convert(ops).getValue(), (MapLike)itemStackMap)).map(merged -> new Dynamic<Object>(ops, merged)).result().orElse(itemStack);
        }
