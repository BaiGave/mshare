...
extends DataFix {
    ...
    private TypeRewriteRule createCriteriaRule() {
        ...
        TaggedChoice.TaggedChoiceType<?> choiceType = criteriaTypeFinder.type().findChoiceType("type", -1).orElseThrow(() -> new IllegalStateException("Can't find choice type for criteria"));
        ...
        return this.fixTypeEverywhereTyped(this.name, inputType, outputType, (Typed<?> input) -> input.updateTyped(criteriaTypeFinder, stats -> stats.updateTyped(customTypeFinder, custom -> custom.update(idFinder, value -> this.renames.getOrDefault(value, (String)value)))));
    }
    ...
    private TypeRewriteRule createStatRule() {
        ...
        return this.fixTypeEverywhereTyped(this.name, inputType, outputType, (Typed<?> input) -> input.updateTyped(statsFinder, stats -> stats.updateTyped(customFinder, custom -> custom.update(nameFinder, value -> this.renames.getOrDefault(value, (String)value)))));
    }
}
