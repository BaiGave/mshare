...
extends DataFix {
    ...
    protected TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped(NAME, oldChunkType, chunkType, (Typed<?> chunk) -> chunk.updateTyped(levelFinder, levelType, level -> {
            Dynamic tag = level.get(DSL.remainderFinder());
            OptionalDynamic<?> contextTag = chunk.get(DSL.remainderFinder()).get(DATAFIXER_CONTEXT_TAG);
            ...
            Dynamic<?> airContainer = ChunkHeightAndBiomeFix.makePalettedContainer(tag.createList(Stream.of(tag.createMap(ImmutableMap.of(tag.createString("Name"), tag.createString("minecraft:air"))))));
            HashSet blocksInChunk = Sets.newHashSet();
            MutableObject<Supplier<@Nullable ChunkProtoTickListFix.PoorMansPalettedContainer>> bedrockSectionBlocks = new MutableObject<Supplier<ChunkProtoTickListFix.PoorMansPalettedContainer>>(() -> null);
            level = level.updateTyped(sectionsFinder, sectionsType, sections -> {
                IntOpenHashSet doneSections = new IntOpenHashSet();
                Dynamic<?> dynamic = sections.write().result().orElseThrow(() -> new IllegalStateException("Malformed Chunk.Level.Sections"));
                List sectionsList = dynamic.asStream().map(section -> {
                    int sectionY = section.get("Y").asInt(0);
                    Dynamic blockStatesContainer = DataFixUtils.orElse(section.get("Palette").result().flatMap(palette -> {
                ...
                for (int sectionIndex = 0; sectionIndex < biomeContainers.length; ++sectionIndex) {
                    ...
                    Dynamic section2 = tag.createMap(Map.of(tag.createString("Y"), tag.createInt(sectionY)));
                    section2 = section2.set("block_states", airContainer);
                    section2 = section2.set("biomes", biomeContainers[sectionIndex]);
    ...
    private static Dynamic<?> updateChunkTag(Dynamic<?> chunkTag, boolean isOverworld, boolean wasIncreasedHeight, boolean needsBlendingAndUpgrade, Supplier<@Nullable ChunkProtoTickListFix.PoorMansPalettedContainer> bedrockSectionBlocks) {
        ...
        if (statusOpt.isPresent() && !"empty".equals(lastStatus = (status = statusOpt.get()).asString(""))) {
    ...
    private static Dynamic<?> makeBiomeContainer(Dynamic<?> tag, Int2IntFunction sourceStorage) {
        ...
        Dynamic palette = tag.createList(idMap.keySet().stream().map(biomeId1 -> tag.createString(BIOMES_BY_ID.getOrDefault((int)biomeId1, DEFAULT_BIOME))));
