...
extends DataFix {
    ...
    private static Dynamic<?> fixItemStackComponents(Dynamic<?> components) {
        return components.update("minecraft:attribute_modifiers", attributeModifiers -> attributeModifiers.update("modifiers", modifiers -> DataFixUtils.orElse(modifiers.asStreamOpt().result().map(AttributeModifierIdFix::fixModifiersTypeWrapper).map(modifiers::createList), modifiers)));
    }

    private static Dynamic<?> fixAttribute(Dynamic<?> attribute) {
        return attribute.renameField("Name", "id").renameField("Base", "base").renameAndFixField("Modifiers", "modifiers", modifiers -> DataFixUtils.orElse(modifiers.asStreamOpt().result().map(s -> s.map(AttributeModifierIdFix::convertModifierForEntity)).map(AttributeModifierIdFix::fixModifiersTypeWrapper).map(attribute::createList), modifiers));
    }

    private static Typed<?> fixEntity(Typed<?> entity) {
        return entity.update(DSL.remainderFinder(), tag -> tag.renameAndFixField("Attributes", "attributes", attributeList -> DataFixUtils.orElse(attributeList.asStreamOpt().result().map(s -> s.map(AttributeModifierIdFix::fixAttribute)).map(attributeList::createList), attributeList)));
    }
