...
extends NamedEntityFix {
    ...
    protected Typed<?> fix(Typed<?> entity) {
        ...
        return entity.set(DSL.remainderFinder(), remainder.remove("Profession").remove("Career").remove("CareerLevel").set("VillagerData", remainder.createMap(ImmutableMap.of(remainder.createString("type"), remainder.createString("minecraft:plains"), remainder.createString("profession"), remainder.createString(VillagerDataFix.upgradeData(remainder.get("Profession").asInt(0), remainder.get("Career").asInt(0))), remainder.createString("level"), DataFixUtils.orElse(remainder.get("CareerLevel").result(), remainder.createInt(1))))));
    }
