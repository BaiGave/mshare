...
public class EncoderCache {
    ...
    public EncoderCache(int maximumSize) {
        this.cache = CacheBuilder.newBuilder().maximumSize(maximumSize).concurrencyLevel(1).softValues().build(new CacheLoader<Key<?, ?>, DataResult<?>>(this){
            {
    ...
    public <A> Codec<A> wrap(final Codec<A> codec) {
        return new Codec<A>(this){
            ...
            public <T> DataResult<T> encode(A input, DynamicOps<T> ops, T prefix) {
                return this.this$0.cache.getUnchecked(new Key(codec, input, ops)).map((? super R value) -> {
