...
implements ProfileResults {
    ...
    public List<ResultField> getTimes(String path) {
        ...
        ArrayList<ResultField> result = Lists.newArrayList();
    ...
    private Map<String, CounterCollector> getCounterValues() {
        TreeMap<String, CounterCollector> result = Maps.newTreeMap();
        this.entries.forEach((path, entry) -> {
            Object2LongMap<String> counters = entry.getCounters();
            if (!counters.isEmpty()) {
                List<String> pathSegments = SPLITTER.splitToList((CharSequence)path);
                counters.forEach((counter, value) -> result.computeIfAbsent((String)counter, k -> new CounterCollector()).addValue(pathSegments.iterator(), value));
            }
        });
        return result;
    }
