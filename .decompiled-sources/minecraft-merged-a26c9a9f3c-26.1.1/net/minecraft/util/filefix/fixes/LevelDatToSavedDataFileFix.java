...
extends FileFix {
    ...
    public void makeFixer() {
        this.addFileContentFix(files -> {
            ...
            return upgradeProgress -> {
                ...
                Dynamic<Tag> content = readData.get();
    ...
    private Dynamic<?> extractPlayerDataToFile(FileAccess<PlayerData> fallbackFile, Dynamic<?> content) {
        ...
        Optional<Dynamic<?>> playerUuid = playerTagFixed.get("UUID").result();
        if (playerUuid.isPresent()) {
            usedUuid = playerUuid.get();
        } else {
    ...
    private static void extractWorldBorderToFile(FileAccess<? extends CompressedNbt> targetFile, Dynamic<?> content, double divider) {
        ...
        Dynamic<?> worldBorderTag = worldBorderTagOpt.result().get().update("center_x", x -> x.createDouble(x.asDouble(0.0) / divider)).update("center_z", z -> z.createDouble(z.asDouble(0.0) / divider));
