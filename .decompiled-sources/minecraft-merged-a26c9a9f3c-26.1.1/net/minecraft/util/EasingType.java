...
public interface EasingType {
    ...
    public static final Codec<EasingType> CODEC = Codec.either(SIMPLE_REGISTRY.codec(Codec.STRING), CubicBezier.CODEC).xmap(Either::unwrap, easing -> {
        Either<EasingType, Object> either;
        if (easing instanceof CubicBezier) {
            CubicBezier bezier = (CubicBezier)easing;
            either = Either.right(bezier);
        } else {
    ...
    implements EasingType {
        public static final Codec<CubicBezier> CODEC = RecordCodecBuilder.create(i -> i.group(((MapCodec)CubicBezierControls.CODEC.fieldOf("cubic_bezier")).forGetter(b -> b.controls)).apply((Applicative<CubicBezier, ?>)i, CubicBezier::new));
