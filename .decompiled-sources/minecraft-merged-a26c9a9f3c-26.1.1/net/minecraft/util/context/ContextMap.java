...
public class ContextMap {
    ...
    public static class Builder {
        ...
        public ContextMap create(ContextKeySet paramSet) {
            Sets.SetView<ContextKey<?>> notAllowed = Sets.difference(this.params.keySet(), paramSet.allowed());
            if (!notAllowed.isEmpty()) {
                throw new IllegalArgumentException("Parameters not allowed in this parameter set: " + String.valueOf(notAllowed));
            }
            Sets.SetView<ContextKey<?>> missingRequired = Sets.difference(paramSet.required(), this.params.keySet());
