...
implements DynamicOps<Tag> {
    ...
    public <U> U convertTo(DynamicOps<U> outOps, Tag input) {
        ...
        switch (SwitchBootstraps.typeSwitch("typeSwitch", new Object[]{EndTag.class, ByteTag.class, ShortTag.class, IntTag.class, LongTag.class, FloatTag.class, DoubleTag.class, ByteArrayTag.class, StringTag.class, ListTag.class, CompoundTag.class, IntArrayTag.class, LongArrayTag.class}, (Tag)tag2, n)) {
            ...
            case 6: {
                ...
                double value6 = d;
                u = outOps.createDouble(value6);
                return u;
            }
            case 7: {
                ByteArrayTag byteArrayTag = (ByteArrayTag)tag2;
                u = outOps.createByteList(ByteBuffer.wrap(byteArrayTag.getAsByteArray()));
                return u;
            }
            case 8: {
                ...
                {
                    String string;
                    String value7 = string = stringTag.value();
                    u = outOps.createString(value7);
                    return u;
                }
            }
            case 9: {
                ListTag listTag = (ListTag)tag2;
                u = this.convertList(outOps, listTag);
                return u;
            }
            case 10: {
                CompoundTag compoundTag = (CompoundTag)tag2;
                u = this.convertMap(outOps, compoundTag);
                return u;
            }
            case 11: {
                IntArrayTag intArrayTag = (IntArrayTag)tag2;
                u = outOps.createIntList(Arrays.stream(intArrayTag.getAsIntArray()));
                return u;
            }
            case 12: 
        }
        LongArrayTag longArrayTag = (LongArrayTag)tag2;
        u = outOps.createLongList(Arrays.stream(longArrayTag.getAsLongArray()));
        return u;
    }
    ...
    public DataResult<Tag> mergeToList(Tag list, Tag value) {
        return NbtOps.createCollector(list).map(collector -> DataResult.success(collector.accept(value).result())).orElseGet(() -> DataResult.error(() -> "mergeToList called with not a list: " + String.valueOf(list), list));
    }

    @Override
    public DataResult<Tag> mergeToList(Tag list, List<Tag> values) {
        return NbtOps.createCollector(list).map(collector -> DataResult.success(collector.acceptAll(values).result())).orElseGet(() -> DataResult.error(() -> "mergeToList called with not a list: " + String.valueOf(list), list));
    }
    ...
    public DataResult<Tag> mergeToMap(Tag map, Tag key, Tag value) {
        ...
        if (!(key instanceof StringTag)) return DataResult.error(() -> "key is not a string: " + String.valueOf(key), map);
    ...
    public DataResult<MapLike<Tag>> getMap(Tag input) {
        if (input instanceof CompoundTag) {
            ...
            return DataResult.success(new MapLike<Tag>(this){
                ...
                public Stream<Pair<Tag, Tag>> entries() {
                    return tag.entrySet().stream().map(entry -> Pair.of(this.this$0.createString((String)entry.getKey()), (Tag)entry.getValue()));
                }
