...
implements Packet<ClientGamePacketListener> {
    ...
    private void write(FriendlyByteBuf output) {
        output.writeCollection(this.entries, (buffer, entry) -> entry.write((FriendlyByteBuf)buffer));
    ...
    private static void validateEntries(List<Entry> entries, BiPredicate<Entry, IntSet> validator) {
        IntOpenHashSet elementsToCheck = new IntOpenHashSet(IntSets.fromTo(0, entries.size()));
    ...
    private static <S> Object2IntMap<CommandNode<S>> enumerateNodes(RootCommandNode<S> root) {
        CommandNode node;
        Object2IntOpenHashMap<CommandNode<S>> nodeToId = new Object2IntOpenHashMap<CommandNode<S>>();
        ArrayDeque queue = new ArrayDeque();
        queue.add(root);
        while ((node = (CommandNode)queue.poll()) != null) {
            if (nodeToId.containsKey(node)) continue;
            int id = nodeToId.size();
            nodeToId.put((CommandNode<S>)node, id);
    ...
    private static <S> List<Entry> createEntries(Object2IntMap<CommandNode<S>> nodeToId, NodeInspector<S> inspector) {
        ObjectArrayList<Entry> result = new ObjectArrayList<Entry>(nodeToId.size());
        result.size(nodeToId.size());
        for (Object2IntMap.Entry entry : Object2IntMaps.fastIterable(nodeToId)) {
            result.set(entry.getIntValue(), ClientboundCommandsPacket.createEntry((CommandNode)entry.getKey(), inspector, nodeToId));
        }
        return result;
    }
    ...
    private static class NodeResolver<S> {
        ...
        private NodeResolver(CommandBuildContext context, NodeBuilder<S> builder, List<Entry> entries) {
            ...
            ObjectArrayList<CommandNode<S>> nodes = new ObjectArrayList<CommandNode<S>>();
