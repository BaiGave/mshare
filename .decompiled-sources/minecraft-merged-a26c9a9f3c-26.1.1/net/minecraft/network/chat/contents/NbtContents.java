...
public record NbtContents(CompilableString<NbtPathArgument.NbtPath> nbtPath, boolean interpreting, boolean plain, Optional<Component> separator, DataSource dataSource) implements ComponentContents
{
    ...
    public static final MapCodec<NbtContents> MAP_CODEC = RecordCodecBuilder.mapCodec(i -> i.group(((MapCodec)NBT_PATH_CODEC.fieldOf("nbt")).forGetter(NbtContents::nbtPath), Codec.BOOL.lenientOptionalFieldOf("interpret", false).forGetter(NbtContents::interpreting), Codec.BOOL.lenientOptionalFieldOf("plain", false).forGetter(NbtContents::plain), ComponentSerialization.CODEC.lenientOptionalFieldOf("separator").forGetter(NbtContents::separator), DataSources.CODEC.forGetter(NbtContents::dataSource)).apply((Applicative<NbtContents, ?>)i, NbtContents::new)).validate(v -> {
