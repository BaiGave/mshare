...
extends ServerBossEvent {
    ...
    public boolean setPlayers(Collection<ServerPlayer> players) {
        ...
        boolean found;
        HashSet<UUID> toRemove = Sets.newHashSet();
        HashSet<ServerPlayer> toAdd = Sets.newHashSet();
    ...
    public record Packed(Component name, boolean visible, int value, int max, BossEvent.BossBarColor color, BossEvent.BossBarOverlay overlay, boolean darkenScreen, boolean playBossMusic, boolean createWorldFog, Set<UUID> players) {
        public static final Codec<Packed> CODEC = RecordCodecBuilder.create(i -> i.group(((MapCodec)ComponentSerialization.CODEC.fieldOf("Name")).forGetter(Packed::name), Codec.BOOL.optionalFieldOf("Visible", false).forGetter(Packed::visible), Codec.INT.optionalFieldOf("Value", 0).forGetter(Packed::value), Codec.INT.optionalFieldOf("Max", 100).forGetter(Packed::max), BossEvent.BossBarColor.CODEC.optionalFieldOf("Color", BossEvent.BossBarColor.WHITE).forGetter(Packed::color), BossEvent.BossBarOverlay.CODEC.optionalFieldOf("Overlay", BossEvent.BossBarOverlay.PROGRESS).forGetter(Packed::overlay), Codec.BOOL.optionalFieldOf("DarkenScreen", false).forGetter(Packed::darkenScreen), Codec.BOOL.optionalFieldOf("PlayBossMusic", false).forGetter(Packed::playBossMusic), Codec.BOOL.optionalFieldOf("CreateWorldFog", false).forGetter(Packed::createWorldFog), UUIDUtil.CODEC_SET.optionalFieldOf("Players", Set.of()).forGetter(Packed::players)).apply((Applicative<Packed, ?>)i, Packed::new));
    }
}
