...
public class AdvancementVisibilityEvaluator {
    ...
    private static boolean evaluateVisiblityForUnfinishedNode(Stack<VisibilityRule> ascendants) {
        for (int i = 0; i <= 2; ++i) {
            VisibilityRule visibility = ascendants.peek(i);
    ...
    public static void evaluateVisibility(AdvancementNode node, Predicate<AdvancementNode> isDone, Output output) {
        ...
        ObjectArrayList<VisibilityRule> visibilityStack = new ObjectArrayList<VisibilityRule>();
        for (int i = 0; i <= 2; ++i) {
            visibilityStack.push(VisibilityRule.NO_CHANGE);
        }
        AdvancementVisibilityEvaluator.evaluateVisibility(root, visibilityStack, isDone, output);
    }
