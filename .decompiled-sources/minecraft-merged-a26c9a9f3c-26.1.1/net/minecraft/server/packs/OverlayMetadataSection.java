...
public record OverlayMetadataSection(List<OverlayEntry> overlays) {
    ...
    public static Codec<OverlayMetadataSection> codecForPackType(PackType packType) {
        return RecordCodecBuilder.create(i -> i.group(((MapCodec)OverlayEntry.listCodecForPackType(packType).fieldOf("entries")).forGetter(OverlayMetadataSection::overlays)).apply((Applicative<OverlayMetadataSection, ?>)i, OverlayMetadataSection::new));
    }
    ...
    public record OverlayEntry(InclusiveRange<PackFormat> format, String overlay) {
        ...
        {
            private static final Codec<IntermediateEntry> CODEC = RecordCodecBuilder.create(i -> i.group(PackFormat.IntermediaryFormat.OVERLAY_CODEC.forGetter(IntermediateEntry::format), ((MapCodec)Codec.STRING.validate(OverlayMetadataSection::validateOverlayDir).fieldOf("directory")).forGetter(IntermediateEntry::overlay)).apply((Applicative<IntermediateEntry, ?>)i, IntermediateEntry::new));
