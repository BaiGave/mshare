...
extends FileSystem {
    ...
    private static LinkFSPath buildPath(DirectoryEntry entry, LinkFileSystem fileSystem, String selfName, @Nullable LinkFSPath parent) {
        Object2ObjectOpenHashMap<String, LinkFSPath> children = new Object2ObjectOpenHashMap<String, LinkFSPath>();
        LinkFSPath result = new LinkFSPath(fileSystem, selfName, parent, new PathContents.DirectoryContents(children));
        entry.files.forEach((name, linkTarget) -> children.put((String)name, new LinkFSPath(fileSystem, (String)name, result, new PathContents.FileContents((Path)linkTarget))));
        entry.children.forEach((name, childEntry) -> children.put((String)name, LinkFileSystem.buildPath(childEntry, fileSystem, name, result)));
        children.trim();
        return result;
    }
