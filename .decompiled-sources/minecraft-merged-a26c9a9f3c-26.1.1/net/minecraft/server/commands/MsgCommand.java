...
public class MsgCommand {
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        LiteralCommandNode<CommandSourceStack> msg = dispatcher.register((LiteralArgumentBuilder)Commands.literal("msg").then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("targets", EntityArgument.players()).then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("message", MessageArgument.message()).executes(c -> {
            Collection<ServerPlayer> players = EntityArgument.getPlayers(c, "targets");
            if (!players.isEmpty()) {
                MessageArgument.resolveChatMessage(c, "message", message -> MsgCommand.sendMessage((CommandSourceStack)c.getSource(), players, message));
            }
            return players.size();
        }))));
