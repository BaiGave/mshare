...
public class TransferCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("transfer").requires(Commands.hasPermission(Commands.LEVEL_ADMINS))).then(((RequiredArgumentBuilder)Commands.argument("hostname", StringArgumentType.string()).executes(c -> TransferCommand.transfer((CommandSourceStack)c.getSource(), StringArgumentType.getString(c, "hostname"), 25565, List.of(((CommandSourceStack)c.getSource()).getPlayerOrException())))).then(((RequiredArgumentBuilder)Commands.argument("port", IntegerArgumentType.integer(1, 65535)).executes(c -> TransferCommand.transfer((CommandSourceStack)c.getSource(), StringArgumentType.getString(c, "hostname"), IntegerArgumentType.getInteger(c, "port"), List.of(((CommandSourceStack)c.getSource()).getPlayerOrException())))).then(Commands.argument("players", EntityArgument.players()).executes(c -> TransferCommand.transfer((CommandSourceStack)c.getSource(), StringArgumentType.getString(c, "hostname"), IntegerArgumentType.getInteger(c, "port"), EntityArgument.getPlayers(c, "players")))))));
    }
