...
public class SpreadPlayersCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("spreadplayers").requires(Commands.hasPermission(Commands.LEVEL_GAMEMASTERS))).then(Commands.argument("center", Vec2Argument.vec2()).then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("spreadDistance", FloatArgumentType.floatArg(0.0f)).then((ArgumentBuilder<CommandSourceStack, ?>)((RequiredArgumentBuilder)Commands.argument("maxRange", FloatArgumentType.floatArg(1.0f)).then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("respectTeams", BoolArgumentType.bool()).then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("targets", EntityArgument.entities()).executes(c -> SpreadPlayersCommand.spreadPlayers((CommandSourceStack)c.getSource(), Vec2Argument.getVec2(c, "center"), FloatArgumentType.getFloat(c, "spreadDistance"), FloatArgumentType.getFloat(c, "maxRange"), ((CommandSourceStack)c.getSource()).getLevel().getMaxY() + 1, BoolArgumentType.getBool(c, "respectTeams"), EntityArgument.getEntities(c, "targets")))))).then(Commands.literal("under").then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("maxHeight", IntegerArgumentType.integer()).then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("respectTeams", BoolArgumentType.bool()).then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("targets", EntityArgument.entities()).executes(c -> SpreadPlayersCommand.spreadPlayers((CommandSourceStack)c.getSource(), Vec2Argument.getVec2(c, "center"), FloatArgumentType.getFloat(c, "spreadDistance"), FloatArgumentType.getFloat(c, "maxRange"), IntegerArgumentType.getInteger(c, "maxHeight"), BoolArgumentType.getBool(c, "respectTeams"), EntityArgument.getEntities(c, "targets")))))))))));
    }
    ...
    private static int getNumberOfTeams(Collection<? extends Entity> players) {
        HashSet<PlayerTeam> teams = Sets.newHashSet();
    ...
    private static void spreadPositions(Vec2 center, double spreadDist, ServerLevel level, RandomSource random, double minX, double minZ, double maxX, double maxZ, int maxHeight, Position[] positions, boolean respectTeams) throws CommandSyntaxException {
        ...
        if (iteration >= 10000) {
            if (respectTeams) {
                throw ERROR_FAILED_TO_SPREAD_TEAMS.create(positions.length, Float.valueOf(center.x), Float.valueOf(center.y), String.format(Locale.ROOT, "%.2f", minDistance));
            }
            throw ERROR_FAILED_TO_SPREAD_ENTITIES.create(positions.length, Float.valueOf(center.x), Float.valueOf(center.y), String.format(Locale.ROOT, "%.2f", minDistance));
        }
    }
    ...
    private static double setPlayerPositions(Collection<? extends Entity> entities, ServerLevel level, Position[] positions, int maxHeight, boolean respectTeams) {
        ...
        int positionIndex = 0;
        HashMap<PlayerTeam, Position> teamPositions = Maps.newHashMap();
