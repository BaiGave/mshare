...
public class StopwatchCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("stopwatch").requires(Commands.hasPermission(Commands.LEVEL_GAMEMASTERS))).then(Commands.literal("create").then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("id", IdentifierArgument.id()).executes(c -> StopwatchCommand.createStopwatch((CommandSourceStack)c.getSource(), IdentifierArgument.getId(c, "id")))))).then(Commands.literal("query").then((ArgumentBuilder<CommandSourceStack, ?>)((RequiredArgumentBuilder)Commands.argument("id", IdentifierArgument.id()).suggests(SUGGEST_STOPWATCHES).then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("scale", DoubleArgumentType.doubleArg()).executes(c -> StopwatchCommand.queryStopwatch((CommandSourceStack)c.getSource(), IdentifierArgument.getId(c, "id"), DoubleArgumentType.getDouble(c, "scale"))))).executes(c -> StopwatchCommand.queryStopwatch((CommandSourceStack)c.getSource(), IdentifierArgument.getId(c, "id"), 1.0))))).then(Commands.literal("restart").then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("id", IdentifierArgument.id()).suggests(SUGGEST_STOPWATCHES).executes(c -> StopwatchCommand.restartStopwatch((CommandSourceStack)c.getSource(), IdentifierArgument.getId(c, "id")))))).then(Commands.literal("remove").then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("id", IdentifierArgument.id()).suggests(SUGGEST_STOPWATCHES).executes(c -> StopwatchCommand.removeStopwatch((CommandSourceStack)c.getSource(), IdentifierArgument.getId(c, "id"))))));
    }
