...
public class TimeCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher, CommandBuildContext context) {
        ...
        dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)baseCommand.then(Commands.literal("query").then((ArgumentBuilder<CommandSourceStack, ?>)Commands.literal("gametime").executes(c -> TimeCommand.queryGameTime((CommandSourceStack)c.getSource()))))).then(Commands.literal("of").then(TimeCommand.addClockNodes(context, Commands.argument("clock", ResourceArgument.resource(context, Registries.WORLD_CLOCK)), c -> ResourceArgument.getClock(c, "clock")))));
    }
    ...
    private static <A extends ArgumentBuilder<CommandSourceStack, A>> A addClockNodes(CommandBuildContext context, A node, ClockGetter clockGetter) {
        return ((ArgumentBuilder)((ArgumentBuilder)((ArgumentBuilder)((ArgumentBuilder)((ArgumentBuilder)node.then(((LiteralArgumentBuilder)Commands.literal("set").then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("time", TimeArgument.time()).executes(c -> TimeCommand.setTotalTicks((CommandSourceStack)c.getSource(), clockGetter.getClock(c), IntegerArgumentType.getInteger(c, "time"))))).then(Commands.argument("timemarker", IdentifierArgument.id()).suggests((c, p) -> TimeCommand.suggestTimeMarkers((CommandSourceStack)c.getSource(), p, clockGetter.getClock(c))).executes(c -> TimeCommand.setTimeToTimeMarker((CommandSourceStack)c.getSource(), clockGetter.getClock(c), ResourceKey.create(ClockTimeMarkers.ROOT_ID, IdentifierArgument.getId(c, "timemarker"))))))).then(Commands.literal("add").then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("time", TimeArgument.time(Integer.MIN_VALUE)).executes(c -> TimeCommand.addTime((CommandSourceStack)c.getSource(), clockGetter.getClock(c), IntegerArgumentType.getInteger(c, "time")))))).then(Commands.literal("pause").executes(c -> TimeCommand.setPaused((CommandSourceStack)c.getSource(), clockGetter.getClock(c), true)))).then(Commands.literal("resume").executes(c -> TimeCommand.setPaused((CommandSourceStack)c.getSource(), clockGetter.getClock(c), false)))).then(Commands.literal("rate").then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("rate", FloatArgumentType.floatArg(1.0E-5f, 1000.0f)).executes(c -> TimeCommand.setRate((CommandSourceStack)c.getSource(), clockGetter.getClock(c), FloatArgumentType.getFloat(c, "rate")))))).then(((LiteralArgumentBuilder)Commands.literal("query").then((ArgumentBuilder<CommandSourceStack, ?>)Commands.literal("time").executes(c -> TimeCommand.queryTime((CommandSourceStack)c.getSource(), clockGetter.getClock(c))))).then(((RequiredArgumentBuilder)Commands.argument("timeline", ResourceArgument.resource(context, Registries.TIMELINE)).suggests((c, p) -> TimeCommand.suggestTimelines((CommandSourceStack)c.getSource(), p, clockGetter.getClock(c))).executes(c -> TimeCommand.queryTimelineTicks((CommandSourceStack)c.getSource(), clockGetter.getClock(c), ResourceArgument.getTimeline(c, "timeline")))).then(Commands.literal("repetition").executes(c -> TimeCommand.queryTimelineRepetitions((CommandSourceStack)c.getSource(), clockGetter.getClock(c), ResourceArgument.getTimeline(c, "timeline"))))));
    }
