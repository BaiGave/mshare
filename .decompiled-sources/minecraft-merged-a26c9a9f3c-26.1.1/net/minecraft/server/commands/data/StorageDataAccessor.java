...
implements DataAccessor {
    private static final SuggestionProvider<CommandSourceStack> SUGGEST_STORAGE = (c, p) -> SharedSuggestionProvider.suggestResource(StorageDataAccessor.getGlobalTags(c).keys(), p);
    public static final Function<String, DataCommands.DataProvider> PROVIDER = arg -> new DataCommands.DataProvider((String)arg){
        ...
        public ArgumentBuilder<CommandSourceStack, ?> wrap(ArgumentBuilder<CommandSourceStack, ?> parent, Function<ArgumentBuilder<CommandSourceStack, ?>, ArgumentBuilder<CommandSourceStack, ?>> function) {
            return parent.then((ArgumentBuilder<CommandSourceStack, ?>)Commands.literal("storage").then(function.apply(Commands.argument(this.val$arg, IdentifierArgument.id()).suggests(SUGGEST_STORAGE))));
        }
    };
