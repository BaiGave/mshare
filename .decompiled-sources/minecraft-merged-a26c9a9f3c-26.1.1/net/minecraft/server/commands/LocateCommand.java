...
public class LocateCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher, CommandBuildContext context) {
        dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("locate").requires(Commands.hasPermission(Commands.LEVEL_GAMEMASTERS))).then(Commands.literal("structure").then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("structure", ResourceOrTagKeyArgument.resourceOrTagKey(Registries.STRUCTURE)).executes(c -> LocateCommand.locateStructure((CommandSourceStack)c.getSource(), ResourceOrTagKeyArgument.getResourceOrTagKey(c, "structure", Registries.STRUCTURE, ERROR_STRUCTURE_INVALID)))))).then(Commands.literal("biome").then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("biome", ResourceOrTagArgument.resourceOrTag(context, Registries.BIOME)).executes(c -> LocateCommand.locateBiome((CommandSourceStack)c.getSource(), ResourceOrTagArgument.getResourceOrTag(c, "biome", Registries.BIOME)))))).then(Commands.literal("poi").then((ArgumentBuilder<CommandSourceStack, ?>)Commands.argument("poi", ResourceOrTagArgument.resourceOrTag(context, Registries.POINT_OF_INTEREST_TYPE)).executes(c -> LocateCommand.locatePoi((CommandSourceStack)c.getSource(), ResourceOrTagArgument.getResourceOrTag(c, "poi", Registries.POINT_OF_INTEREST_TYPE))))));
    }
    ...
    private static int locateStructure(CommandSourceStack source, ResourceOrTagKeyArgument.Result<Structure> resourceOrTag) throws CommandSyntaxException {
        ...
        HolderSet target = LocateCommand.getHolders(resourceOrTag, (Registry<Structure>)registry).orElseThrow(() -> ERROR_STRUCTURE_INVALID.create(resourceOrTag.asPrintable()));
    ...
    public static int showLocateResult(CommandSourceStack source, ResourceOrTagArgument.Result<?> name, BlockPos sourcePos, Pair<BlockPos, ? extends Holder<?>> found, String successMessageKey, boolean includeY, Duration taskDuration) {
        String foundName = name.unwrap().map(element -> name.asPrintable(), tag -> name.asPrintable() + " (" + ((Holder)found.getSecond()).getRegisteredName() + ")");
    ...
    public static int showLocateResult(CommandSourceStack source, ResourceOrTagKeyArgument.Result<?> name, BlockPos sourcePos, Pair<BlockPos, ? extends Holder<?>> found, String successMessageKey, boolean includeY, Duration taskDuration) {
        String foundName = name.unwrap().map(element -> element.identifier().toString(), tag -> "#" + String.valueOf(tag.location()) + " (" + ((Holder)found.getSecond()).getRegisteredName() + ")");
        return LocateCommand.showLocateResult(source, sourcePos, found, successMessageKey, includeY, foundName, taskDuration);
    }
