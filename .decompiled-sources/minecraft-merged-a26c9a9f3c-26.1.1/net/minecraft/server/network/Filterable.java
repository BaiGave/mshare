...
public record Filterable<T>(T raw, Optional<T> filtered) {
    public static <T> Codec<Filterable<T>> codec(Codec<T> valueCodec) {
        Codec fullCodec = RecordCodecBuilder.create(i -> i.group(((MapCodec)valueCodec.fieldOf("raw")).forGetter(Filterable::raw), valueCodec.optionalFieldOf("filtered").forGetter(Filterable::filtered)).apply((Applicative<Filterable, ?>)i, Filterable::new));
        Codec<Filterable> simpleCodec = valueCodec.xmap(Filterable::passThrough, Filterable::raw);
