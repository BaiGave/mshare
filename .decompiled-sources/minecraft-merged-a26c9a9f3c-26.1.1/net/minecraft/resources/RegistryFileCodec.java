...
implements Codec<Holder<E>> {
    ...
    public <T> DataResult<T> encode(Holder<E> input, DynamicOps<T> ops, T prefix) {
        ...
        if (ops instanceof RegistryOps && (maybeOwner = (registryOps = (RegistryOps)ops).owner(this.registryKey)).isPresent()) {
            ...
            return input.unwrap().map(id -> Identifier.CODEC.encode(id.identifier(), ops, prefix), value -> this.elementCodec.encode(value, ops, prefix));
        }
    ...
    public <T> DataResult<Pair<Holder<E>, T>> decode(DynamicOps<T> ops, T input) {
        if (ops instanceof RegistryOps) {
            ...
            if (decoded.result().isEmpty()) {
                ...
                return this.elementCodec.decode(ops, input).map((? super R p) -> p.mapFirst(Holder::direct));
            }
            Pair pair = decoded.result().get();
            ResourceKey elementKey = ResourceKey.create(this.registryKey, (Identifier)pair.getFirst());
            return lookup.get(elementKey).map(DataResult::success).orElseGet(() -> DataResult.error(() -> "Failed to get element " + String.valueOf(elementKey))).map((? super R h) -> Pair.of(h, pair.getSecond())).setLifecycle(Lifecycle.stable());
        }
        return this.elementCodec.decode(ops, input).map((? super R p) -> p.mapFirst(Holder::direct));
    }
