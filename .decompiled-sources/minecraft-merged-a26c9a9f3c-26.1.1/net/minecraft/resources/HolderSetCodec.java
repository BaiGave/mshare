...
implements Codec<HolderSet<E>> {
    ...
    public <T> DataResult<Pair<HolderSet<E>, T>> decode(DynamicOps<T> ops, T input) {
        ...
        if (ops instanceof RegistryOps && (registryOptional = (registryOps = (RegistryOps)ops).getter(this.registryKey)).isPresent()) {
            ...
            return this.registryAwareCodec.decode(ops, input).flatMap((? super R p) -> {
                DataResult result = ((Either)p.getFirst()).map(tag -> HolderSetCodec.lookupTag(registry, tag), values -> DataResult.success(HolderSet.direct(values)));
                return result.map((? super R holders) -> Pair.of(holders, p.getSecond()));
            });
        }
    ...
    private <T> DataResult<Pair<HolderSet<E>, T>> decodeWithoutRegistry(DynamicOps<T> ops, T input) {
        return this.elementCodec.listOf().decode(ops, input).flatMap((? super R p) -> {
