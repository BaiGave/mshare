...
implements AutoCloseable {
    ...
    public NativeImage(Format format, int width, int height, boolean zero) {
        ...
        this.pixels = zero ? MemoryUtil.nmemCalloc(1L, this.size) : MemoryUtil.nmemAlloc(this.size);
    ...
    public boolean copyFromFont(FT_Face face, int index) {
        ...
        if (FreeTypeUtil.checkError(FreeType.FT_Load_Glyph(face, index, 4), "Loading glyph")) {
            return false;
        }
    ...
    public void copyFrom(NativeImage from) {
        ...
        if (this.width == from.width) {
            ...
            for (int y = 0; y < minHeight; ++y) {
                ...
                MemoryUtil.memCopy(from.pixels + (long)fromOffset, this.pixels + (long)toOffset, minWidth);
            }
        }
    }
    ...
    extends STBIWriteCallback {
        ...
        public void invoke(long context, long data, int size) {
            ByteBuffer dataBuf = WriteCallback.getData(data, size);
            try {
