...
implements AutoCloseable {
    ...
    private CompletableFuture<UploadResult> requestUpload(int currentAttempt, long fileSize) {
        HttpRequest.BodyPublisher publisher = FileUpload.inputStreamPublisherWithSize(() -> {
            try {
                return new UploadCountingInputStream((InputStream)new FileInputStream(this.file), this.uploadStatus);
            }
            catch (IOException e) {
    ...
    private UploadResult handleResponse(HttpResponse<String> response) {
        ...
        if (statusCode == 401) {
            LOGGER.debug("Realms server returned 401: {}", (Object)response.headers().firstValue("WWW-Authenticate"));
        }
