...
extends AbstractOSProcess {
    ...
    public List<OSThread> getThreadDetails() {
        ...
        return ExecutingCommand.runNative(psCommand).stream().skip(1L).map(thread -> ParseUtil.stringToEnumMap(PsThreadColumns.class, thread.trim(), ' ')).filter(hasColumnsArgs).map(threadMap -> new OpenBsdOSThread(this.getProcessID(), (Map<PsThreadColumns, String>)threadMap)).filter(OSThread.ThreadFiltering.VALID_THREAD).collect(Collectors.toList());
    }
