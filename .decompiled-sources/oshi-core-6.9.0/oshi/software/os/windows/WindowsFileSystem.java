...
extends AbstractFileSystem {
        ...
        try {
            ...
            try {
                do {
                    ...
                    fs.add(new WindowsOSFileStore(String.format(Locale.ROOT, "%s (%s)", strName, strMount), volume, strName, strMount, options.toString(), uuid, "", WindowsFileSystem.getDriveType(strMount), strFsType, systemFreeBytes.getValue(), userFreeBytes.getValue(), totalBytes.getValue(), 0L, 0L));
    ...
    static List<OSFileStore> getWmiVolumes(String nameToMatch, boolean localOnly) {
        ...
        for (int i = 0; i < drives.getResultCount(); ++i) {
            ...
            fs.add(new WindowsOSFileStore(String.format(Locale.ROOT, "%s (%s)", description, name), volume, label, name + "\\", options, "", "", WindowsFileSystem.getDriveType(name), WmiUtil.getString(drives, Win32LogicalDisk.LogicalDiskProperty.FILESYSTEM, i), free, free, total, 0L, 0L));
        }
        return fs;
    }
