...
Map<Float, Integer> {
    ...
    default public void forEach(FloatIntBiConsumer consumer) {
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Float, ? super Integer> action) {
    ...
    public static Float2IntMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Float2IntMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Float2IntMaps.singleton(entries[0].getFloatKey(), entries[0].getIntValue());
        }
        if (entries.length <= 8) {
            ...
            return Float2IntMaps.unmodifiable(new Float2IntArrayMap(keys, vals, entries.length));
        }
        Float2IntOpenHashMap newMap = new Float2IntOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getFloatKey(), entry.getIntValue()) == 0) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getFloatKey());
        }
        return Float2IntMaps.unmodifiable(newMap);
    }

    public static Entry entry(float key, int value) {
