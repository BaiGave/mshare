...
Size64 {
    ...
    public boolean add(float k) {
        if (Float.floatToRawIntBits(k) == 0) {
            ...
            if (Float.floatToRawIntBits(curr) != 0) {
                ...
                while (Float.floatToRawIntBits(curr = key[base = base + ((displ = displ + 1 & this.segmentMask) == 0 ? 1 : 0) & this.baseMask][displ]) != 0) {
    ...
    protected final void shiftKeys(long pos) {
        ...
        while (true) {
            ...
            while (true) {
                if (Float.floatToRawIntBits(BigArrays.get(key, pos)) == 0) {
    ...
    public boolean remove(float k) {
        ...
        do {
            if (Float.floatToRawIntBits(curr = key[base = base + ((displ = displ + 1 & this.segmentMask) == 0 ? 1 : 0) & this.baseMask][displ]) != 0) continue;
            return false;
        } while (Float.floatToRawIntBits(curr) != Float.floatToRawIntBits(k));
        return this.removeEntry(base, displ);
    }
    ...
    public void forEach(FloatConsumer action) {
        ...
        while (pos < max) {
            float gotten;
            if (Float.floatToRawIntBits(gotten = BigArrays.get(key, pos++)) == 0) continue;
    ...
    protected void rehash(long newN) {
        ...
        while (i-- != 0L) {
            int d;
            while (Float.floatToRawIntBits(key[base][displ]) == 0) {
                base += (displ = displ + 1 & this.segmentMask) == 0 ? 1 : 0;
            }
            float k = key[base][displ];
            long h = HashCommon.mix((long)HashCommon.float2int(k));
            int b = (int)((h & mask) >>> 27);
            if (Float.floatToRawIntBits(newKey[b][d = (int)(h & (long)newSegmentMask)]) != 0) {
                while (Float.floatToRawIntBits(newKey[b = b + ((d = d + 1 & newSegmentMask) == 0 ? 1 : 0) & newBaseMask][d]) != 0) {
                }
            }
            newKey[b][d] = k;
            base += (displ = displ + 1 & this.segmentMask) == 0 ? 1 : 0;
    ...
    private void readObject(ObjectInputStream s) throws IOException, ClassNotFoundException {
        ...
        while (i-- != 0L) {
            ...
            long h = HashCommon.mix((long)HashCommon.float2int(k));
            int base = (int)((h & this.mask) >>> 27);
            if (Float.floatToRawIntBits(key[base][displ = (int)(h & (long)this.segmentMask)]) != 0) {
                while (Float.floatToRawIntBits(key[base = base + ((displ = displ + 1 & this.segmentMask) == 0 ? 1 : 0) & this.baseMask][displ]) != 0) {
                }
            }
            key[base][displ] = k;
        }
    ...
    implements FloatIterator {
        ...
        public float nextFloat() {
            ...
            } while (Float.floatToRawIntBits(k = key[this.base][this.displ]) == 0);
        ...
        private final void shiftKeys(long pos) {
            ...
            while (true) {
                ...
                while (true) {
                    if (Float.floatToRawIntBits(curr = BigArrays.get(key, pos)) == 0) {
                        BigArrays.set(key, last, 0.0f);
                        return;
                    }
    ...
    implements FloatSpliterator {
        ...
        public boolean tryAdvance(FloatConsumer action) {
            ...
            while (this.pos < this.max) {
                ...
                if (Float.floatToRawIntBits(gotten) != 0) {
                    ++this.c;
                    ++this.pos;
                    action.accept(gotten);
                    return true;
        ...
        public long skip(long n) {
            ...
            while (this.pos < this.max && n > 0L) {
                if (Float.floatToRawIntBits(BigArrays.get(key, this.pos++)) == 0) continue;
                ++skipped;
                --n;
            }
            return skipped;
