...
Map<Float, Boolean> {
    ...
    default public void forEach(FloatBooleanBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.float2BooleanEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getFloatKey(), entry.getBooleanValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Float, ? super Boolean> action) {
    ...
    public static Float2BooleanMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Float2BooleanMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Float2BooleanMaps.singleton(entries[0].getFloatKey(), entries[0].getBooleanValue());
        }
        if (entries.length <= 8) {
            ...
            return Float2BooleanMaps.unmodifiable(new Float2BooleanArrayMap(keys, vals, entries.length));
        }
        Float2BooleanOpenHashMap newMap = new Float2BooleanOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (!newMap.put(entry.getFloatKey(), entry.getBooleanValue())) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getFloatKey());
        }
        return Float2BooleanMaps.unmodifiable(newMap);
    }

    public static Entry entry(float key, boolean value) {
