...
Map<Float, Float> {
    ...
    default public void forEach(FloatFloatBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.float2FloatEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getFloatKey(), entry.getFloatValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Float, ? super Float> action) {
    ...
    public static Float2FloatMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Float2FloatMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Float2FloatMaps.singleton(entries[0].getFloatKey(), entries[0].getFloatValue());
        }
        if (entries.length <= 8) {
            ...
            return Float2FloatMaps.unmodifiable(new Float2FloatArrayMap(keys, vals, entries.length));
        }
        Float2FloatOpenHashMap newMap = new Float2FloatOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (Float.floatToRawIntBits(newMap.put(entry.getFloatKey(), entry.getFloatValue())) == 0) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getFloatKey());
        }
        return Float2FloatMaps.unmodifiable(newMap);
    }

    public static Entry entry(float key, float value) {
