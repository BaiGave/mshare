...
Map<Float, Character> {
    ...
    default public void forEach(FloatCharBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.float2CharEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getFloatKey(), entry.getCharValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Float, ? super Character> action) {
    ...
    public static Float2CharMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Float2CharMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Float2CharMaps.singleton(entries[0].getFloatKey(), entries[0].getCharValue());
        }
        if (entries.length <= 8) {
            ...
            return Float2CharMaps.unmodifiable(new Float2CharArrayMap(keys, vals, entries.length));
        }
        Float2CharOpenHashMap newMap = new Float2CharOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getFloatKey(), entry.getCharValue()) == '\u0000') continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getFloatKey());
        }
        return Float2CharMaps.unmodifiable(newMap);
    }

    public static Entry entry(float key, char value) {
