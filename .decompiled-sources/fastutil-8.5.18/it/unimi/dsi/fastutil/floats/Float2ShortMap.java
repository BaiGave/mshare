...
Map<Float, Short> {
    ...
    default public void forEach(FloatShortBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.float2ShortEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getFloatKey(), entry.getShortValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Float, ? super Short> action) {
    ...
    public static Float2ShortMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Float2ShortMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Float2ShortMaps.singleton(entries[0].getFloatKey(), entries[0].getShortValue());
        }
        if (entries.length <= 8) {
            ...
            return Float2ShortMaps.unmodifiable(new Float2ShortArrayMap(keys, vals, entries.length));
        }
        Float2ShortOpenHashMap newMap = new Float2ShortOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getFloatKey(), entry.getShortValue()) == 0) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getFloatKey());
        }
        return Float2ShortMaps.unmodifiable(newMap);
    }

    public static Entry entry(float key, short value) {
