...
Map<Float, Double> {
    ...
    default public void forEach(FloatDoubleBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.float2DoubleEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getFloatKey(), entry.getDoubleValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Float, ? super Double> action) {
    ...
    public static Float2DoubleMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Float2DoubleMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Float2DoubleMaps.singleton(entries[0].getFloatKey(), entries[0].getDoubleValue());
        }
        if (entries.length <= 8) {
            ...
            return Float2DoubleMaps.unmodifiable(new Float2DoubleArrayMap(keys, vals, entries.length));
        }
        Float2DoubleOpenHashMap newMap = new Float2DoubleOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (Double.doubleToRawLongBits(newMap.put(entry.getFloatKey(), entry.getDoubleValue())) == 0L) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getFloatKey());
        }
        return Float2DoubleMaps.unmodifiable(newMap);
    }

    public static Entry entry(float key, double value) {
