...
Map<Float, Long> {
    ...
    default public void forEach(FloatLongBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.float2LongEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getFloatKey(), entry.getLongValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Float, ? super Long> action) {
    ...
    public static Float2LongMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Float2LongMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Float2LongMaps.singleton(entries[0].getFloatKey(), entries[0].getLongValue());
        }
        if (entries.length <= 8) {
            ...
            return Float2LongMaps.unmodifiable(new Float2LongArrayMap(keys, vals, entries.length));
        }
        Float2LongOpenHashMap newMap = new Float2LongOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getFloatKey(), entry.getLongValue()) == 0L) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getFloatKey());
        }
        return Float2LongMaps.unmodifiable(newMap);
    }

    public static Entry entry(float key, long value) {
