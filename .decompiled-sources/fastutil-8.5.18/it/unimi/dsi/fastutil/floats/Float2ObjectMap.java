...
Map<Float, V> {
    ...
    @SafeVarargs
    public static <V> Float2ObjectMap<V> ofEntries(Entry<V> ... entries) {
        if (entries.length == 0) {
            return Float2ObjectMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Float2ObjectMaps.singleton(entries[0].getFloatKey(), entries[0].getValue());
        }
        if (entries.length <= 8) {
            ...
            return Float2ObjectMaps.unmodifiable(new Float2ObjectArrayMap(keys, vals, entries.length));
        }
        Float2ObjectOpenHashMap newMap = new Float2ObjectOpenHashMap(entries.length, 0.75f);
        ...
        return Float2ObjectMaps.unmodifiable(newMap);
    }

    public static <V> Entry<V> entry(float key, V value) {
        return new AbstractFloat2ObjectMap.BasicEntry<V>(key, value);
    }
