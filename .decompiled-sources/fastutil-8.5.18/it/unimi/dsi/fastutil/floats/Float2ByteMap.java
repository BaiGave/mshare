...
Map<Float, Byte> {
    ...
    default public void forEach(FloatByteBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.float2ByteEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getFloatKey(), entry.getByteValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Float, ? super Byte> action) {
    ...
    public static Float2ByteMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Float2ByteMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Float2ByteMaps.singleton(entries[0].getFloatKey(), entries[0].getByteValue());
        }
        if (entries.length <= 8) {
            ...
            return Float2ByteMaps.unmodifiable(new Float2ByteArrayMap(keys, vals, entries.length));
        }
        Float2ByteOpenHashMap newMap = new Float2ByteOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getFloatKey(), entry.getByteValue()) == 0) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getFloatKey());
        }
        return Float2ByteMaps.unmodifiable(newMap);
    }

    public static Entry entry(float key, byte value) {
