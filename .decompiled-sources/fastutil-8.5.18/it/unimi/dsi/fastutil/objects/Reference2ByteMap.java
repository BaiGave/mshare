...
import it.unimi.dsi.fastutil.objects.ObjectByteBiConsumer;
...
Map<K, Byte> {
    ...
    default public void forEach(ObjectByteBiConsumer<? super K> consumer) {
        ObjectSet<Entry<K>> entrySet = this.reference2ByteEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept((Object)entry.getKey(), entry.getByteValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super K, ? super Byte> action) {
    ...
    @SafeVarargs
    public static <K> Reference2ByteMap<K> ofEntries(Entry<K> ... entries) {
        if (entries.length == 0) {
            return Reference2ByteMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Reference2ByteMaps.singleton(entries[0].getKey(), entries[0].getByteValue());
        }
        if (entries.length <= 8) {
            ...
            return Reference2ByteMaps.unmodifiable(new Reference2ByteArrayMap(keys, vals, entries.length));
        }
        Reference2ByteOpenHashMap newMap = new Reference2ByteOpenHashMap(entries.length, 0.75f);
        for (Entry<K> entry : entries) {
            if (newMap.put(entry.getKey(), entry.getByteValue()) == 0) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getKey());
        }
        return Reference2ByteMaps.unmodifiable(newMap);
    }

    public static <K> Entry<K> entry(K key, byte value) {
        return new AbstractReference2ByteMap.BasicEntry<K>(key, value);
    }
