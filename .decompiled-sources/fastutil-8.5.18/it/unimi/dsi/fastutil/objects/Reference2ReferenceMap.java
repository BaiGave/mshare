...
Map<K, V> {
    ...
    @SafeVarargs
    public static <K, V> Reference2ReferenceMap<K, V> ofEntries(Entry<K, V> ... entries) {
        if (entries.length == 0) {
            return Reference2ReferenceMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Reference2ReferenceMaps.singleton(entries[0].getKey(), entries[0].getValue());
        }
        if (entries.length <= 8) {
            ...
            return Reference2ReferenceMaps.unmodifiable(new Reference2ReferenceArrayMap(keys, vals, entries.length));
        }
        Reference2ReferenceOpenHashMap newMap = new Reference2ReferenceOpenHashMap(entries.length, 0.75f);
        ...
        return Reference2ReferenceMaps.unmodifiable(newMap);
    }

    public static <K, V> Entry<K, V> entry(K key, V value) {
        return new AbstractReference2ReferenceMap.BasicEntry<K, V>(key, value);
    }
