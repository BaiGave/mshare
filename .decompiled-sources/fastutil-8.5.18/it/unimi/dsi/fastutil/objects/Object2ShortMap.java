...
Map<K, Short> {
    ...
    @SafeVarargs
    public static <K> Object2ShortMap<K> ofEntries(Entry<K> ... entries) {
        if (entries.length == 0) {
            return Object2ShortMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Object2ShortMaps.singleton(entries[0].getKey(), entries[0].getShortValue());
        }
        if (entries.length <= 8) {
            ...
            return Object2ShortMaps.unmodifiable(new Object2ShortArrayMap(keys, vals, entries.length));
        }
        Object2ShortOpenHashMap newMap = new Object2ShortOpenHashMap(entries.length, 0.75f);
        ...
        return Object2ShortMaps.unmodifiable(newMap);
    }

    public static <K> Entry<K> entry(K key, short value) {
        return new AbstractObject2ShortMap.BasicEntry<K>(key, value);
    }
