...
Hash {
    ...
    @Override
    public byte mergeByte(K k, byte v, ByteBinaryOperator remappingFunction) {
        byte newValue;
        Objects.requireNonNull(remappingFunction);
        int pos = this.find(k);
        if (pos < 0) {
    ...
    @Override
    public void forEach(ObjectByteBiConsumer<? super K> consumer) {
        if (this.containsNullKey) {
        ...
        if (this.values == null) {
            this.values = new AbstractByteCollection(){
                ...
                public void forEach(ByteConsumer consumer) {
                    ...
                    byte[] value = Reference2ByteOpenCustomHashMap.this.value;
                    if (Reference2ByteOpenCustomHashMap.this.containsNullKey) {
                        consumer.accept(value[Reference2ByteOpenCustomHashMap.this.n]);
                    }
    ...
    extends AbstractReferenceSet<K> {
        ...
        public void forEach(Consumer<? super K> consumer) {
            K[] key = Reference2ByteOpenCustomHashMap.this.key;
            if (Reference2ByteOpenCustomHashMap.this.containsNullKey) {
                consumer.accept(key[Reference2ByteOpenCustomHashMap.this.n]);
            }
            int pos = Reference2ByteOpenCustomHashMap.this.n;
            while (pos-- != 0) {
                Object k = key[pos];
