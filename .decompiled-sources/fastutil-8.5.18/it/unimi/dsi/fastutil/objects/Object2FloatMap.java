...
Map<K, Float> {
    ...
    @SafeVarargs
    public static <K> Object2FloatMap<K> ofEntries(Entry<K> ... entries) {
        if (entries.length == 0) {
            return Object2FloatMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Object2FloatMaps.singleton(entries[0].getKey(), entries[0].getFloatValue());
        }
        if (entries.length <= 8) {
            ...
            return Object2FloatMaps.unmodifiable(new Object2FloatArrayMap(keys, vals, entries.length));
        }
        Object2FloatOpenHashMap newMap = new Object2FloatOpenHashMap(entries.length, 0.75f);
        ...
        return Object2FloatMaps.unmodifiable(newMap);
    }

    public static <K> Entry<K> entry(K key, float value) {
        return new AbstractObject2FloatMap.BasicEntry<K>(key, value);
    }
