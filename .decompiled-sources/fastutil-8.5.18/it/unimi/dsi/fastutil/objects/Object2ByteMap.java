...
Map<K, Byte> {
    ...
    @SafeVarargs
    public static <K> Object2ByteMap<K> ofEntries(Entry<K> ... entries) {
        if (entries.length == 0) {
            return Object2ByteMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Object2ByteMaps.singleton(entries[0].getKey(), entries[0].getByteValue());
        }
        if (entries.length <= 8) {
            ...
            return Object2ByteMaps.unmodifiable(new Object2ByteArrayMap(keys, vals, entries.length));
        }
        Object2ByteOpenHashMap newMap = new Object2ByteOpenHashMap(entries.length, 0.75f);
        ...
        return Object2ByteMaps.unmodifiable(newMap);
    }

    public static <K> Entry<K> entry(K key, byte value) {
        return new AbstractObject2ByteMap.BasicEntry<K>(key, value);
    }
