...
Map<K, Character> {
    ...
    @SafeVarargs
    public static <K> Object2CharMap<K> ofEntries(Entry<K> ... entries) {
        if (entries.length == 0) {
            return Object2CharMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Object2CharMaps.singleton(entries[0].getKey(), entries[0].getCharValue());
        }
        if (entries.length <= 8) {
            ...
            return Object2CharMaps.unmodifiable(new Object2CharArrayMap(keys, vals, entries.length));
        }
        Object2CharOpenHashMap newMap = new Object2CharOpenHashMap(entries.length, 0.75f);
        ...
        return Object2CharMaps.unmodifiable(newMap);
    }

    public static <K> Entry<K> entry(K key, char value) {
        return new AbstractObject2CharMap.BasicEntry<K>(key, value);
    }
