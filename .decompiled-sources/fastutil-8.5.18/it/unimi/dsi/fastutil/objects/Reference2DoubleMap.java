...
Map<K, Double> {
    ...
    default public void forEach(ObjectDoubleBiConsumer<? super K> consumer) {
        ObjectSet<Entry<K>> entrySet = this.reference2DoubleEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept((Object)entry.getKey(), entry.getDoubleValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super K, ? super Double> action) {
    ...
    @SafeVarargs
    public static <K> Reference2DoubleMap<K> ofEntries(Entry<K> ... entries) {
        if (entries.length == 0) {
            return Reference2DoubleMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Reference2DoubleMaps.singleton(entries[0].getKey(), entries[0].getDoubleValue());
        }
        if (entries.length <= 8) {
            ...
            return Reference2DoubleMaps.unmodifiable(new Reference2DoubleArrayMap(keys, vals, entries.length));
        }
        Reference2DoubleOpenHashMap newMap = new Reference2DoubleOpenHashMap(entries.length, 0.75f);
        for (Entry<K> entry : entries) {
            if (Double.doubleToRawLongBits(newMap.put(entry.getKey(), entry.getDoubleValue())) == 0L) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getKey());
        }
        return Reference2DoubleMaps.unmodifiable(newMap);
    }

    public static <K> Entry<K> entry(K key, double value) {
        return new AbstractReference2DoubleMap.BasicEntry<K>(key, value);
    }
