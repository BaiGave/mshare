...
Map<K, Long> {
    ...
    @SafeVarargs
    public static <K> Reference2LongMap<K> ofEntries(Entry<K> ... entries) {
        if (entries.length == 0) {
            return Reference2LongMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Reference2LongMaps.singleton(entries[0].getKey(), entries[0].getLongValue());
        }
        if (entries.length <= 8) {
            ...
            return Reference2LongMaps.unmodifiable(new Reference2LongArrayMap(keys, vals, entries.length));
        }
        Reference2LongOpenHashMap newMap = new Reference2LongOpenHashMap(entries.length, 0.75f);
        ...
        return Reference2LongMaps.unmodifiable(newMap);
    }

    public static <K> Entry<K> entry(K key, long value) {
        return new AbstractReference2LongMap.BasicEntry<K>(key, value);
    }
