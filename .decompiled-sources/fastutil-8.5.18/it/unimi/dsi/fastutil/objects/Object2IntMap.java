...
Map<K, Integer> {
    ...
    @SafeVarargs
    public static <K> Object2IntMap<K> ofEntries(Entry<K> ... entries) {
        if (entries.length == 0) {
            return Object2IntMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Object2IntMaps.singleton(entries[0].getKey(), entries[0].getIntValue());
        }
        if (entries.length <= 8) {
            ...
            return Object2IntMaps.unmodifiable(new Object2IntArrayMap(keys, vals, entries.length));
        }
        Object2IntOpenHashMap newMap = new Object2IntOpenHashMap(entries.length, 0.75f);
    ...
    public static <K> Entry<K> entry(K key, int value) {
        return new AbstractObject2IntMap.BasicEntry<K>(key, value);
    }
