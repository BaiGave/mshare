...
Map<K, Double> {
    ...
    @SafeVarargs
    public static <K> Object2DoubleMap<K> ofEntries(Entry<K> ... entries) {
        if (entries.length == 0) {
            return Object2DoubleMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Object2DoubleMaps.singleton(entries[0].getKey(), entries[0].getDoubleValue());
        }
        if (entries.length <= 8) {
            ...
            return Object2DoubleMaps.unmodifiable(new Object2DoubleArrayMap(keys, vals, entries.length));
        }
        Object2DoubleOpenHashMap newMap = new Object2DoubleOpenHashMap(entries.length, 0.75f);
        ...
        return Object2DoubleMaps.unmodifiable(newMap);
    }

    public static <K> Entry<K> entry(K key, double value) {
        return new AbstractObject2DoubleMap.BasicEntry<K>(key, value);
    }
