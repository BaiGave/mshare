...
import it.unimi.dsi.fastutil.objects.ObjectCharBiConsumer;
...
Map<K, Character> {
    ...
    default public void forEach(ObjectCharBiConsumer<? super K> consumer) {
        ObjectSet<Entry<K>> entrySet = this.reference2CharEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept((Object)entry.getKey(), entry.getCharValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super K, ? super Character> action) {
    ...
    @SafeVarargs
    public static <K> Reference2CharMap<K> ofEntries(Entry<K> ... entries) {
        if (entries.length == 0) {
            return Reference2CharMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Reference2CharMaps.singleton(entries[0].getKey(), entries[0].getCharValue());
        }
        if (entries.length <= 8) {
            ...
            return Reference2CharMaps.unmodifiable(new Reference2CharArrayMap(keys, vals, entries.length));
        }
        Reference2CharOpenHashMap newMap = new Reference2CharOpenHashMap(entries.length, 0.75f);
        for (Entry<K> entry : entries) {
            if (newMap.put(entry.getKey(), entry.getCharValue()) == '\u0000') continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getKey());
        }
        return Reference2CharMaps.unmodifiable(newMap);
    }

    public static <K> Entry<K> entry(K key, char value) {
        return new AbstractReference2CharMap.BasicEntry<K>(key, value);
    }
