...
Map<Long, Float> {
    ...
    default public void forEach(LongFloatBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.long2FloatEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getLongKey(), entry.getFloatValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Long, ? super Float> action) {
    ...
    public static Long2FloatMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Long2FloatMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Long2FloatMaps.singleton(entries[0].getLongKey(), entries[0].getFloatValue());
        }
        if (entries.length <= 8) {
            ...
            return Long2FloatMaps.unmodifiable(new Long2FloatArrayMap(keys, vals, entries.length));
        }
        Long2FloatOpenHashMap newMap = new Long2FloatOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (Float.floatToRawIntBits(newMap.put(entry.getLongKey(), entry.getFloatValue())) == 0) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getLongKey());
        }
        return Long2FloatMaps.unmodifiable(newMap);
    }

    public static Entry entry(long key, float value) {
