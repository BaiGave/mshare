...
Map<Long, V> {
    ...
    @SafeVarargs
    public static <V> Long2ObjectMap<V> ofEntries(Entry<V> ... entries) {
        if (entries.length == 0) {
            return Long2ObjectMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Long2ObjectMaps.singleton(entries[0].getLongKey(), entries[0].getValue());
        }
        if (entries.length <= 8) {
        ...
        Long2ObjectOpenHashMap newMap = new Long2ObjectOpenHashMap(entries.length, 0.75f);
    ...
    public static <V> Entry<V> entry(long key, V value) {
        return new AbstractLong2ObjectMap.BasicEntry<V>(key, value);
    }
