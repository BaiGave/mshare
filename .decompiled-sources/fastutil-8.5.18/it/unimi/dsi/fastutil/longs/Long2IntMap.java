...
import it.unimi.dsi.fastutil.longs.LongIntBiConsumer;
...
Map<Long, Integer> {
    ...
    default public void forEach(LongIntBiConsumer consumer) {
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Long, ? super Integer> action) {
    ...
    public static Long2IntMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Long2IntMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Long2IntMaps.singleton(entries[0].getLongKey(), entries[0].getIntValue());
        }
        if (entries.length <= 8) {
            ...
            return Long2IntMaps.unmodifiable(new Long2IntArrayMap(keys, vals, entries.length));
        }
        Long2IntOpenHashMap newMap = new Long2IntOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getLongKey(), entry.getIntValue()) == 0) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getLongKey());
        }
        return Long2IntMaps.unmodifiable(newMap);
    }

    public static Entry entry(long key, int value) {
