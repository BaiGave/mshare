...
import it.unimi.dsi.fastutil.longs.LongByteBiConsumer;
...
Map<Long, Byte> {
    ...
    default public void forEach(LongByteBiConsumer consumer) {
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Long, ? super Byte> action) {
    ...
    public static Long2ByteMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Long2ByteMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Long2ByteMaps.singleton(entries[0].getLongKey(), entries[0].getByteValue());
        }
        if (entries.length <= 8) {
            ...
            return Long2ByteMaps.unmodifiable(new Long2ByteArrayMap(keys, vals, entries.length));
        }
        Long2ByteOpenHashMap newMap = new Long2ByteOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getLongKey(), entry.getByteValue()) == 0) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getLongKey());
        }
        return Long2ByteMaps.unmodifiable(newMap);
    }

    public static Entry entry(long key, byte value) {
