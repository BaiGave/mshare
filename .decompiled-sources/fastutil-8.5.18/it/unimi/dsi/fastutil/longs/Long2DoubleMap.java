...
Map<Long, Double> {
    ...
    default public void forEach(LongDoubleBiConsumer consumer) {
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Long, ? super Double> action) {
    ...
    public static Long2DoubleMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Long2DoubleMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Long2DoubleMaps.singleton(entries[0].getLongKey(), entries[0].getDoubleValue());
        }
        if (entries.length <= 8) {
            ...
            return Long2DoubleMaps.unmodifiable(new Long2DoubleArrayMap(keys, vals, entries.length));
        }
        Long2DoubleOpenHashMap newMap = new Long2DoubleOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (Double.doubleToRawLongBits(newMap.put(entry.getLongKey(), entry.getDoubleValue())) == 0L) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getLongKey());
        }
        return Long2DoubleMaps.unmodifiable(newMap);
    }

    public static Entry entry(long key, double value) {
