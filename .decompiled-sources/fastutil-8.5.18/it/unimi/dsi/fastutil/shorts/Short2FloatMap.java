...
Map<Short, Float> {
    ...
    default public void forEach(ShortFloatBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.short2FloatEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getShortKey(), entry.getFloatValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Short, ? super Float> action) {
    ...
    public static Short2FloatMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Short2FloatMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Short2FloatMaps.singleton(entries[0].getShortKey(), entries[0].getFloatValue());
        }
        if (entries.length <= 8) {
            ...
            return Short2FloatMaps.unmodifiable(new Short2FloatArrayMap(keys, vals, entries.length));
        }
        Short2FloatOpenHashMap newMap = new Short2FloatOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (Float.floatToRawIntBits(newMap.put(entry.getShortKey(), entry.getFloatValue())) == 0) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getShortKey());
        }
        return Short2FloatMaps.unmodifiable(newMap);
    }

    public static Entry entry(short key, float value) {
