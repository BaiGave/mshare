...
import it.unimi.dsi.fastutil.shorts.ShortBooleanBiConsumer;
...
Map<Short, Boolean> {
    ...
    default public void forEach(ShortBooleanBiConsumer consumer) {
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Short, ? super Boolean> action) {
    ...
    public static Short2BooleanMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Short2BooleanMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Short2BooleanMaps.singleton(entries[0].getShortKey(), entries[0].getBooleanValue());
        }
        if (entries.length <= 8) {
            ...
            return Short2BooleanMaps.unmodifiable(new Short2BooleanArrayMap(keys, vals, entries.length));
        }
        Short2BooleanOpenHashMap newMap = new Short2BooleanOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (!newMap.put(entry.getShortKey(), entry.getBooleanValue())) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getShortKey());
        }
        return Short2BooleanMaps.unmodifiable(newMap);
    }

    public static Entry entry(short key, boolean value) {
