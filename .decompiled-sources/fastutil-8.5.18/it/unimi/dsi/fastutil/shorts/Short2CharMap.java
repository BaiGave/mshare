...
import it.unimi.dsi.fastutil.shorts.ShortCharBiConsumer;
...
Map<Short, Character> {
    ...
    default public void forEach(ShortCharBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.short2CharEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getShortKey(), entry.getCharValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Short, ? super Character> action) {
    ...
    public static Short2CharMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Short2CharMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Short2CharMaps.singleton(entries[0].getShortKey(), entries[0].getCharValue());
        }
        if (entries.length <= 8) {
            ...
            return Short2CharMaps.unmodifiable(new Short2CharArrayMap(keys, vals, entries.length));
        }
        Short2CharOpenHashMap newMap = new Short2CharOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getShortKey(), entry.getCharValue()) == '\u0000') continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getShortKey());
        }
        return Short2CharMaps.unmodifiable(newMap);
    }

    public static Entry entry(short key, char value) {
