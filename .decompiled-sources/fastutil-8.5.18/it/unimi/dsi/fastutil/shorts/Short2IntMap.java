...
import it.unimi.dsi.fastutil.shorts.ShortIntBiConsumer;
...
Map<Short, Integer> {
    ...
    default public void forEach(ShortIntBiConsumer consumer) {
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Short, ? super Integer> action) {
    ...
    public static Short2IntMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Short2IntMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Short2IntMaps.singleton(entries[0].getShortKey(), entries[0].getIntValue());
        }
        if (entries.length <= 8) {
            ...
            return Short2IntMaps.unmodifiable(new Short2IntArrayMap(keys, vals, entries.length));
        }
        Short2IntOpenHashMap newMap = new Short2IntOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getShortKey(), entry.getIntValue()) == 0) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getShortKey());
        }
        return Short2IntMaps.unmodifiable(newMap);
    }

    public static Entry entry(short key, int value) {
