...
import it.unimi.dsi.fastutil.shorts.ShortByteBiConsumer;
...
Map<Short, Byte> {
    ...
    default public void forEach(ShortByteBiConsumer consumer) {
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Short, ? super Byte> action) {
    ...
    public static Short2ByteMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Short2ByteMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Short2ByteMaps.singleton(entries[0].getShortKey(), entries[0].getByteValue());
        }
        if (entries.length <= 8) {
            ...
            return Short2ByteMaps.unmodifiable(new Short2ByteArrayMap(keys, vals, entries.length));
        }
        Short2ByteOpenHashMap newMap = new Short2ByteOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getShortKey(), entry.getByteValue()) == 0) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getShortKey());
        }
        return Short2ByteMaps.unmodifiable(newMap);
    }

    public static Entry entry(short key, byte value) {
