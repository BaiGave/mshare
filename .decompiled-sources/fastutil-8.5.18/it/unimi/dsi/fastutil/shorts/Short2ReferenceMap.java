...
import it.unimi.dsi.fastutil.shorts.ShortObjectBiConsumer;
...
Map<Short, V> {
    ...
    default public void forEach(ShortObjectBiConsumer<? super V> consumer) {
        ObjectSet<Entry<V>> entrySet = this.short2ReferenceEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getShortKey(), (Object)entry.getValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Short, ? super V> action) {
    ...
    @SafeVarargs
    public static <V> Short2ReferenceMap<V> ofEntries(Entry<V> ... entries) {
        if (entries.length == 0) {
            return Short2ReferenceMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Short2ReferenceMaps.singleton(entries[0].getShortKey(), entries[0].getValue());
        }
        if (entries.length <= 8) {
            ...
            return Short2ReferenceMaps.unmodifiable(new Short2ReferenceArrayMap(keys, vals, entries.length));
        }
        Short2ReferenceOpenHashMap newMap = new Short2ReferenceOpenHashMap(entries.length, 0.75f);
        for (Entry<V> entry : entries) {
            if (newMap.put(entry.getShortKey(), entry.getValue()) == null) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getShortKey());
        }
        return Short2ReferenceMaps.unmodifiable(newMap);
    }

    public static <V> Entry<V> entry(short key, V value) {
        return new AbstractShort2ReferenceMap.BasicEntry<V>(key, value);
    }
