...
Map<Short, Double> {
    ...
    default public void forEach(ShortDoubleBiConsumer consumer) {
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Short, ? super Double> action) {
    ...
    public static Short2DoubleMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Short2DoubleMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Short2DoubleMaps.singleton(entries[0].getShortKey(), entries[0].getDoubleValue());
        }
        if (entries.length <= 8) {
            ...
            return Short2DoubleMaps.unmodifiable(new Short2DoubleArrayMap(keys, vals, entries.length));
        }
        Short2DoubleOpenHashMap newMap = new Short2DoubleOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (Double.doubleToRawLongBits(newMap.put(entry.getShortKey(), entry.getDoubleValue())) == 0L) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getShortKey());
        }
        return Short2DoubleMaps.unmodifiable(newMap);
    }

    public static Entry entry(short key, double value) {
