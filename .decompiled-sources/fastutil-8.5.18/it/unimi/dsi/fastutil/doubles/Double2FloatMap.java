...
Map<Double, Float> {
    ...
    default public void forEach(DoubleFloatBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.double2FloatEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getDoubleKey(), entry.getFloatValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Double, ? super Float> action) {
    ...
    public static Double2FloatMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Double2FloatMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Double2FloatMaps.singleton(entries[0].getDoubleKey(), entries[0].getFloatValue());
        }
        if (entries.length <= 8) {
            ...
            return Double2FloatMaps.unmodifiable(new Double2FloatArrayMap(keys, vals, entries.length));
        }
        Double2FloatOpenHashMap newMap = new Double2FloatOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (Float.floatToRawIntBits(newMap.put(entry.getDoubleKey(), entry.getFloatValue())) == 0) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getDoubleKey());
        }
        return Double2FloatMaps.unmodifiable(newMap);
    }

    public static Entry entry(double key, float value) {
