...
Map<Double, Boolean> {
    ...
    default public void forEach(DoubleBooleanBiConsumer consumer) {
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Double, ? super Boolean> action) {
    ...
    public static Double2BooleanMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Double2BooleanMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Double2BooleanMaps.singleton(entries[0].getDoubleKey(), entries[0].getBooleanValue());
        }
        if (entries.length <= 8) {
            ...
            return Double2BooleanMaps.unmodifiable(new Double2BooleanArrayMap(keys, vals, entries.length));
        }
        Double2BooleanOpenHashMap newMap = new Double2BooleanOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (!newMap.put(entry.getDoubleKey(), entry.getBooleanValue())) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getDoubleKey());
        }
        return Double2BooleanMaps.unmodifiable(newMap);
    }

    public static Entry entry(double key, boolean value) {
