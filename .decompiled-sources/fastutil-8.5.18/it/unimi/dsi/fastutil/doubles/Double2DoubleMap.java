...
Map<Double, Double> {
    ...
    default public void forEach(DoubleDoubleBiConsumer consumer) {
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Double, ? super Double> action) {
    ...
    public static Double2DoubleMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Double2DoubleMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Double2DoubleMaps.singleton(entries[0].getDoubleKey(), entries[0].getDoubleValue());
        }
        if (entries.length <= 8) {
            ...
            return Double2DoubleMaps.unmodifiable(new Double2DoubleArrayMap(keys, vals, entries.length));
        }
        Double2DoubleOpenHashMap newMap = new Double2DoubleOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (Double.doubleToRawLongBits(newMap.put(entry.getDoubleKey(), entry.getDoubleValue())) == 0L) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getDoubleKey());
        }
        return Double2DoubleMaps.unmodifiable(newMap);
    }

    public static Entry entry(double key, double value) {
