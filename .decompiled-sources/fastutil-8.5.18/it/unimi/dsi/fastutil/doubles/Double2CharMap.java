...
import it.unimi.dsi.fastutil.doubles.DoubleCharBiConsumer;
...
Map<Double, Character> {
    ...
    default public void forEach(DoubleCharBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.double2CharEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getDoubleKey(), entry.getCharValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Double, ? super Character> action) {
    ...
    public static Double2CharMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Double2CharMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Double2CharMaps.singleton(entries[0].getDoubleKey(), entries[0].getCharValue());
        }
        if (entries.length <= 8) {
            ...
            return Double2CharMaps.unmodifiable(new Double2CharArrayMap(keys, vals, entries.length));
        }
        Double2CharOpenHashMap newMap = new Double2CharOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getDoubleKey(), entry.getCharValue()) == '\u0000') continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getDoubleKey());
        }
        return Double2CharMaps.unmodifiable(newMap);
    }

    public static Entry entry(double key, char value) {
