...
Map<Double, Byte> {
    ...
    default public void forEach(DoubleByteBiConsumer consumer) {
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Double, ? super Byte> action) {
    ...
    public static Double2ByteMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Double2ByteMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Double2ByteMaps.singleton(entries[0].getDoubleKey(), entries[0].getByteValue());
        }
        if (entries.length <= 8) {
            ...
            return Double2ByteMaps.unmodifiable(new Double2ByteArrayMap(keys, vals, entries.length));
        }
        Double2ByteOpenHashMap newMap = new Double2ByteOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getDoubleKey(), entry.getByteValue()) == 0) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getDoubleKey());
        }
        return Double2ByteMaps.unmodifiable(newMap);
    }

    public static Entry entry(double key, byte value) {
