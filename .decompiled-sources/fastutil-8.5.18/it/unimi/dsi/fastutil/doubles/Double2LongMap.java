...
Map<Double, Long> {
    ...
    default public void forEach(DoubleLongBiConsumer consumer) {
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Double, ? super Long> action) {
    ...
    public static Double2LongMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Double2LongMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Double2LongMaps.singleton(entries[0].getDoubleKey(), entries[0].getLongValue());
        }
        if (entries.length <= 8) {
            ...
            return Double2LongMaps.unmodifiable(new Double2LongArrayMap(keys, vals, entries.length));
        }
        Double2LongOpenHashMap newMap = new Double2LongOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getDoubleKey(), entry.getLongValue()) == 0L) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getDoubleKey());
        }
        return Double2LongMaps.unmodifiable(newMap);
    }

    public static Entry entry(double key, long value) {
