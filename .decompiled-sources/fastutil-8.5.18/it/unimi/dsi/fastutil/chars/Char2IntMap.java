...
Map<Character, Integer> {
    ...
    default public void forEach(CharIntBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.char2IntEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getCharKey(), entry.getIntValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Character, ? super Integer> action) {
    ...
    public static Char2IntMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Char2IntMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Char2IntMaps.singleton(entries[0].getCharKey(), entries[0].getIntValue());
        }
        if (entries.length <= 8) {
            ...
            return Char2IntMaps.unmodifiable(new Char2IntArrayMap(keys, vals, entries.length));
        }
        Char2IntOpenHashMap newMap = new Char2IntOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getCharKey(), entry.getIntValue()) == 0) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getCharKey());
        }
        return Char2IntMaps.unmodifiable(newMap);
    }

    public static Entry entry(char key, int value) {
