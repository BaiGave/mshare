...
Map<Character, Float> {
    ...
    default public void forEach(CharFloatBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.char2FloatEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getCharKey(), entry.getFloatValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Character, ? super Float> action) {
    ...
    default public boolean remove(char key, float value) {
        ...
        if (Float.floatToRawIntBits(curValue) != Float.floatToRawIntBits(value) || curValue == this.defaultReturnValue() && !this.containsKey(key)) {
            return false;
        }
        this.remove(key);
        return true;
    ...
    default public boolean replace(char key, float oldValue, float newValue) {
        float curValue = this.get(key);
        if (Float.floatToRawIntBits(curValue) != Float.floatToRawIntBits(oldValue) || curValue == this.defaultReturnValue() && !this.containsKey(key)) {
            return false;
        }
    ...
    public static Char2FloatMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Char2FloatMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Char2FloatMaps.singleton(entries[0].getCharKey(), entries[0].getFloatValue());
        }
        if (entries.length <= 8) {
            char[] keys = new char[entries.length];
            float[] vals = new float[entries.length];
            for (int i = 0; i < entries.length; ++i) {
                ...
                vals[i] = entry.getFloatValue();
            }
            return Char2FloatMaps.unmodifiable(new Char2FloatArrayMap(keys, vals, entries.length));
        }
        Char2FloatOpenHashMap newMap = new Char2FloatOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (Float.floatToRawIntBits(newMap.put(entry.getCharKey(), entry.getFloatValue())) == 0) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getCharKey());
        }
        return Char2FloatMaps.unmodifiable(newMap);
    }

    public static Entry entry(char key, float value) {
