...
import it.unimi.dsi.fastutil.chars.CharShortBiConsumer;
...
Map<Character, Short> {
    ...
    default public void forEach(CharShortBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.char2ShortEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getCharKey(), entry.getShortValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Character, ? super Short> action) {
    ...
    public static Char2ShortMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Char2ShortMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Char2ShortMaps.singleton(entries[0].getCharKey(), entries[0].getShortValue());
        }
        if (entries.length <= 8) {
            ...
            return Char2ShortMaps.unmodifiable(new Char2ShortArrayMap(keys, vals, entries.length));
        }
        Char2ShortOpenHashMap newMap = new Char2ShortOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getCharKey(), entry.getShortValue()) == 0) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getCharKey());
        }
        return Char2ShortMaps.unmodifiable(newMap);
    }

    public static Entry entry(char key, short value) {
