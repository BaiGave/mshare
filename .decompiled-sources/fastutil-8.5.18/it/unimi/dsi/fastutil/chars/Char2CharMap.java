...
import it.unimi.dsi.fastutil.chars.CharCharBiConsumer;
...
Map<Character, Character> {
    ...
    default public void forEach(CharCharBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.char2CharEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getCharKey(), entry.getCharValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Character, ? super Character> action) {
    ...
    public static Char2CharMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Char2CharMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Char2CharMaps.singleton(entries[0].getCharKey(), entries[0].getCharValue());
        }
        if (entries.length <= 8) {
            char[] keys = new char[entries.length];
            char[] vals = new char[entries.length];
            for (int i = 0; i < entries.length; ++i) {
                ...
                vals[i] = entry.getCharValue();
            }
            return Char2CharMaps.unmodifiable(new Char2CharArrayMap(keys, vals, entries.length));
        }
        Char2CharOpenHashMap newMap = new Char2CharOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getCharKey(), entry.getCharValue()) == '\u0000') continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getCharKey());
        }
        return Char2CharMaps.unmodifiable(newMap);
    }

    public static Entry entry(char key, char value) {
