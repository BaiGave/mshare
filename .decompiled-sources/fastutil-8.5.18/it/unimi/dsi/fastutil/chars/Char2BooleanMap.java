...
import it.unimi.dsi.fastutil.chars.CharBooleanBiConsumer;
...
Map<Character, Boolean> {
    ...
    default public void forEach(CharBooleanBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.char2BooleanEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getCharKey(), entry.getBooleanValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Character, ? super Boolean> action) {
    ...
    public static Char2BooleanMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Char2BooleanMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Char2BooleanMaps.singleton(entries[0].getCharKey(), entries[0].getBooleanValue());
        }
        if (entries.length <= 8) {
            ...
            return Char2BooleanMaps.unmodifiable(new Char2BooleanArrayMap(keys, vals, entries.length));
        }
        Char2BooleanOpenHashMap newMap = new Char2BooleanOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (!newMap.put(entry.getCharKey(), entry.getBooleanValue())) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getCharKey());
        }
        return Char2BooleanMaps.unmodifiable(newMap);
    }

    public static Entry entry(char key, boolean value) {
