...
import it.unimi.dsi.fastutil.chars.CharByteBiConsumer;
...
Map<Character, Byte> {
    ...
    default public void forEach(CharByteBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.char2ByteEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getCharKey(), entry.getByteValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Character, ? super Byte> action) {
    ...
    public static Char2ByteMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Char2ByteMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Char2ByteMaps.singleton(entries[0].getCharKey(), entries[0].getByteValue());
        }
        if (entries.length <= 8) {
            char[] keys = new char[entries.length];
            byte[] vals = new byte[entries.length];
            for (int i = 0; i < entries.length; ++i) {
                ...
                vals[i] = entry.getByteValue();
            }
            return Char2ByteMaps.unmodifiable(new Char2ByteArrayMap(keys, vals, entries.length));
        }
        Char2ByteOpenHashMap newMap = new Char2ByteOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getCharKey(), entry.getByteValue()) == 0) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getCharKey());
        }
        return Char2ByteMaps.unmodifiable(newMap);
    }

    public static Entry entry(char key, byte value) {
