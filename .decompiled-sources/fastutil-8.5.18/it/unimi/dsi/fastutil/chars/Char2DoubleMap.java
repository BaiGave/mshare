...
Map<Character, Double> {
    ...
    default public void forEach(CharDoubleBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.char2DoubleEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getCharKey(), entry.getDoubleValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Character, ? super Double> action) {
    ...
    default public boolean remove(char key, double value) {
        ...
        if (Double.doubleToRawLongBits(curValue) != Double.doubleToRawLongBits(value) || curValue == this.defaultReturnValue() && !this.containsKey(key)) {
            return false;
        }
        this.remove(key);
        return true;
    ...
    default public boolean replace(char key, double oldValue, double newValue) {
        double curValue = this.get(key);
        if (Double.doubleToRawLongBits(curValue) != Double.doubleToRawLongBits(oldValue) || curValue == this.defaultReturnValue() && !this.containsKey(key)) {
            return false;
        }
    ...
    public static Char2DoubleMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Char2DoubleMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Char2DoubleMaps.singleton(entries[0].getCharKey(), entries[0].getDoubleValue());
        }
        if (entries.length <= 8) {
            char[] keys = new char[entries.length];
            double[] vals = new double[entries.length];
            for (int i = 0; i < entries.length; ++i) {
                Entry entry = entries[i];
                keys[i] = entry.getCharKey();
                for (int j = 0; j < i; ++j) {
                    if (keys[j] != keys[i]) continue;
                    throw new IllegalArgumentException("duplicate key: " + keys[i]);
                }
                vals[i] = entry.getDoubleValue();
            }
            return Char2DoubleMaps.unmodifiable(new Char2DoubleArrayMap(keys, vals, entries.length));
        }
        Char2DoubleOpenHashMap newMap = new Char2DoubleOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (Double.doubleToRawLongBits(newMap.put(entry.getCharKey(), entry.getDoubleValue())) == 0L) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getCharKey());
        }
        return Char2DoubleMaps.unmodifiable(newMap);
    }

    public static Entry entry(char key, double value) {
