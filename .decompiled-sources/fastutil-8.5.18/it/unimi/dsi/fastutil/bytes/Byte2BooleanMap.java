...
Map<Byte, Boolean> {
    ...
    default public void forEach(ByteBooleanBiConsumer consumer) {
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Byte, ? super Boolean> action) {
    ...
    public static Byte2BooleanMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Byte2BooleanMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Byte2BooleanMaps.singleton(entries[0].getByteKey(), entries[0].getBooleanValue());
        }
        if (entries.length <= 8) {
            ...
            return Byte2BooleanMaps.unmodifiable(new Byte2BooleanArrayMap(keys, vals, entries.length));
        }
        Byte2BooleanOpenHashMap newMap = new Byte2BooleanOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (!newMap.put(entry.getByteKey(), entry.getBooleanValue())) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getByteKey());
        }
        return Byte2BooleanMaps.unmodifiable(newMap);
    }

    public static Entry entry(byte key, boolean value) {
