...
import it.unimi.dsi.fastutil.bytes.ByteLongBiConsumer;
...
Map<Byte, Long> {
    ...
    default public void forEach(ByteLongBiConsumer consumer) {
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Byte, ? super Long> action) {
    ...
    public static Byte2LongMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Byte2LongMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Byte2LongMaps.singleton(entries[0].getByteKey(), entries[0].getLongValue());
        }
        if (entries.length <= 8) {
            ...
            return Byte2LongMaps.unmodifiable(new Byte2LongArrayMap(keys, vals, entries.length));
        }
        Byte2LongOpenHashMap newMap = new Byte2LongOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getByteKey(), entry.getLongValue()) == 0L) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getByteKey());
        }
        return Byte2LongMaps.unmodifiable(newMap);
    }

    public static Entry entry(byte key, long value) {
