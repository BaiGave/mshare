...
import it.unimi.dsi.fastutil.bytes.ByteCharBiConsumer;
...
Map<Byte, Character> {
    ...
    default public void forEach(ByteCharBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.byte2CharEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getByteKey(), entry.getCharValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Byte, ? super Character> action) {
    ...
    public static Byte2CharMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Byte2CharMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Byte2CharMaps.singleton(entries[0].getByteKey(), entries[0].getCharValue());
        }
        if (entries.length <= 8) {
            ...
            return Byte2CharMaps.unmodifiable(new Byte2CharArrayMap(keys, vals, entries.length));
        }
        Byte2CharOpenHashMap newMap = new Byte2CharOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getByteKey(), entry.getCharValue()) == '\u0000') continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getByteKey());
        }
        return Byte2CharMaps.unmodifiable(newMap);
    }

    public static Entry entry(byte key, char value) {
