...
import it.unimi.dsi.fastutil.bytes.ByteByteBiConsumer;
...
Hash {
    ...
    @Override
    public byte mergeByte(byte k, byte v, ByteBinaryOperator remappingFunction) {
        byte newValue;
        Objects.requireNonNull(remappingFunction);
        int pos = this.find(k);
        if (pos < 0) {
    ...
    @Override
    public void forEach(ByteByteBiConsumer consumer) {
        if (this.containsNullKey) {
        ...
        if (this.values == null) {
            this.values = new AbstractByteCollection(){
                ...
                public void forEach(ByteConsumer consumer) {
                    ...
                    byte[] value = Byte2ByteOpenCustomHashMap.this.value;
                    if (Byte2ByteOpenCustomHashMap.this.containsNullKey) {
                        consumer.accept(value[Byte2ByteOpenCustomHashMap.this.n]);
                    }
    ...
    extends AbstractByteSet {
        ...
        public void forEach(ByteConsumer consumer) {
            byte[] key = Byte2ByteOpenCustomHashMap.this.key;
            if (Byte2ByteOpenCustomHashMap.this.containsNullKey) {
                consumer.accept(key[Byte2ByteOpenCustomHashMap.this.n]);
            }
            int pos = Byte2ByteOpenCustomHashMap.this.n;
            while (pos-- != 0) {
                byte k = key[pos];
