...
Map<Byte, Double> {
    ...
    default public void forEach(ByteDoubleBiConsumer consumer) {
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Byte, ? super Double> action) {
    ...
    public static Byte2DoubleMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Byte2DoubleMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Byte2DoubleMaps.singleton(entries[0].getByteKey(), entries[0].getDoubleValue());
        }
        if (entries.length <= 8) {
            ...
            return Byte2DoubleMaps.unmodifiable(new Byte2DoubleArrayMap(keys, vals, entries.length));
        }
        Byte2DoubleOpenHashMap newMap = new Byte2DoubleOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (Double.doubleToRawLongBits(newMap.put(entry.getByteKey(), entry.getDoubleValue())) == 0L) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getByteKey());
        }
        return Byte2DoubleMaps.unmodifiable(newMap);
    }

    public static Entry entry(byte key, double value) {
