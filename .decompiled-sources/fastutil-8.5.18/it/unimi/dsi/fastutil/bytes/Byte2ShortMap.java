...
import it.unimi.dsi.fastutil.bytes.ByteShortBiConsumer;
...
Map<Byte, Short> {
    ...
    default public void forEach(ByteShortBiConsumer consumer) {
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Byte, ? super Short> action) {
    ...
    public static Byte2ShortMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Byte2ShortMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Byte2ShortMaps.singleton(entries[0].getByteKey(), entries[0].getShortValue());
        }
        if (entries.length <= 8) {
            ...
            return Byte2ShortMaps.unmodifiable(new Byte2ShortArrayMap(keys, vals, entries.length));
        }
        Byte2ShortOpenHashMap newMap = new Byte2ShortOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getByteKey(), entry.getShortValue()) == 0) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getByteKey());
        }
        return Byte2ShortMaps.unmodifiable(newMap);
    }

    public static Entry entry(byte key, short value) {
