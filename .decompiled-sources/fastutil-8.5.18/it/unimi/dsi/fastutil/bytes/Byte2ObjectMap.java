...
Map<Byte, V> {
    ...
    @SafeVarargs
    public static <V> Byte2ObjectMap<V> ofEntries(Entry<V> ... entries) {
        if (entries.length == 0) {
            return Byte2ObjectMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Byte2ObjectMaps.singleton(entries[0].getByteKey(), entries[0].getValue());
        }
        if (entries.length <= 8) {
            ...
            return Byte2ObjectMaps.unmodifiable(new Byte2ObjectArrayMap(keys, vals, entries.length));
        }
        Byte2ObjectOpenHashMap newMap = new Byte2ObjectOpenHashMap(entries.length, 0.75f);
        ...
        return Byte2ObjectMaps.unmodifiable(newMap);
    }

    public static <V> Entry<V> entry(byte key, V value) {
        return new AbstractByte2ObjectMap.BasicEntry<V>(key, value);
    }
