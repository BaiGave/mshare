...
import it.unimi.dsi.fastutil.bytes.ByteIntBiConsumer;
...
Map<Byte, Integer> {
    ...
    default public void forEach(ByteIntBiConsumer consumer) {
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Byte, ? super Integer> action) {
    ...
    public static Byte2IntMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Byte2IntMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Byte2IntMaps.singleton(entries[0].getByteKey(), entries[0].getIntValue());
        }
        if (entries.length <= 8) {
            ...
            return Byte2IntMaps.unmodifiable(new Byte2IntArrayMap(keys, vals, entries.length));
        }
        Byte2IntOpenHashMap newMap = new Byte2IntOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getByteKey(), entry.getIntValue()) == 0) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getByteKey());
        }
        return Byte2IntMaps.unmodifiable(newMap);
    }

    public static Entry entry(byte key, int value) {
