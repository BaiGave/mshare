...
import it.unimi.dsi.fastutil.bytes.ByteObjectBiConsumer;
...
Map<Byte, V> {
    ...
    default public void forEach(ByteObjectBiConsumer<? super V> consumer) {
        ObjectSet<Entry<V>> entrySet = this.byte2ReferenceEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getByteKey(), (Object)entry.getValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Byte, ? super V> action) {
    ...
    @SafeVarargs
    public static <V> Byte2ReferenceMap<V> ofEntries(Entry<V> ... entries) {
        if (entries.length == 0) {
            return Byte2ReferenceMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Byte2ReferenceMaps.singleton(entries[0].getByteKey(), entries[0].getValue());
        }
        if (entries.length <= 8) {
            ...
            return Byte2ReferenceMaps.unmodifiable(new Byte2ReferenceArrayMap(keys, vals, entries.length));
        }
        Byte2ReferenceOpenHashMap newMap = new Byte2ReferenceOpenHashMap(entries.length, 0.75f);
        for (Entry<V> entry : entries) {
            if (newMap.put(entry.getByteKey(), entry.getValue()) == null) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getByteKey());
        }
        return Byte2ReferenceMaps.unmodifiable(newMap);
    }

    public static <V> Entry<V> entry(byte key, V value) {
        return new AbstractByte2ReferenceMap.BasicEntry<V>(key, value);
    }
