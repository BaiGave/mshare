...
Map<Byte, Float> {
    ...
    default public void forEach(ByteFloatBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.byte2FloatEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getByteKey(), entry.getFloatValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Byte, ? super Float> action) {
    ...
    public static Byte2FloatMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Byte2FloatMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Byte2FloatMaps.singleton(entries[0].getByteKey(), entries[0].getFloatValue());
        }
        if (entries.length <= 8) {
            ...
            return Byte2FloatMaps.unmodifiable(new Byte2FloatArrayMap(keys, vals, entries.length));
        }
        Byte2FloatOpenHashMap newMap = new Byte2FloatOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (Float.floatToRawIntBits(newMap.put(entry.getByteKey(), entry.getFloatValue())) == 0) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getByteKey());
        }
        return Byte2FloatMaps.unmodifiable(newMap);
    }

    public static Entry entry(byte key, float value) {
