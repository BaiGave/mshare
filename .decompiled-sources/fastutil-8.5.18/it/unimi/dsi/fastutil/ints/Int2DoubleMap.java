...
Map<Integer, Double> {
    ...
    default public void forEach(IntDoubleBiConsumer consumer) {
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Integer, ? super Double> action) {
    ...
    public static Int2DoubleMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Int2DoubleMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Int2DoubleMaps.singleton(entries[0].getIntKey(), entries[0].getDoubleValue());
        }
        if (entries.length <= 8) {
            ...
            return Int2DoubleMaps.unmodifiable(new Int2DoubleArrayMap(keys, vals, entries.length));
        }
        Int2DoubleOpenHashMap newMap = new Int2DoubleOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (Double.doubleToRawLongBits(newMap.put(entry.getIntKey(), entry.getDoubleValue())) == 0L) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getIntKey());
        }
        return Int2DoubleMaps.unmodifiable(newMap);
    }

    public static Entry entry(int key, double value) {
