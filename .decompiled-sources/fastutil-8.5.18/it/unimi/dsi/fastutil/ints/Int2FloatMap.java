...
Map<Integer, Float> {
    ...
    default public void forEach(IntFloatBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.int2FloatEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getIntKey(), entry.getFloatValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Integer, ? super Float> action) {
    ...
    public static Int2FloatMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Int2FloatMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Int2FloatMaps.singleton(entries[0].getIntKey(), entries[0].getFloatValue());
        }
        if (entries.length <= 8) {
            ...
            return Int2FloatMaps.unmodifiable(new Int2FloatArrayMap(keys, vals, entries.length));
        }
        Int2FloatOpenHashMap newMap = new Int2FloatOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (Float.floatToRawIntBits(newMap.put(entry.getIntKey(), entry.getFloatValue())) == 0) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getIntKey());
        }
        return Int2FloatMaps.unmodifiable(newMap);
    }

    public static Entry entry(int key, float value) {
