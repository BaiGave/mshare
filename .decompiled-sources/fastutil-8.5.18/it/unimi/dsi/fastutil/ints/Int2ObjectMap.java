...
Map<Integer, V> {
    ...
    @SafeVarargs
    public static <V> Int2ObjectMap<V> ofEntries(Entry<V> ... entries) {
        if (entries.length == 0) {
            return Int2ObjectMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Int2ObjectMaps.singleton(entries[0].getIntKey(), entries[0].getValue());
        }
        if (entries.length <= 8) {
            ...
            return Int2ObjectMaps.unmodifiable(new Int2ObjectArrayMap(keys, vals, entries.length));
        }
        Int2ObjectOpenHashMap newMap = new Int2ObjectOpenHashMap(entries.length, 0.75f);
        ...
        return Int2ObjectMaps.unmodifiable(newMap);
    }

    public static <V> Entry<V> entry(int key, V value) {
        return new AbstractInt2ObjectMap.BasicEntry<V>(key, value);
    }
