...
Map<Integer, Integer> {
    ...
    default public void forEach(IntIntBiConsumer consumer) {
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Integer, ? super Integer> action) {
    ...
    public static Int2IntMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Int2IntMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Int2IntMaps.singleton(entries[0].getIntKey(), entries[0].getIntValue());
        }
        if (entries.length <= 8) {
            ...
            return Int2IntMaps.unmodifiable(new Int2IntArrayMap(keys, vals, entries.length));
        }
        Int2IntOpenHashMap newMap = new Int2IntOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getIntKey(), entry.getIntValue()) == 0) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getIntKey());
        }
        return Int2IntMaps.unmodifiable(newMap);
    }

    public static Entry entry(int key, int value) {
