...
Map<Integer, V> {
    ...
    default public void forEach(IntObjectBiConsumer<? super V> consumer) {
        ObjectSet<Entry<V>> entrySet = this.int2ReferenceEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getIntKey(), (Object)entry.getValue());
    ...
    @SafeVarargs
    public static <V> Int2ReferenceMap<V> ofEntries(Entry<V> ... entries) {
        if (entries.length == 0) {
            return Int2ReferenceMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Int2ReferenceMaps.singleton(entries[0].getIntKey(), entries[0].getValue());
        }
        if (entries.length <= 8) {
            ...
            return Int2ReferenceMaps.unmodifiable(new Int2ReferenceArrayMap(keys, vals, entries.length));
        }
        Int2ReferenceOpenHashMap newMap = new Int2ReferenceOpenHashMap(entries.length, 0.75f);
        for (Entry<V> entry : entries) {
            if (newMap.put(entry.getIntKey(), entry.getValue()) == null) continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getIntKey());
        }
        return Int2ReferenceMaps.unmodifiable(newMap);
    }

    public static <V> Entry<V> entry(int key, V value) {
        return new AbstractInt2ReferenceMap.BasicEntry<V>(key, value);
    }
