...
import it.unimi.dsi.fastutil.ints.IntCharBiConsumer;
...
Map<Integer, Character> {
    ...
    default public void forEach(IntCharBiConsumer consumer) {
        ObjectSet<Entry> entrySet = this.int2CharEntrySet();
        Consumer<Entry> wrappingConsumer = entry -> consumer.accept(entry.getIntKey(), entry.getCharValue());
    ...
    @Override
    @Deprecated
    default public void forEach(BiConsumer<? super Integer, ? super Character> action) {
    ...
    public static Int2CharMap ofEntries(Entry ... entries) {
        if (entries.length == 0) {
            return Int2CharMaps.EMPTY_MAP;
        }
        if (entries.length == 1) {
            return Int2CharMaps.singleton(entries[0].getIntKey(), entries[0].getCharValue());
        }
        if (entries.length <= 8) {
            ...
            return Int2CharMaps.unmodifiable(new Int2CharArrayMap(keys, vals, entries.length));
        }
        Int2CharOpenHashMap newMap = new Int2CharOpenHashMap(entries.length, 0.75f);
        for (Entry entry : entries) {
            if (newMap.put(entry.getIntKey(), entry.getCharValue()) == '\u0000') continue;
            throw new IllegalArgumentException("duplicate key: " + entry.getIntKey());
        }
        return Int2CharMaps.unmodifiable(newMap);
    }

    public static Entry entry(int key, char value) {
