...
public final class QuadSpriteBaker {
    ...
    public static void bakeSprite(MutableQuadView quad, TextureAtlasSprite sprite, int bakeFlags) {
        if (quad.nominalFace() != null && (4 & bakeFlags) != 0) {
            QuadSpriteBaker.applyModifier(quad, UVLOCKERS[quad.nominalFace().get3DDataValue()]);
        } else if ((0x20 & bakeFlags) == 0) {
            QuadSpriteBaker.applyModifier(quad, (q, i) -> q.uv(i, q.u(i) * 0.0625f, q.v(i) * 0.0625f));
        }
        int rotation = bakeFlags & 3;
        if (rotation != 0) {
            QuadSpriteBaker.applyModifier(quad, ROTATIONS[rotation]);
        }
        if ((8 & bakeFlags) != 0) {
            QuadSpriteBaker.applyModifier(quad, (q, i) -> q.uv(i, 1.0f - q.u(i), q.v(i)));
        }
        if ((0x10 & bakeFlags) != 0) {
            QuadSpriteBaker.applyModifier(quad, (q, i) -> q.uv(i, q.u(i), 1.0f - q.v(i)));
        }
        QuadSpriteBaker.interpolate(quad, sprite);
    }
    ...
    static {
        QuadSpriteBaker.UVLOCKERS[Direction.DOWN.get3DDataValue()] = (q, i) -> q.uv(i, q.x(i), 1.0f - q.z(i));
        QuadSpriteBaker.UVLOCKERS[Direction.UP.get3DDataValue()] = (q, i) -> q.uv(i, q.x(i), q.z(i));
        QuadSpriteBaker.UVLOCKERS[Direction.NORTH.get3DDataValue()] = (q, i) -> q.uv(i, 1.0f - q.x(i), 1.0f - q.y(i));
        QuadSpriteBaker.UVLOCKERS[Direction.SOUTH.get3DDataValue()] = (q, i) -> q.uv(i, q.x(i), 1.0f - q.y(i));
        QuadSpriteBaker.UVLOCKERS[Direction.WEST.get3DDataValue()] = (q, i) -> q.uv(i, q.z(i), 1.0f - q.y(i));
        QuadSpriteBaker.UVLOCKERS[Direction.EAST.get3DDataValue()] = (q, i) -> q.uv(i, 1.0f - q.z(i), 1.0f - q.y(i));
    }
