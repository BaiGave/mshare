...
public class Archiver {
    ...
    public void create(String format, Path target, Path directory) throws IOException, ArchiveException {
        ...
        try (Object outputStream = ArchiveStreamFactory.DEFAULT.createArchiveOutputStream(format, Files.newOutputStream(target, new OpenOption[0]));){
    ...
    extends SimpleFileVisitor<Path> {
        ...
        protected FileVisitResult visit(Path path, BasicFileAttributes attrs, boolean isFile) throws IOException {
            ...
            if (!name.isEmpty()) {
                Object archiveEntry = ((ArchiveOutputStream)this.outputStream).createArchiveEntry(path, isFile || name.endsWith("/") ? name : name + "/", this.linkOptions);
                ((ArchiveOutputStream)this.outputStream).putArchiveEntry(archiveEntry);
                if (isFile) {
                    ((CompressFilterOutputStream)this.outputStream).write(path);
                }
