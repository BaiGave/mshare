...
public record Product(TypeTemplate f, TypeTemplate g) implements TypeTemplate
{
    ...
    private <A, B, LS, RS, LT, RT> TypedOptic<?, ?, A, B> cap(FamilyOptic<A, B> lo, FamilyOptic<A, B> ro, int index) {
        ...
        TypedOptic<?, ?, A, B> lp = lo.apply(index);
        TypedOptic<?, ?, A, B> rp = ro.apply(index);
        Optic<TraversalP.Mu, ?, ?, A, B> l = lp.upCast(bound).orElseThrow(IllegalArgumentException::new);
        Optic<TraversalP.Mu, ?, ?, A, B> r = rp.upCast(bound).orElseThrow(IllegalArgumentException::new);
        final Traversal<?, ?, A, B> lt = Optics.toTraversal(l);
        final Traversal<?, ?, A, B> rt = Optics.toTraversal(r);
        return new TypedOptic(ImmutableSet.of(bound), DSL.and(lp.sType(), rp.sType()), DSL.and(lp.tType(), rp.tType()), lp.aType(), lp.bType(), new Traversal<Pair<LS, RS>, Pair<LT, RT>, A, B>(){
    ...
    extends Type<Pair<F, G>> {
        ...
        public int hashCode() {
            if (this.hashCode == 0) {
                ...
                this.hashCode = result = 31 * result + this.second.hashCode();
            }
