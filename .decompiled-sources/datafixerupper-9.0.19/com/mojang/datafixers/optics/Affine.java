...
Optic<AffineP.Mu, S, T, A, B> {
    ...
    default public <P extends K2> FunctionType<App2<P, A, B>, App2<P, S, T>> eval(App<? extends AffineP.Mu, P> proof) {
        ...
        return input -> cartesian.dimap(cocartesian.left(cartesian.rmap(cartesian.first(input), p -> this.set(p.getFirst(), p.getSecond()))), s -> this.preview(s).map(Either::right, a -> Either.left(Pair.of(a, s))), Either::unwrap);
    }
