...
implements UserApiService {
    ...
    public UserApiService.UserProperties fetchProperties() throws AuthenticationException {
        try {
            ...
            ImmutableMap.Builder bannedScopes = ImmutableMap.builder();
            if (response != null) {
                ...
                if (privileges != null) {
                    YggdrasilUserApiService.addFlagIfUserHasPrivilege(privileges.getOnlineChat(), UserApiService.UserFlag.CHAT_ALLOWED, flags);
                    YggdrasilUserApiService.addFlagIfUserHasPrivilege(privileges.getMultiplayerServer(), UserApiService.UserFlag.SERVERS_ALLOWED, flags);
                    YggdrasilUserApiService.addFlagIfUserHasPrivilege(privileges.getMultiplayerRealms(), UserApiService.UserFlag.REALMS_ALLOWED, flags);
                    YggdrasilUserApiService.addFlagIfUserHasPrivilege(privileges.getTelemetry(), UserApiService.UserFlag.TELEMETRY_ENABLED, flags);
                    YggdrasilUserApiService.addFlagIfUserHasPrivilege(privileges.getOptionalTelemetry(), UserApiService.UserFlag.OPTIONAL_TELEMETRY_AVAILABLE, flags);
                }
                if ((profanityFilterPreferences = response.profanityFilterPreferences()) != null && profanityFilterPreferences.enabled()) {
                    flags.add((Object)UserApiService.UserFlag.PROFANITY_FILTER_ENABLED);
                }
                if (response.banStatus() != null) {
                    response.banStatus().bannedScopes().forEach((scopeType, scope) -> bannedScopes.put(scopeType, new BanDetails(scope.banId(), scope.expires(), scope.reason(), scope.reasonMessage())));
                }
            }
            return new UserApiService.UserProperties((Set<UserApiService.UserFlag>)((Object)flags.build()), bannedScopes.build());
        }
        catch (MinecraftClientHttpException e) {
